...
const FormBuilder = ({ fields = [], onSubmit, initialValues = {}, layout = 'vertical' }) => {
  ...
  const renderField = (field) => {
    ...
    const baseRules = [
      ...(required ? [{ required: true, message: `Please provide ${label}` }] : []),
      ...rules,
    ];
          ...
          >
            ...
            >
              {options.map((option) => (
                ...
                >
                  {typeof option === 'string' ? capitalize(option) : option.label}
                </Select.Option>
              ))}
            </Select>
          </Form.Item>
