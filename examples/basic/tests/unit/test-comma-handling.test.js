...
describe('Comma Handling Tests', () => {
  test('should handle trailing commas correctly in object literals', () => {
    ...
    for (const file of distFiles) {
      ...
      const objectLiteralPattern = /(module\.exports|exports)\s*=\s*\{[\s\S]*?\}/g;
    ...
    console.log(`📊 Summary: ${foundObjectLiterals} object literals checked, ${syntaxErrors} potential issues`);
...
function checkCommaIssues(objectContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/\/\*\s*@common:endif\s*\*\/\s*,\s*$/) && i < lines.length - 1) {
        ...
        if (nextLine.match(/^\/\*\s*@common:if/) || nextLine.match(/^\s*\}/)) {
          issues.push(`Line ${i + 1}: Trailing comma after macro could become orphaned`);
        }
      }
    }
