...
describe('Macro export shape validation for CJS chunks', () => {
	...
	for (const chunkFile of chunkFiles) {
		test(`should have valid macro export shapes in ${chunkFile}`, () => {
			...
			assert.ok(fs.existsSync(targetFile), `Target file not found: ${targetFile}`);
			...
			if (!hasModuleExports && !hasExports) {
				console.log(`ℹ️  No CommonJS exports found in ${chunkFile} (this may be expected)`);
				return;
			}
			...
			if (invalidLines.length > 0) {
				...
				assert.fail(`Found ${invalidLines.length} invalid macro export patterns in ${chunkFile} - commas should be INSIDE macro boundaries`);
			}
	...
	test('should have correct transformData pattern in module-exports-pattern', () => {
		...
		const hasCorrectPattern = content.includes('transformData, /* @common:endif */');
		assert.ok(hasCorrectPattern, 'module-exports-pattern should have transformData with comma inside macro');
		
		// Test that there's no extra comma outside
		const hasIncorrectPattern = content.includes('/* @common:endif */,') && 
			content.split('\n').some(line => line.includes('transformData') && line.includes('/* @common:endif */,'));
		
		if (hasIncorrectPattern) {
			const problematicLines = content.split('\n').filter(line => 
				line.includes('transformData') && line.includes('/* @common:endif */,')
			);
	...
	test('should have correct hashString pattern in pure-cjs-helper', () => {
		const targetFile = path.join(__dirname, "dist", "cjs-modules_pure-cjs-helper_js.js");
		...
		const hasCorrectPattern = content.includes('exports.hashString = function(input) {') &&
			content.includes('} /* @common:endif */');
		
		if (!hasCorrectPattern) {
			console.log('ℹ️  hashString pattern not found in pure-cjs-helper (this may be expected)');
		} else {
