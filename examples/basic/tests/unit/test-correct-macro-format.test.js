...
describe('Correct Macro Format Tests', () => {
  test('should have correct macro format for export assignments', () => {
    ...
    distFiles.forEach(file => {
      ...
      const correctPattern = /\/\*\s*@common:if\s*\[condition="[^"]+"\]\s*\*\/\s*(?:exports|module\.exports)\.\w+\s*=[\s\S]*?\/\*\s*@common:endif\s*\*\//g;
      ...
      if (incorrectMatches.length > 0) {
        console.log(`❌ ${file}: ${incorrectMatches.length} incorrect patterns found`);
        incorrectMatches.forEach((match, i) => {
          const preview = match[0].length > 100 ? match[0].substring(0, 100) + '...' : match[0];
          console.log(`  ${i + 1}. ${preview}`);
        });
      }
    });
    
    console.log(`📊 Total: ${totalIncorrectPatterns} incorrect, ${totalCorrectPatterns} correct patterns`);
  ...
  test('should demonstrate expected correct format examples', () => {
    ...
    console.log('   /* @common:if [condition="treeShake.cjs-data-processor.processArray"] */ exports.processArray = processArray; /* @common:endif */');
    console.log('');
    console.log('2. Object assignment:');
    console.log('   /* @common:if [condition="treeShake.cjs-legacy-utils.constants"] */ exports.constants = {');
    ...
    console.log('   /* @common:if [condition="treeShake.cjs-pure-helper.processData"] */ exports.processData = function(data) {');
    ...
    console.log('     /* @common:if [condition="treeShake.cjs-module-exports.calculateSum"] */ calculateSum /* @common:endif */,');
    console.log('     /* @common:if [condition="treeShake.cjs-module-exports.calculateAverage"] */ calculateAverage /* @common:endif */');
    console.log('   };');
