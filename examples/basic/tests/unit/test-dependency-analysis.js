...
const sharingDataMatch = mainContent.match(/__webpack_require__\.initializeSharingData\s*=\s*{[^}]+scopeToSharingDataMapping[^}]+}/);
if (sharingDataMatch) {
    ...
    for (const moduleKey of sharedModules) {
        ...
        const moduleType = moduleKey.includes("lib") && !["api-lib", "component-lib", "utility-lib"].includes(moduleKey) ? "CommonJS" : "ESM/External";
        console.log(`  ${isProvideShared ? "✅" : "❌"} ${moduleKey} (${moduleType})`);
    }
} else {
...
if (fs.existsSync(shareUsagePath)) {
    ...
    const consumeSharedModules = Object.keys(shareUsageData.consume_shared_modules || {});
    ...
    for (const moduleKey of consumeSharedModules) {
        ...
        console.log(`  📊 ${moduleKey}: ${usedCount} used, ${unusedCount} unused exports`);
    }
    
    console.log("\nCommonJS modules NOT in ConsumeShared tracking:");
    const commonJSModules = ["legacy-utils-lib", "data-processor-lib", "pure-cjs-helper-lib"];
...
console.log("4. ✅ ESM modules go through ConsumeShared and get macro annotations");
console.log("5. 📝 This explains why CommonJS modules don't appear in share-usage.json");

console.log("\n🎯 Conclusion:");
console.log("The ShareUsagePlugin is working correctly - it tracks ConsumeShared modules.");
console.log("CommonJS modules are ProvideShared but not ConsumeShared when accessed via require().");