...
for (const file of expectedESMFiles) {
    ...
    const macroMatches = content.match(/@common:if\s*\[condition="treeShake\.[^"]+"\]/g) || [];
...
for (const file of expectedCommonJSFiles) {
    ...
    const hasMacros = content.includes("@common:if") || content.includes("treeShake");
    const hasCommonJS = content.includes("module.exports") && content.includes("exports.");
    
    if (!hasMacros && hasCommonJS) {
        console.log(`✅ ${file}: No macros found (correct), CommonJS structure present`);
        testsPassed++;
    } else if (hasMacros) {
