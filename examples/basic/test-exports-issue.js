...
console.log("🔍 Testing specific macro positioning issues in CommonJS exports...\n");

const filePath = "/Users/bytedance/RustroverProjects/rspack/examples/basic/dist/cjs-modules_pure-cjs-helper_js.js";
...
const incorrectExportsRegex = /\/\*\s*@common:if[^*]+\*\/\s*exports\.[\w]+\s*\/\*\s*@common:endif[^*]*\*\/\s*=/g;
...
const incorrectModuleExportsRegex = /\/\*\s*@common:if[^*]+\*\/\s*module\.exports\.[\w]+\s*\/\*\s*@common:endif[^*]*\*\/\s*=/g;
const incorrectModuleExportsMatches = content.match(incorrectModuleExportsRegex) || [];

if (incorrectExportsMatches.length > 0) {
    console.log(`❌ Found ${incorrectExportsMatches.length} incorrect exports.prop patterns:`);
...
if (incorrectModuleExportsMatches.length > 0) {
    console.log(`❌ Found ${incorrectModuleExportsMatches.length} incorrect module.exports.prop patterns:`);
...
lines.forEach((line, index) => {
    if (line.includes('@common:if') && line.includes('@common:endif') && line.includes('=')) {
        ...
        if (macroEndIndex !== -1 && macroEndIndex < equalsIndex) {
            ...
            console.log(`   Issue: @common:endif appears at position ${macroEndIndex}, but = appears at position ${equalsIndex}`);
        }
    }
});
...
if (moduleToExportsMatches.length > 0) {
    console.log(`  - Found module.exports patterns: ${moduleToExportsMatches.join(', ')}`);
}
...
// Save report to file
const reportPath = path.join(path.dirname(filePath), "../exports-issue-report.json");
...
if (totalIssues > 0) {
    ...
    console.log(`   - Incorrect exports positioning: ${report.patterns.incorrectExportsCount}`);
    console.log(`   - Incorrect module.exports positioning: ${report.patterns.incorrectModuleExportsCount}`);
