...
it("Insert module path names with concatenateModules", () => {
	...
	expect(content).toMatch(/const m\d+_filename =/);
	expect(content).toMatch(/const m\d+_dirname =/);
	expect(content).toContain(JSON.stringify(path.resolve(sourceDir, "./foo.js")));
	expect(content).toContain(JSON.stringify(path.resolve(sourceDir, ".")));
