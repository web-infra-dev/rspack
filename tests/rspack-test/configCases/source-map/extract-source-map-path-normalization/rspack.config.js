			...
			apply(compiler) {
				compiler.hooks.afterCompile.tap("Test", compilation => {
					for (const file of compilation.fileDependencies) {
						// Should already be normalized (do not contains '../' like '/extract-source-map-path-normalization/lib/../lib/src/foo.js')
						...
						// fileDependencies' path is not normalized, then file path mismatch, watchpack will
