...
import { type Compilation, experiments } from '@rspack/core';
...
const clientEntry = path.join(
  import.meta.dirname,
  'src/framework/entry.client.js',
);
const rootPath = path.join(import.meta.dirname, 'src/Root.js');
const pageAPath = path.join(import.meta.dirname, 'src/pages/PageA.js');
const pageANestedPath = path.join(
  import.meta.dirname,
  'src/pages/PageANested.js',
);
const pageBPath = path.join(import.meta.dirname, 'src/pages/PageB.js');
...
function readAsset(compilation: Compilation, file: string) {
  return compilation.getAsset(file)!.source.source().toString();
}

function findCssAsset(compilation: Compilation, marker: string) {
