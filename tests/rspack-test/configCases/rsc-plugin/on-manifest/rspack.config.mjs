    ...
    plugins: [
      new ServerPlugin({
        onManifest(manifest) {
          ...
          const clientPath = `${path.join(import.meta.dirname, './src/Client.js')}?foo=1`;
          ...
          const appPath = path.join(import.meta.dirname, './src/App.js');
      ...
      new rspack.DefinePlugin({
        CLIENT_PATH: JSON.stringify(
          `${path.resolve(import.meta.dirname, 'src/Client.js')}?foo=1`,
        ),
      }),
    ],
    optimization: {
