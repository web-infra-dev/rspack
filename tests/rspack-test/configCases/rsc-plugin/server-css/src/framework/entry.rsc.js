...
import { Page1Nested } from '../pages/Page1Nested';
...
it('should expose entry CSS only for server-entry components', async () => {
	...
	expect(Page1Nested.entryCssFiles).toBeDefined();
	expect(Page1Nested.entryCssFiles.length).toBe(1);
	expect(Page1Nested.entryCssFiles[0]).toMatch(/\.css$/);

	expect(Page2.entryCssFiles).toBeDefined();
	expect(Page2.entryCssFiles.length).toBe(1);
	expect(Page2.entryCssFiles[0]).toMatch(/\.css$/);

	expect(Page1.entryCssFiles[0]).not.toBe(Page1Nested.entryCssFiles[0]);
	expect(Page1.entryCssFiles[0]).not.toBe(Page2.entryCssFiles[0]);
	expect(Page1Nested.entryCssFiles[0]).not.toBe(Page2.entryCssFiles[0]);
});
