...
const page1Path = path.join(import.meta.dirname, 'src/pages/Page1.js');
const page1NestedPath = path.join(
  import.meta.dirname,
  'src/pages/Page1Nested.js',
);
const page2Path = path.join(import.meta.dirname, 'src/pages/Page2.js');
    ...
    plugins: [
      ...
      definePlugin((compiler) => {
        compiler.hooks.done.tap('AssertServerCssChunking', (stats) => {
          ...
          const sharedCssFile = findCssAsset(compilation, 'shared-server-css');
          expect(entryCssFiles).toContain(sharedCssFile);
          expect(readAsset(compilation, sharedCssFile)).toContain(
            'shared-server-css',
          );
          expect(readAsset(compilation, sharedCssFile)).toContain(
            'shared-nested-server-css',
          );
