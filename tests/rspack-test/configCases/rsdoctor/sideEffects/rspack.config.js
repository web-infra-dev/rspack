      ...
      apply(compiler) {
        compiler.hooks.compilation.tap(
          ...
          (compilation) => {
            ...
            hooks.moduleGraph.tap('TestPlugin::SideEffects', (moduleGraph) => {
              ...
              const modulesWithSideEffects = modules.filter(
                (m) =>
                  m.sideEffectsLocations && m.sideEffectsLocations.length > 0,
              );
              ...
              const utilsModule = modulesWithSideEffects.find(
              ...
              const pureModule = modules.find(
                (m) => m.path && m.path.includes('pure.js'),
              );
              if (pureModule) {
                expect(pureModule.sideEffectsLocations?.length ?? 0).toBe(0);
              }
