...
it("should inline constants from modules with import and re-export declarations", () => {
  ...
  expect(block.includes(`((/* inlined export .REMOVE_with_import */123)).toBe(123)`)).toBe(true);
  expect(block.includes(`((/* inlined export .REMOVE_with_export_all */456)).toBe(456)`)).toBe(true);
  expect(block.includes(`((/* inlined export .REMOVE_with_named_reexport */789)).toBe(789)`)).toBe(true);
  expect(block.includes(`((/* inlined export .REMOVE_with_namespace_reexport */654)).toBe(654)`)).toBe(true);
  expect(block.includes(`((/* inlined export .REMOVE_with_side_effect_reexport */987)).toBe(987)`)).toBe(true);
  expect(block.includes(`((/* inlined export .REMOVE_with_all_module_declarations */321)).toBe(321)`)).toBe(true);
});
