...
const exportTypes = ['text', 'css-style-sheet', 'style', 'link'] as const;

/**
 * @param exportType the css module exportType
 * @returns a single config that builds an isolated bundle
 */
const makeConfig = (exportType: (typeof exportTypes)[number]) =>
  defineConfig({
    externals: {
      fs: 'node-commonjs fs',
...
export default defineConfig(exportTypes.map(makeConfig));
