...
it("should create a style tag for wrapper-style.css own content", () => {
  const allCSS = Array.from(window.document.getElementsByTagName("style")).map(
    style => style.textContent,
  );

  expect(allCSS.some(css => css.includes("padding: 10px"))).toBe(true);
});

it("should create a style tag for base.css when imported by a style-type parent", () => {
  const allCSS = Array.from(window.document.getElementsByTagName("style")).map(
    style => style.textContent,
  );

  expect(allCSS.some(css => css.includes("color: red"))).toBe(true);
});
