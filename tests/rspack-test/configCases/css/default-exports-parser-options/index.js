...
it("should able to import with default and named exports", () => {
	expect(style1.default).toEqual(nsObj({ foo: 'style_module_css_namespace-foo' }));
	expect(style1.foo).toEqual("style_module_css_namespace-foo");
	expect(style2).toEqual(nsObj({ foo: 'style_module_css_default-foo' }));
	expect(foo).toEqual("style_module_css_named-foo");
});
