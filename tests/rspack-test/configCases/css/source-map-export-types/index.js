...
it(`should generate a valid source map for ${label}`, () => {
  ...
  // runtime stylesheet helper for `css-style-sheet`). In runtimeMode:
  // "rspack", the same calls are emitted through `__rspack_context`.
  // Either way, this must be the full generated wrapper, not the raw CSS
  // or just its JS literal form.
  ...
  expect(cssModuleSourcesContent).toMatch(
    /__(?:webpack_require__|rspack_context)/
  );
  ...
  if (exportType === "text") {
    ...
    expect(cssModuleSourcesContent).toMatch(
      /__(?:webpack_require__|rspack_context)\.css/
    );
