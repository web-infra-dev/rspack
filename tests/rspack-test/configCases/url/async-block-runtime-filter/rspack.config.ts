...
export default defineConfig(
  [true, false].map((includeUsed, index): Configuration => ({
    mode: 'production',
    devtool: false,
    target: 'web',
    entry: includeUsed
        ...
        compiler.hooks.compilation.tap(
          ...
          (compilation) => {
            compilation.hooks.afterSeal.tap('CheckUrlRuntimeFilter', () => {
              ...
              if (includeUsed) {
                expect([
                  ...(moduleGraph.getUsedExports(issuer, 'used') as string[]),
                ]).toEqual(['loadAsset']);
              }
              expect([
                ...(moduleGraph.getUsedExports(issuer, 'unused') as string[]),
              ]).toEqual(['value']);
              ...
              expect(entryConnection.getActiveState('unused')).toBe(
                includeUsed,
              );
