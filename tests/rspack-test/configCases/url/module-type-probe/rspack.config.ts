...
import { NormalModule, NormalModuleReplacementPlugin } from '@rspack/core';
  ...
  plugins: [
    ...
    definePlugin((compiler) => {
      compiler.hooks.compilation.tap('CheckTypeProbe', (compilation) => {
        const builds: (string | undefined)[] = [];
        compilation.hooks.buildModule.tap('CheckTypeProbe', (module) => {
          builds.push(
            module instanceof NormalModule ? module.resource : undefined,
          );
        });
