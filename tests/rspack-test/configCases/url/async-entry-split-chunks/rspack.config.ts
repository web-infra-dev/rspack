...
export default defineConfig(
  ['jsonp', 'import', 'require', 'async-node'].flatMap((loading) =>
    ([true, 'relative'] as const).flatMap((url, mode) =>
      ['disabled', 'default-vendors', 'enforced'].map(
        (split): Configuration => ({
                ...
                (compilation) => {
                  compilation.hooks.processAssets.tap(
                    ...
                    () => {
                      ...
                      const targetChunks = [
                        ...compilation.chunkGraph.getModuleChunksIterable(
                          target,
                        ),
                      ];
