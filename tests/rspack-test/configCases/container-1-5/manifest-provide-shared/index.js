...
for (const suffix of ['', '-stats']) {
  it(`should retain the expose chunk when a local dependency is provided (${suffix || 'manifest'})`, () => {
    ...
    const chunkName = NAMED_EXPOSE ? 'expose-mount' : 'mount_js';
    const exposeFile = `${CASE_INDEX}-${chunkName}.js`;
    ...
    if (PROVIDED_REQUEST === './static.js') {
      ...
      expect(shared.assets.css.sync).toContain(`${CASE_INDEX}-${chunkName}.css`);
