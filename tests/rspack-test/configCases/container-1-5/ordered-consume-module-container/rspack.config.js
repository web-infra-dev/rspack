...
module.exports = (_, { testPath }) => ['async-only', 'initial'].map(name => ({
  name,
  target: 'node',
  entry: name === 'initial'
  ...
  resolve: {
    alias: { lib: require.resolve('../ordered-consume-startup/node_modules/lib') },
  },
  ...
  plugins: [
    ...
    new ConsumeSharedPlugin({
      enhanced: true,
      consumes: {
        lib: { shareScope: ['primary', 'default'], eager: true, requiredVersion: false },
      },
    }),
  ],
}));
