it("should ignore", function () {
	...
	// DIFF: webpackIgnore will not be respected if it is not a boolean
	...
	const args = ["file3.css", document.baseURI || window.location.href];
	const url7 = new URL(...args);
	expect(url7.pathname.endsWith("file3.css")).toBe(true);
	const url8 = new URL(document.baseURI || window.location.href);
	expect(url8.toString()).toBe(document.baseURI || window.location.href);
	const url9 = new URL(window.location.href);
	expect(url9.toString()).toBe(window.location.href);
	const url10 = new URL(/* webpackIgnore: true */ window.location.href);
