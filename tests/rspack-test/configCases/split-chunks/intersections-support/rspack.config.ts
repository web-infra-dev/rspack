...
import { type Configuration, experiments } from '@rspack/core';
...
function addEntry(name: string, selected: number[]) {
  entry[name] = `./${name}`;
...
export default defineConfig(
  [false, true]
    .flatMap((usedExports) =>
      ['all', 'pairs', 'common'].map((selection) => ({
        usedExports,
        selection,
      })),
    )
    .map<Configuration>(({ usedExports, selection }, index) => ({
      mode: 'production',
      target: 'node',
      entry,
      output: {
        ...
        definePlugin({
          apply(compiler) {
            compiler.hooks.done.tap('AssertIntersectionSupport', (stats) => {
              ...
              expect(shared.length).toBe(moduleCount);
              ...
              expect(common.length).toBe(
                selection === 'all' ? 0 : selection === 'pairs' ? 2 : 1,
              );
              for (const module of shared) {
                expect(module.chunks?.length).toBe(selection === 'all' ? 2 : 4);
              }
              for (let i = 0; i < moduleCount; i += 2) {
                ...
                expect(
                  left?.chunks?.filter((chunk) =>
                    right?.chunks?.includes(chunk),
                  ).length,
                ).toBe(selection === 'all' ? 1 : 3);
              }
            });
          },
        }),
