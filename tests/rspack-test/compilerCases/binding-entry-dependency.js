...
function createContextCacheCase(method, hook, request) {
	let root;
	let contextA;
	let contextB;

	return {
		description: `should keep the raw context in the ${method} dependency cache key`,
		options() {
			root = fs.mkdtempSync(
				path.join(os.tmpdir(), `rspack-binding-${method}-`)
			);
			contextA = path.join(root, "a");
			contextB = path.join(root, "b");
			...
			return {
				...
				plugins: [
					{
						apply(compiler) {
							compiler.hooks[hook].tapPromise(
...
let lazyCompilationRoot;

/** @type {import('@rspack/test-tools').TCompilerCaseConfig[]} */
module.exports = [
	{
		description:
			"should keep unnamed entries and named/global includes eager during lazy compilation",
		options() {
			lazyCompilationRoot = fs.mkdtempSync(
				path.join(os.tmpdir(), "rspack-binding-lazy-")
			);
			write(
				lazyCompilationRoot,
				"src/main.js",
				"globalThis.named = 'NAMED_ENTRY';\n"
			);
			write(
				lazyCompilationRoot,
				"src/global.js",
				"globalThis.global = 'GLOBAL_ENTRY';\n"
			);
			write(
				lazyCompilationRoot,
		...
		async build(_context, compiler) {
			...
			try {
				...
				fs.rmSync(lazyCompilationRoot, { force: true, recursive: true });
			}
		}
	},
	createContextCacheCase("addEntry", "make", "entry.js"),
	createContextCacheCase("addInclude", "finishMake", "include.js")
];
