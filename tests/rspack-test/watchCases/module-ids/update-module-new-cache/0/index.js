...
it("should have correct log when incremental enabled", async () => {
  ...
  const incrementalLog = /LOG from rspack\.incremental\.moduleIds[\s\S]*?(?=\nLOG from |$)/.exec(statsString);
  if (!process.env.RSPACK_INCREMENTAL_WATCH_TEST) return;
  if (WATCH_STEP === "1") expect(incrementalLog).not.toBeNull();
  if (!incrementalLog) return;
