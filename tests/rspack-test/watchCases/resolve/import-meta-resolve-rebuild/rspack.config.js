    ...
    compiler.hooks.compilation.tap(
      ...
      (compilation) => {
        ...
        compilation.hooks.finishModules.tap(
          'CheckUrlEntryBlocksPlugin',
          (modules) => {
            if (this.incremental) expect(issuerBuilds).toBe(1);
            ...
            const directUrls = issuer.dependencies.filter(
              (dep) => dep.type === 'import.meta.resolve',
            );
            expect(directUrls).toHaveLength(0);
            const dependencies = issuer.blocks[0].dependencies;
            expect(dependencies).toHaveLength(1);
            expect(dependencies[0].type).toBe('import.meta.resolve');
            expect(
...
module.exports = [false, true].flatMap((cache) =>
  [false, undefined].map((incremental) => {
    ...
    return {
      ...
      plugins: [new CheckUrlEntryBlocksPlugin(incremental !== false)],
    };
  }),
);
