...
const reporters: RstestConfig['reporters'] = testFilter
  ? ['verbose' as const]
  : ['default' as const];
if (process.env.CI) {
  reporters.push(
    new StreamedEventReporter(
      path.join(__dirname, '../../', 'rspack-test-event-report.txt'),
    ),
  );
}
...
const sharedConfig = defineProject({
  ...
  env: {
    ...
    __RSPACK_TEST_TOOLS_PATH__: path.resolve(
      root,
