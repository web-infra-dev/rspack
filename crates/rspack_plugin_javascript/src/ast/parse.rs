...
pub fn parse(
  ...
  match parse_js(
    ...
    Err(errs) => Err(Error::BatchErrors(
      errs
        .into_iter()
        .map(|err| ecma_parse_error_to_rspack_error(err, filename, fm.src.to_string(), module_type))
        .collect::<Vec<_>>(),
    )),
  }
}
...
pub fn parse_js_code(js_code: String, module_type: &ModuleType) -> Result<Program, Error> {
  ...
  match parse_js(
    ...
    Err(errs) => Err(Error::BatchErrors(
      errs
        .into_iter()
        .map(|err| ecma_parse_error_to_rspack_error(err, "", fm.src.to_string(), module_type))
        .collect::<Vec<_>>(),
    )),
  }
}
