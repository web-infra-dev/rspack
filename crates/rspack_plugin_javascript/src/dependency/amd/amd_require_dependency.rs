...
impl DependencyTemplate for AMDRequireDependency {
  fn apply(
    ...
    if self.array_range.is_some() && self.function_range.is_none() {
      ...
      source.replace(self.outer_range.0, array_range.0 - 1, &start_block, None);
      source.replace(array_range.1, self.outer_range.1 - 1, &end_block, None);
      return;
    }
    ...
    if self.function_range.is_some() && self.array_range.is_none() {
      ...
      source.replace(self.outer_range.0, function_range.0 - 1, &start_block, None);
      source.replace(function_range.1, self.outer_range.1 - 1, &end_block, None);
      return;
    }
      ...
      && self.error_callback_range.is_some()
    {
      ...
      source.replace(array_range.1, function_range.0 - 1, "; (", None);
      ...
      source.replace(
        function_range.1,
        error_callback_range.0 - 1,
        error_range_block,
        None,
      );

      source.replace(
        error_callback_range.1,
        self.outer_range.1 - 1,
        end_block,
        None,
      );
