...
pub fn esm_import_dependency_apply<T: ModuleDependency>(
  ...
  let is_async_module = matches!(target_module, Some(target_module) if ModuleGraph::is_async(&compilation.collect_build_module_graph_effects_artifact, &target_module.identifier()));
  if is_async_module {
