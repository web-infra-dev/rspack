...
impl CommonJsExportsDependencyTemplate {
  ...
  // Helper to detect ConsumeShared modules for macro generation
  fn is_consume_shared_module(module_type: &rspack_core::ModuleType) -> bool {
    matches!(module_type, rspack_core::ModuleType::ConsumeShared)
  }

  // Extract ConsumeShared info for tree-shaking macros
  fn get_consume_shared_info(
    &self,
    module_graph: &ModuleGraph,
    parent_module_id: &rspack_core::ModuleIdentifier,
  ) -> Option<String> {
    if let Some(module) = module_graph.module_by_identifier(parent_module_id) {
      if Self::is_consume_shared_module(module.module_type()) {
...
impl DependencyTemplate for CommonJsExportsDependencyTemplate {
  fn render(
    ...
    // Check for ConsumeShared context for macro generation
    let consume_shared_info = module_graph
      .get_parent_module(&dep.id)
      .and_then(|parent_id| self.get_consume_shared_info(&module_graph, parent_id));
    ...
    if dep.base.is_expression() {
      if let Some(UsedName::Normal(used)) = used {
        let export_access = property_access(&used, 0);
        let content = if let Some(share_key) = &consume_shared_info {
          let export_name = used.last().unwrap_or(&dep.names[0]);
          format!("/* @common:if [condition=\"treeShake.{share_key}.{export_name}\"] */{base}{export_access} /* @common:endif */")
        } else {
          format!("{base}{export_access}")
        };
      ...
      if let Some(value_range) = &dep.value_range {
        if let Some(UsedName::Normal(used)) = used {
          if !used.is_empty() {
            ...
            let final_content = if let Some(share_key) = &consume_shared_info {
              let export_name = used.last().unwrap_or(&dep.names[0]);
              format!(
                "/* @common:if [condition=\"treeShake.{share_key}.{export_name}\"] */{content}"
              )
            } else {
              content
            };

            source.replace(dep.range.start, value_range.start, &final_content, None);
            ...
            source.replace(value_range.end, dep.range.end, end_content, None);
          } else {
            panic!("Unexpected base type");
          }
        } else {
