...
impl ModuleConcatenationPlugin {
  ...
  fn try_to_add(
    ...
    let module_readable_string = module.readable_identifier(&options.context).to_string();
    ...
    if !missing_chunks.is_empty() {
      let problem_string = {
        ...
        let chunks = chunk_graph
          ...
          .collect::<Vec<_>>();
        format!("Module {} is not in the same chunk(s) (expected in chunk(s) {}, module is in chunk(s) {})",module_readable_string,missing_chunks_list.join(", "),chunks.join(", "))
      };
