...
use rspack_core::{
  ...
  DependencyId, ExportsInfoArtifact, GetTargetResult, InnerGraphMapValue, InnerGraphState,
  InnerGraphUsageOperation, Plugin, PrefetchExportsInfoMode, get_target,
};
...
async fn finish_modules(
  ...
  // SAFETY: We need mutable access to extract inner_graph states from parser_and_generator.
  // The CompilationFinishModules hook provides &Compilation, but we need to mutate
  // module-local parser state. This is safe because we only mutate parser metadata,
  // not the module graph structure.
  #[allow(invalid_reference_casting)]
  let compilation_mut = unsafe { &mut *(compilation as *const Compilation as *mut Compilation) };
  let module_graph = compilation_mut.get_module_graph_mut();
  ...
  #[allow(invalid_reference_casting)]
  let compilation_mut2 = unsafe { &mut *(compilation as *const Compilation as *mut Compilation) };
  let module_graph = compilation_mut2.get_module_graph_mut();
