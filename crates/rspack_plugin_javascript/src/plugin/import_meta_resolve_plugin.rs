...
pub(super) async fn optimize_chunks(&self, compilation: &mut Compilation) -> Result<Option<bool>> {
  ...
  for (issuer, _) in module_graph.modules() {
    for connection in module_graph.get_outgoing_connections(issuer) {
      if module_graph
      ...
      if !matches!(group.kind, ChunkGroupKind::Entrypoint { .. }) {
        continue;
      }
      let chunks = graph
        .chunk_graph
