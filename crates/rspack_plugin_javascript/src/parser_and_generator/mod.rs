...
use crate::{
  ...
  dependency::{
    ...
    ExportsBase, ModuleArgumentDependency, ModuleDecoratorDependency, OBJECT_PROTOTYPE_METHODS,
  },
...
impl JavaScriptParserAndGenerator {
  ...
  fn commonjs_concatenation_bailout_reason(
    ...
    Self::runtime_concatenation_bailout_reason(module, module_graph)
  }

  fn runtime_concatenation_bailout_reason(
    module: &dyn Module,
    module_graph: &ModuleGraph,
  ) -> Option<Cow<'static, str>> {
    if module.get_dependencies().iter().any(|dependency_id| {
      // The decorator rewrites and subsequently mutates the factory-local
      // `module` argument. It cannot be lowered into a chunk-scope binding.
      // Looking at the dependency itself is necessary here because runtime
      // requirements are finalized later during code generation.
      module_graph
        .dependency_by_id(dependency_id)
        .as_any()
        .is::<ModuleDecoratorDependency>()
    }) {
      return Some("Module uses a CommonJS module decorator".into());
    }
    ...
    if let Some(dependencies) = module.get_presentational_dependencies() {
      for dependency in dependencies {
        if dependency.as_any().is::<ModuleArgumentDependency>() {
          return Some("Module uses module".into());
        }
  ...
  pub fn get_concatenation_bailout_reason_with_commonjs(
    ...
    } else if let Some(reason) = Self::runtime_concatenation_bailout_reason(module, module_graph) {
      // An ESM syntax marker does not make CommonJS-only execution state
      // (`module`, `exports`, the module decorator, ...) safe to scope-hoist.
      // Keep those modules behind the same factory boundary as pure CommonJS.
      return Some(reason);
    }
