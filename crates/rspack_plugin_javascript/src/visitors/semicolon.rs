...
/// Scans the parser's flat node storage, reading spans only for ASI-relevant nodes.
/// Requires successful parser output before AST transformations, which may leave detached nodes.
pub fn collect(ast: &Ast<'_>, semicolons: &mut FxHashSet<u32>, tokens: &[Token]) {
  let mut collector = InsertedSemicolons { semicolons, tokens };
