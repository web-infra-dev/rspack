...
use crate::parser_plugin::is_logic_op;
...
impl JavascriptParser<'_> {
  ...
  pub fn is_pure_expr(&mut self, expr: &Expr, comments_start_pos: BytePos) -> bool {
    match expr {
      ...
      Expr::Call(call) => {
        let res = if call.span.lo.0 - comments_start_pos.0 > 12
          && !self.has_pure_flag_comment(call.span.lo)
        {
          false
        } else {
  ...
  fn is_pure_identifier(&mut self, _ident: &Ident) -> bool {
    // TODO: trait
    true
  }
