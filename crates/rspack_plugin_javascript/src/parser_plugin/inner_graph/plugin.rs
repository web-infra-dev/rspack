...
use swc_core::{
  ...
  ecma::ast::{ClassMember, Decl, DefaultDecl, ExportDefaultExpr, ModuleDecl, Pat, Stmt},
};
...
pub static TOP_LEVEL_SYMBOL: &'static str = "inner graph top level symbol";
...
impl JavascriptParserPlugin for InnerGraphPlugin {
  ...
  fn pre_statement(
    ...
    if matches!(parser.top_level_scope, TopLevelScope::Top)
      && let Stmt::Decl(Decl::Fn(fn_decl)) = stmt
    {
  ...
  fn pre_module_declaration(
    ...
    if let ModuleDecl::ExportDefaultExpr(ExportDefaultExpr { expr, .. }) = export_decl
      ...
      && is_pure_expression(export_part, self.unresolved_context, parser.comments.into())
    {
      let variable = Self::tag_top_level_symbol(parser, &DEFAULT_STAR_JS_WORD);
      ...
      parser
        .inner_graph
        .statement_with_top_level_symbol
        .insert(export_span.clone(), variable);

      if !export_part.is_fn_expr() && !export_part.is_lit() {
        parser
          .inner_graph
          .statement_pure_part
      ...
      && let Some(init) = &decl.init
    {
        ...
        )
      {
        ...
        parser
          ...
          .class_with_top_level_symbol
          .insert(init.span(), v);
      } else if is_pure_expression(&init, self.unresolved_context, parser.comments.into()) {
        let v = Self::tag_top_level_symbol(parser, name);
        parser
          .inner_graph
        ...
        if !init.is_fn_expr() && !init.is_lit() {
  ...
  fn statement(&self, parser: &mut crate::visitors::JavascriptParser, stmt: &Stmt) -> Option<bool> {
    if !parser.inner_graph.is_enabled() || !matches!(parser.top_level_scope, TopLevelScope::Top) {
      return None;
    }
  ...
  fn declarator(
      ...
      .get(&decl.span())
    {
      ...
      if parser.inner_graph.pure_declarators.contains(&decl.span) {
        ...
        let init_start = init.span().real_lo();
        let init_end = init.span().real_hi();
        ...
        InnerGraphPlugin::on_usage(
          ...
          Box::new(move |parser, used_by_exports| {
            if !matches!(used_by_exports, Some(UsedByExports::Bool(true)) | None) {
              let mut dep =
                PureExpressionDependency::new(init_start, init_end, *parser.module_identifier);
              dep.set_used_by_exports(used_by_exports);
              parser.dependencies.push(Box::new(dep));
            }
          }),
