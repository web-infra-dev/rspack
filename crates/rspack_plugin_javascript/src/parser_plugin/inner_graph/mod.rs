...
impl JavascriptParser<'_> {
  ...
  fn on_usage(&mut self, on_usage_callback: UsageCallback) {
    if self.inner_graph_state.is_enabled() {
...
impl JavascriptParserPlugin for InnerGraphPlugin {
  ...
  fn pre_statement(
    ...
    if !parser.inner_graph_state.is_enabled() {
      None
    } else if in_top_level(parser)
      && let Some(fn_decl) = stmt.as_decl().and_then(|decl| decl.as_fn_decl())
    {
  ...
  fn block_pre_statement(
    ...
    if !parser.inner_graph_state.is_enabled() {
      ...
      && let Some(class_decl) = stmt.as_decl().and_then(|decl| decl.as_class())
  ...
  fn block_pre_module_declration(
    ...
    if let Some(export_default_decl) = decl.as_export_default_decl() {
      let name = "*default*";
      let f = parser.expect_tag_top_level_symbol(&Atom::new(name));
  ...
  fn class_extends_expression(
    ...
    if in_top_level(parser)
      && let pos = ident.map(|ident| ident.span.hi).unwrap_or(classy.span_lo())
      && parser.is_pure_class(classy, pos)
      && let Some(f) = parser
        .inner_graph_state
