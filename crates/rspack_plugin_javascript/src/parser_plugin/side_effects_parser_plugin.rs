...
use swc_experimental_ecma_ast::{
  ...
  ModuleItem, ObjectPatProp, OptChainBase, Pat, Program, Prop, PropName, PropOrSpread,
  SimpleAssignTarget, Span, Span as AstSpan, Stmt, SuperProp, VarDecl, VarDeclKind, VarDeclOrExpr,
  Visit, VisitWith,
};
...
impl IdentifierReadCollector {
  ...
  fn collect_identifier_read(&mut self, ident: &Ident<'_>) {
    self.reads.push(compat_atom(&ident.sym));
  }

  fn visit_prop_name<'a>(&mut self, prop_name: &PropName<'a>) {
    if let PropName::Computed(computed) = prop_name {
      computed.expr.visit_with(self);
    }
  }

  fn visit_object_prop<'a>(&mut self, prop: &Prop<'a>) {
    match prop {
      Prop::Shorthand(ident) => {
        self.collect_identifier_read(ident);
      }
      Prop::KeyValue(prop) => {
  ...
  fn visit_expr(&mut self, expr: &Expr<'a>) {
    match expr {
      ...
      Expr::Fn(_) | Expr::Arrow(_) => {
        // Creating a function does not evaluate its body or parameter defaults.
      }
      Expr::Member(member) => {
      ...
      Expr::Object(object) => {
        for prop in &object.props {
          match prop {
            ...
            PropOrSpread::Prop(prop) => self.visit_object_prop(prop),
          }
        }
      }
      Expr::Class(class) => {
        ...
        for member in &class.class.body {
          match member {
            ...
            ClassMember::Constructor(_) | ClassMember::Empty(_) | ClassMember::AutoAccessor(_) => {}
          }
        }
      }
...
fn contains_free_identifier_read(parser: &mut JavascriptParser, expr: &Expr) -> bool {
  let mut collector = IdentifierReadCollector::new();
  expr.visit_with(&mut collector);
  collector.reads.into_iter().any(|ident| {
    parser
      .get_variable_info(&ident)
      .is_none_or(|info| info.is_free())
  })
...
fn is_module_eval_pure_expression<'a>(
  ...
  {
    if contains_free_identifier_read(parser, &assign.right) {
      // Reading a free/unresolved global anywhere in the RHS is observable
      // (ReferenceError, or a getter on the global object), so the export
