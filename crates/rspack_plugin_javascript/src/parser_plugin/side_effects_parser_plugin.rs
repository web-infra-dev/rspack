    ...
    match decl {
      ...
      ModuleDecl::ExportDecl(decl) => {
        if !is_pure_decl(
          ...
          let range = DependencyRange::from(decl.decl(&parser.ast).span(&parser.ast));
          let loc = parser.to_dependency_location(range);
          parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
            range,
            loc,
    ...
    match stmt {
      Statement::If(if_stmt) => {
        if !is_pure_expression(
          ...
          let range = DependencyRange::from(if_stmt.span(&parser.ast));
          let loc = parser.to_dependency_location(range);
          parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
            range,
            loc,
      ...
      Statement::While(while_stmt) => {
        if !is_pure_expression(
          ...
          let range = DependencyRange::from(while_stmt.span(&parser.ast));
          let loc = parser.to_dependency_location(range);
          parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
            range,
            loc,
      ...
      Statement::For(for_stmt) => {
        ...
        if !pure_init {
          let range = DependencyRange::from(for_stmt.span(&parser.ast));
          let loc = parser.to_dependency_location(range);
          parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
            range,
            loc,
      ...
      Statement::Expr(expr_stmt) => {
        if !is_pure_expression(
          ...
          let range = DependencyRange::from(expr_stmt.span(&parser.ast));
          let loc = parser.to_dependency_location(range);
          parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
            range,
            loc,
      ...
      Statement::Var(var_stmt) => match var_stmt {
        VariableDeclaration::VarDecl(var_decl) => {
          if !is_pure_var_decl(parser, var_decl, self.unresolved_scope_id, parser.comments) {
            let range = DependencyRange::from(var_stmt.span(&parser.ast));
            let loc = parser.to_dependency_location(range);
            parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
              range,
              loc,
      ...
      Statement::Fn(_) => {}
      _ => {
        let range = DependencyRange::from(stmt.span(&parser.ast));
        let loc = parser.to_dependency_location(range);
        parser.side_effects_item = Some(SideEffectsBailoutItemWithSpan::new(
          range,
          loc,
