...
pub fn enable_tracing_by_env(filter: &str) {
...
pub fn enable_tracing_by_env_with_chrome_layer(filter: &str, output: &Path) -> Option<FlushGuard> {
  if !IS_TRACING_ENABLED.swap(true, std::sync::atomic::Ordering::SeqCst) {
    ...
    let (chrome_layer, guard) = ChromeLayerBuilder::new()
      .include_args(true)
      .writer(Box::new(
        std::fs::File::create(output).expect("Failed to create trace file."),
      ))
      .build();
    let layers = generate_common_layers(filter);
