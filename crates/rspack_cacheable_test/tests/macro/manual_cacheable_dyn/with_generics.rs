...
const _: () = {
  ...
  where
    Archived<Dog>: Deserialize<Dog, Deserializer>,
  {
    fn deserialize_dyn(
      &self,
      deserializer: &mut Deserializer,
...
const _: () = {
  ...
  const fn get_vtable() -> VTablePtr {
    VTablePtr::new(ptr_meta::metadata(
      core::ptr::null::<Archived<Cat>>() as *const <dyn Animal<String> as ArchiveUnsized>::Archived
    ))
  }
  ...
  where
    Archived<Cat>: Deserialize<Cat, Deserializer>,
  {
    fn deserialize_dyn(
      &self,
      deserializer: &mut Deserializer,
