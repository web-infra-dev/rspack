...
use rspack_napi::spawn_local::SpawnLocalExt;
...
async fn run<R>(
  ...
  f: impl FnOnce(SharedReference<JsCompiler, &mut Compiler>) -> LocalBoxFuture<'static, Result<R>>,
) -> Result<R> {
  ...
  let shared_reference = reference.share_with(env, |this| Ok(&mut this.compiler))?;
...
fn node_init(mut _exports: Object, env: Env) -> Result<()> {
  rspack_core::bindings::set_thread_local_allocator(Box::new(allocator::NapiAllocatorImpl::new(
    env,
  )));
  Ok(())
}
