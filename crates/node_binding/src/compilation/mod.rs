...
impl JsCompilation {
  ...
  #[napi(
    ts_args_type = "args: [string, JsEntryDependency, JsEntryOptions | undefined][], callback: (errMsg: Error | null, results: [string | null, JsModule][]) => void"
  )]
  pub fn add_include(
    &mut self,
    env: Env,
    js_args: Vec<(
      String,
      ClassInstance<JsEntryDependency>,
      Option<JsEntryOptions>,
    )>,
    f: Function,
  ) -> napi::Result<()> {
    let compilation = self.as_mut()?;
    ...
    let args = js_args
      .into_iter()
      .map(|(js_context, mut js_dependency, js_options)| {
        let layer = match &js_options {
          Some(options) => options.layer.clone(),
          None => None,
        };
        let dependency =
          js_dependency.resolve(compilation.compiler_id(), js_context.into(), layer)?;
