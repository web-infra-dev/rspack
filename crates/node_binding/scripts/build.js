...
async function build() {
	return new Promise((resolve, reject) => {
		...
		const previousDts = existsSync(dts) ? readFileSync(dts, "utf8") : undefined;
		const args = [
			"build",
			"--platform",
			"--dts",
		...
		cp.on("exit", (code) => {
			if (code === CARGO_SAFELY_EXIT_CODE) {
				const generatedDts = existsSync(dts) ? readFileSync(dts, "utf8") : "";
				const patchedDts = generatedDts
				...
				if (patchedDts.trim().length > 0) {
					writeFileSync(dts, patchedDts);
				} else if (previousDts !== undefined) {
					console.warn(
						`napi generated an empty ${NAPI_BINDING_DTS}, restoring the checked-in declaration file`
					);
					writeFileSync(dts, previousDts);
				} else {
					reject(
						new Error(`napi generated an empty ${NAPI_BINDING_DTS} and no previous file was found`)
					);
					return;
				}
