...
use crate::{
  ...
  chunks_info_item::{compare_entries, ModuleGroup},
  Mb,
};
...
impl SplitChunksPlugin {
  ...
  fn ensure_max_size_fit(&self, compilation: &mut Compilation) {
    let max_size = Mb::from_mb(2.0);
  }
  ...
  fn remove_all_modules_from_other_entries_and_update_size(
    ...
    let invalid_group_keys = module_group_map
      .iter_mut()
      .par_bridge()
    ...
    invalid_group_keys.into_iter().for_each(|key| {
  ...
  /// Thus, if `original_chunks` want to know which chunk contains the moved module,
  ...
  fn find_best_module_group(&self, module_group_map: &mut ModuleGroupMap) -> (String, ModuleGroup) {
    ...
    let (key, mut best_module_group) = iter.next().unwrap();
    ...
    for (key, each_module_group) in iter {
      println!(
        ...
        best_entry_key,
        key,
        compare_entries(best_module_group, each_module_group)
      );
