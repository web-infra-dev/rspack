...
use crate::{with::AsCacheable, DeserializeError, Deserializer};
...
impl<K, V, WK, WV, T>
  DeserializeWith<ArchivedVec<RkyvEntry<WK::Archived, WV::Archived>>, T, Deserializer>
  for AsMap<WK, WV>
where
  T: AsMapConverter<Key = K, Value = V>,
  K: std::hash::Hash + Eq,
  WK: ArchiveWith<K> + DeserializeWith<WK::Archived, K, Deserializer>,
  WV: ArchiveWith<V> + DeserializeWith<WV::Archived, V, Deserializer>,
{
  fn deserialize_with(
