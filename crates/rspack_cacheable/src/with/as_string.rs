...
impl<'a, T> SerializeWith<T, Serializer<'a>> for AsString
where
  T: AsStringConverter,
{
  #[inline]
...
impl<T> DeserializeWith<ArchivedString, T, Deserializer> for AsString
where
  T: AsStringConverter,
{
  #[inline]
  fn deserialize_with(field: &ArchivedString, _: &mut Deserializer) -> Result<T, DeserializeError> {
