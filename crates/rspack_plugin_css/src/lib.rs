...
impl SwcCssCompiler {
  ...
  pub fn parse_file(
    ...
    let diagnostics = parser
      .take_errors()
      .into_iter()
      .flat_map(|error| css_parse_error_to_diagnostic(error, path, fm.src.to_string()))
      .collect();
    stylesheet
...
pub fn css_parse_error_to_diagnostic(error: Error, path: &str, source: String) -> Vec<Diagnostic> {
