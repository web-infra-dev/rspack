...
impl FsWatcher {
  ...
  fn wait_for_event(
    ...
    eprintln!("[WATCHER_DEBUG] FsWatcher::wait_for_event() - Step 1: Updating path manager");
    self.path_manager.update(files, directories, missing)?;

    eprintln!("[WATCHER_DEBUG] FsWatcher::wait_for_event() - Step 2: Starting scanner (async)");
    self.scanner.scan(start_time);

    eprintln!("[WATCHER_DEBUG] FsWatcher::wait_for_event() - Step 3: Analyzing watch patterns");
    ...
    eprintln!("[WATCHER_DEBUG] FsWatcher::wait_for_event() - Step 4: Setting up disk watcher");
    self.disk_watcher.watch(watch_patterns.into_iter())?;

    eprintln!("[WATCHER_DEBUG] FsWatcher::wait_for_event() - Complete");
    Ok(())
  }
}
