...
async fn optimize_chunks(&self, compilation: &mut Compilation) -> Result<Option<bool>> {
  ...
  for module in all_modules_to_hoist {
    ...
    for chunk in non_runtime_chunks {
      ...
      if compilation.build_chunk_graph_artifact.chunk_graph.get_number_of_chunk_modules(&chunk) == 0
        && compilation.build_chunk_graph_artifact.chunk_graph.get_number_of_entry_modules(&chunk) == 0
        && let Some(mut removed_chunk) = compilation.build_chunk_graph_artifact.chunk_by_ukey.remove(&chunk)
      {
        compilation
          .build_chunk_graph_artifact.chunk_graph
          .disconnect_chunk(&mut removed_chunk, &mut compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
        compilation.build_chunk_graph_artifact.chunk_graph.remove_chunk(&chunk);
