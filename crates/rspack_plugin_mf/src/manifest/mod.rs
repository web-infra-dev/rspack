...
fn get_remote_entry_name(compilation: &Compilation, container_name: &str) -> Option<String> {
  let chunk_group_ukey = compilation.build_chunk_graph_artifact.entrypoints.get(container_name)?;
  let chunk_group = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(chunk_group_ukey);
  ...
  let runtime_chunk_file = {
    let runtime_chunk_key = chunk_group.get_runtime_chunk(&compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
  ...
  let (exposes, shared, remote_list) = if self.options.disable_assets_analyze {
        ...
        for (index, (expose_key, options)) in container_entry.exposes().iter().enumerate() {
          ...
          if let Some(block_id) = blocks.get(index)
            && let Some(chunk_group) = compilation
              ...
              .get_block_chunk_group(block_id, &compilation.build_chunk_graph_artifact.chunk_group_by_ukey)
            && let Some(chunk_key) = chunk_group.chunks.first()
            && let Some(chunk) = compilation.build_chunk_graph_artifact.chunk_by_ukey.get(chunk_key)
    ...
    for (expose_file_key, expose) in exposes_map.iter_mut() {
      ...
      if let Some(chunk_name) = expose_chunk_names.get(expose_file_key)
        && let Some(chunk_key) = compilation.build_chunk_graph_artifact.named_chunks.get(chunk_name)
      {
      ...
      if assets.is_none()
        && let Some(chunk_key) = compilation.build_chunk_graph_artifact.named_chunks.get(expose_file_key)
      {
        assets = Some(collect_assets_from_chunk(
          compilation,
          chunk_key,
