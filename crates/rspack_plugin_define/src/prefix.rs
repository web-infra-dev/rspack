...
impl<'a> DefinePrefix<'a> {
  ...
  fn insert_cant_rename(&mut self, name: &str) {
    self.defintions.contains_key(name).then(|| {
      self.can_not_rename.insert(name.to_string());
    });
  }
  ...
  pub(super) fn insert_cant_rename_from_pat(&mut self, pat: &ast::Pat) {
    match pat {
      ast::Pat::Ident(ident) => {
        self.insert_cant_rename(ident.id.sym.as_ref());
      }
      ...
      ast::Pat::Object(obj) => {
        obj.props.iter().for_each(|prop| match prop {
          ...
          ast::ObjectPatProp::Assign(assign) => self.insert_cant_rename(assign.key.sym.as_ref()),
    ...
    import_decl
      ...
      .for_each(|specifier| match specifier {
        ast::ImportSpecifier::Default(s) => {
          self.insert_cant_rename(s.local.sym.as_ref());
        }
        ast::ImportSpecifier::Named(named) => {
          self.insert_cant_rename(named.local.sym.as_ref());
        }
        ast::ImportSpecifier::Namespace(namespace) => {
          self.insert_cant_rename(namespace.local.sym.as_ref());
        }
      });
  }
