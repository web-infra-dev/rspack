...
use rspack_plugin_javascript::{
  dependency::is_import_meta_resolve_entry_chunk, impl_plugin_for_js_plugin::chunk_has_js,
};
...
static RUNTIME_MODULE_VARIABLES: LazyLock<Vec<&'static str>> = LazyLock::new(|| {
  ...
  variables.push("__rspack_hmr_s_jsonp");
  variables
});
...
impl JsonpChunkLoadingRuntimeModule {
  fn generate_base_uri(
    ...
    let base_uri = chunk
      ...
      .map_or_else(
        || {
          Cow::Borrowed(if is_import_meta_resolve_entry_chunk(compilation, &chunk.ukey()) {
