...
impl RuntimeModule for GetChunkFilenameRuntimeModule {
  ...
  async fn generate(&self, compilation: &Compilation) -> rspack_error::Result<String> {
    let chunks = self
      .chunk
      .and_then(|chunk_ukey| compilation.build_chunk_graph_artifact.chunk_by_ukey.get(&chunk_ukey))
      .map(|chunk| {
        ...
        if (self.all_chunks)(runtime_requirements) {
          ...
          let mut chunks = chunk.get_all_async_chunks(&compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
          ...
          for entrypoint in
            chunk.get_all_referenced_async_entrypoints(&compilation.build_chunk_graph_artifact.chunk_group_by_ukey)
          {
            let entrypoint = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(&entrypoint);
    ...
    if let Some(chunks) = chunks {
      chunks
        .iter()
        .filter_map(|chunk_ukey| compilation.build_chunk_graph_artifact.chunk_by_ukey.get(chunk_ukey))
        .for_each(|chunk| {
