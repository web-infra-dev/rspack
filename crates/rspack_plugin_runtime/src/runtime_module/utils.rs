...
pub fn get_initial_chunk_ids(
  ...
  match chunk {
    Some(chunk_ukey) => match compilation.build_chunk_graph_artifact.chunk_by_ukey.get(&chunk_ukey) {
      Some(chunk) => {
...
pub fn is_enabled_for_chunk(
  ...
  let chunk_loading = compilation
    ...
    .get(chunk_ukey)
    .and_then(|chunk| chunk.get_entry_options(&compilation.build_chunk_graph_artifact.chunk_group_by_ukey))
