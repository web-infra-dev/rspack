...
impl Plugin for ImportScriptsChunkLoadingPlugin {
  ...
  fn runtime_requirements_in_tree(
    ...
    let chunk_loading_value = ChunkLoading::ImportScripts;
    ...
    for runtime_requirement in runtime_requirements.iter() {
      match runtime_requirement {
        RuntimeGlobals::ENSURE_CHUNK_HANDLERS
          if is_enabled_for_chunk(chunk, &chunk_loading_value, compilation) =>
        {
      ...
      if has_chunk_loading && is_enabled_for_chunk(chunk, &chunk_loading_value, compilation) {
        runtime_requirements.insert(RuntimeGlobals::MODULE_FACTORIES_ADD_ONLY);
