...
impl LoaderChain {
  ...
  #[inline]
  fn children(&self) -> Option<&Range<u16>> {
    match &self.kind {
  ...
  #[inline]
  fn set_children(&mut self, range: Range<u16>) {
...
fn plan_loader_chains<Context: Send>(loaders: &[LoaderItem<Context>]) -> LoaderChains {
  ...
  while index < loaders.len() {
    ...
    execution_chain_by_loader[index..end].fill(root_chain_index as u16);
    index = end;
  }
  ...
  for root_chain_index in 0..root_chain_count {
    ...
    chains[root_chain_index].set_children(children_start as u16..children_end as u16);
  }
...
fn validate_loader_chain_plan<Context: Send>(
  ...
  for (root_chain_index, chain) in chains[..root_chain_count].iter().enumerate() {
    ...
    for loader_index in range {
      ...
      let chain_index = execution_chain_by_loader[loader_index];
      assert_ne!(chain_index, u16::MAX);
      let execution_chain = &chains[usize::from(chain_index)];
