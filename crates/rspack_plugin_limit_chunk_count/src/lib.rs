...
async fn optimize_chunks(&self, compilation: &mut Compilation) -> Result<Option<bool>> {
  ...
  let chunk_group_by_ukey = &compilation.build_chunk_graph_artifact.chunk_group_by_ukey.clone();
  let chunk_graph = &compilation.build_chunk_graph_artifact.chunk_graph.clone();
  let mut new_chunk_by_ukey = std::mem::take(&mut compilation.build_chunk_graph_artifact.chunk_by_ukey);
  let mut new_chunk_group_by_ukey = std::mem::take(&mut compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
