// webpack: "tsh mode" in FileSystemInfo.unittest.js (L318-L449).
use super::*;

const OPTIONS: SnapshotStrategyOptions = SnapshotStrategyOptions::hash_and_timestamp();
...
#[tokio::test]
async fn should_not_invalidate_snapshot_when_only_timestamps_have_changed() {
  check_changed_timestamps(OPTIONS).await;
}

// These need the ignore state too: webpack stores a hash-only file entry,
// whereas an ignored file inside a context can take the timestamp fast path.
#[test]
#[should_panic(expected = "not yet implemented: webpack L378:")]
fn should_invalidate_when_nested_deep_ignored_file_changes() {
  todo!(
    "webpack L378: ignored timestamp must produce a hash-only entry for /path/nested/deep/ignored.txt; TimestampAndHash cannot represent it yet"
  );
}
#[test]
#[should_panic(expected = "not yet implemented: webpack L378:")]
fn should_invalidate_when_context_files_sub_ignored_file_changes() {
  todo!(
    "webpack L378: ignored timestamp must produce a hash-only entry for /path/context+files/sub/ignored.txt; requires watcher ignore state"
  );
}
#[test]
...
fn should_not_invalidate_when_context_sub_ignored_file_changes() {
  todo!(
    "webpack L397: unchanged context timestamp excluding /path/context/sub/ignored.txt must avoid hash fallback; requires watcher ignore state"
  );
}
