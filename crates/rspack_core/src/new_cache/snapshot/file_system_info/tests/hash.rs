// webpack: "hash mode" in FileSystemInfo.unittest.js (L318-L449).
use super::*;

const OPTIONS: SnapshotStrategyOptions = SnapshotStrategyOptions::hash();
...
#[tokio::test]
async fn should_invalidate_when_nested_deep_ignored_file_changes() {
  check_change(OPTIONS, "/path/nested/deep/ignored.txt", false, false).await;
}

#[tokio::test]
async fn should_invalidate_when_context_files_sub_ignored_file_changes() {
  check_change(OPTIONS, "/path/context+files/sub/ignored.txt", false, false).await;
}

#[tokio::test]
async fn should_invalidate_when_context_sub_ignored_file_changes() {
  check_change(OPTIONS, "/path/context/sub/ignored.txt", false, false).await;
}
