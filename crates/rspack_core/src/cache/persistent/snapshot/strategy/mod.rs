...
impl StrategyHelper {
  ...
  async fn modified_time(&self, path: &UstrPath) -> Option<u64> {
    if let Ok(info) = self.fs.metadata(path.as_utf8_path()).await {
  ...
  pub async fn package_version(&self, path: &UstrPath) -> Option<Strategy> {
    self
      .package_helper
  ...
  pub async fn file_hash(&self, path: &UstrPath) -> Strategy {
  ...
  pub async fn dir_hash(&self, path: &UstrPath) -> Strategy {
  ...
  pub async fn validate(&self, path: &UstrPath, strategy: &Strategy) -> ValidateResult {
    match strategy {
...
mod tests {
  ...
  async fn validate_package_version() {
    ...
    assert!(matches!(
      helper
        .validate(&UstrPath::from("/packages/lib/file.js"), &strategy)
        .await,
      ValidateResult::NoChanged
    ));
  ...
  async fn validate_file_hash() {
    ...
    let helper = StrategyHelper::new(fs.clone(), Default::default());
    let strategy = helper.file_hash(&UstrPath::from("/file1.js")).await;
    assert!(matches!(
      helper
        .validate(&UstrPath::from("/file1.js"), &strategy)
        .await,
      ValidateResult::NoChanged
    ));
