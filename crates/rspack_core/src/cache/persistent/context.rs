...
pub(crate) struct LoadedSnapshotPaths {
  pub(crate) file: ArcPathSet,
  pub(crate) context: ArcPathSet,
  pub(crate) missing: ArcPathSet,
}

impl LoadedSnapshotPaths {
  fn get_mut(&mut self, scope: SnapshotScope) -> &mut ArcPathSet {
    match scope {
...
impl CacheContext {
  ...
  pub(crate) async fn load_snapshot(
    &mut self,
    snapshot: &Snapshot,
  ) -> Option<(bool, ArcPathSet, ArcPathSet, LoadedSnapshotPaths)> {
    if !self.load_failed {
