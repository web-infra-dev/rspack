...
use crate::{
  make::MakeArtifact, DependencyParents, ExportInfoData, ExportsInfoData, DEPENDENCY_ID,
};
...
const MAKE_SCOPE: &str = "MAKE";
const META_SCOPE: &str = "MAKE_META";
...
impl MakeOccasion {
  ...
  pub fn recovery(&self) -> Result<MakeArtifact, DeserializeError> {
    ...
    for (k, v) in self.storage.get_all(META_SCOPE) {
      if String::from_utf8(k).expect("should have utf8 key") == "default" {
