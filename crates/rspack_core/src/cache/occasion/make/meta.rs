use rspack_cacheable::{
  cacheable, CacheableSerializer, SerializeError,
...
impl<'a> Archive for MetaRef<'a> {
  ...
  fn resolve(&self, resolver: Self::Resolver, out: Place<Self::Archived>) {
    ...
    let field_ptr = unsafe { &raw mut (*out.ptr()).next_connection_id };
    let field_out = unsafe { Place::from_field_unchecked(out, field_ptr) };
    Archive::resolve(
...
impl<'a> Serialize<CacheableSerializer<'_>> for MetaRef<'a> {
  #[inline]
  fn serialize(
    &self,
    serializer: &mut CacheableSerializer<'_>,
