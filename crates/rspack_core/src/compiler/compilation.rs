...
use super::{
  ...
  AssetFilename, AssetInfo, CompilationAsset, CompilationAssets,
};
use crate::{
...
pub struct Compilation {
  ...
  pub module_assets: IdentifierMap<HashSet<AssetFilename>>,
  pub emitted_assets: DashSet<AssetFilename, BuildHasherDefault<FxHasher>>,
...
impl Compilation {
  ...
  pub fn emit_asset(&mut self, filename: AssetFilename, asset: CompilationAsset) {
  ...
  pub fn rename_asset(&mut self, filename: &str, new_name: AssetFilename) {
    if let Some(asset) = self.assets.remove(filename) {
  ...
  async fn create_chunk_assets(&mut self, plugin_driver: SharedPluginDriver) -> Result<()> {
    ...
    for result in chunk_ukey_and_manifest {
      ...
      for file_manifest in manifest {
        let filename = AssetFilename::from(file_manifest.filename);
