...
define_hook!(CompilationFinishModules: AsyncSeries(compilation: &mut Root<Compilation>));
...
define_hook!(CompilationProcessAssets: AsyncSeries(compilation: &mut Root<Compilation>));
define_hook!(CompilationAfterProcessAssets: AsyncSeries(compilation: &mut Root<Compilation>));
...
impl Compilation {
  ...
  async fn process_assets(self: &mut Root<Self>, plugin_driver: SharedPluginDriver) -> Result<()> {
    plugin_driver
      .compilation_hooks
      .process_assets
      .call(self)
  ...
  pub async fn finish(self: &mut Root<Self>, plugin_driver: SharedPluginDriver) -> Result<()> {
    let logger = self.get_logger("rspack.Compilation");

    {
      let compilation = &mut **self;

      // clean up the entry deps
      let make_artifact = std::mem::take(&mut compilation.make_artifact);
      compilation.make_artifact = update_module_graph(
        compilation,
        make_artifact,
      ...
      if let Some(mutations) = compilation.incremental.mutations_write() {
        mutations.extend(
          compilation
            .make_artifact
            .revoked_modules
            .difference(&compilation.make_artifact.built_modules)
        ...
        mutations.extend(
          compilation
            .make_artifact
            .built_modules
            .intersection(&compilation.make_artifact.revoked_modules)
        ...
        mutations.extend(
          compilation
            .make_artifact
            .built_modules
            .difference(&compilation.make_artifact.revoked_modules)
  ...
  pub async fn seal(self: &mut Root<Self>, plugin_driver: SharedPluginDriver) -> Result<()> {
...

    {
      ...
      compilation.assign_runtime_ids();
        ...
        for mutation in mutations.iter() {
          match mutation {
            ...
            Mutation::ChunkAdd { chunk } => {
              affected_modules.extend(compilation.chunk_graph.get_chunk_modules_identifier(chunk));
            }
      ...
      compilation.create_module_hashes(create_module_hashes_modules)?;
    }
...

    {
      ...
      compilation.code_generation(code_generation_modules)?;
...

    {
      ...
      compilation
        ...
        .await?;
      compilation.runtime_modules_code_generation()?;
