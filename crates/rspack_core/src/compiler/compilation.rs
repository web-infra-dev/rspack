...
impl Compilation {
  ...
  pub async fn seal(&mut self, plugin_driver: SharedPluginDriver) -> Result<()> {
    ...
    {
      let mut chunks_ids = vec![];
      for chunk in self.chunk_by_ukey.values() {
        chunks_ids.push(chunk.id.clone().unwrap_or_default());
      }
      chunks_ids.sort();
      std::fs::write("chunk_ids.txt", chunks_ids.join("\n"));
    }
