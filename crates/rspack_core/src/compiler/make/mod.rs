...
use crate::{
  ...
  AsyncDependenciesBlock, BoxDependency, BuildDependency, BuildQueue, BuildTask, BuildTaskResult,
  BuildTimeExecutionQueue, BuildTimeExecutionTask, CacheOptions, CleanQueue, CleanTask,
  CleanTaskResult, Compilation, Context, ContextDependency, DependencyId, DependencyParents,
  DependencyType, FactorizeQueue, FactorizeTask, FactorizeTaskResult, Module,
  ModuleCreationCallback, ModuleFactoryResult, ModuleGraph, ModuleGraphModule, ModuleIdentifier,
  ModuleProfile, NormalModuleSource, ProcessDependenciesQueue, ProcessDependenciesResult,
  ProcessDependenciesTask, Resolve, TaskResult, WorkerTask,
};
...
pub async fn update_module_graph(
  ...
  let origin_module_deps: HashMap<_, _> = deps_builder
    ...
    .map(|module_identifier| {
      (
        *module_identifier,
        module_deps(compilation, module_identifier),
      )
    })
    .collect();
  ...
  compilation.factorize_queue = Some(factorize_queue.queue_handler());
  compilation.build_queue = Some(build_queue.queue_handler());
  compilation.add_queue = Some(add_queue.queue_handler());
  compilation.process_dependencies_queue = Some(process_dependencies_queue.queue_handler());
  compilation.build_time_execution_queue = Some(buildtime_execution_queue.queue_handler());

  deps_builder
    .revoke_modules(&mut compilation.module_graph)
    .into_iter()
    .for_each(|(id, parent_module_identifier)| {
      ...
      let parent_module =
        parent_module_identifier.and_then(|id| compilation.module_graph.module_by_identifier(&id));
  ...
  tokio::task::block_in_place(|| loop {
    ...
    while let Some(task) = factorize_queue.get_task(compilation) {
      active_task_count += 1;
    ...
    while let Some(task) = build_queue.get_task(compilation) {
      active_task_count += 1;
      tokio::spawn({
        let result_tx = result_tx.clone();
        let is_expected_shutdown = is_expected_shutdown.clone();
    ...
    while let Some(task) = add_queue.get_task(compilation) {
      active_task_count += 1;
    ...
    while let Some(task) = process_dependencies_queue.get_task(compilation) {
      active_task_count += 1;
    ...
    while let Some(task) = buildtime_execution_queue.get_task(compilation) {
      ...
      Ok(item) => {
        if let Ok(item) = &item {
          match item {
            TaskResult::Factorize(result) => {
              ...
              {
                factorize_queue.complete_task(result.dependency, module.identifier(), compilation)
              }
            }
            TaskResult::Add(result) => {
              ...
              add_queue.complete_task(module, module, compilation)
            }
        ...
        match item {
          ...
          Ok(TaskResult::Build(box task_result)) => {
            ...
            module.set_module_build_info_and_meta(build_result.build_info, build_result.build_meta);
  ...
  compilation.make_failed_module.extend(make_failed_module);
  ...
  compilation.has_module_import_export_change = if origin_module_deps.is_empty() {
    true
  } else {
    compilation.has_module_import_export_change
      || !origin_module_deps.into_iter().all(|(module_id, deps)| {
          ...
          .is_none()
        {
          ...
          let (now_deps, mut now_blocks) = module_deps(compilation, &module_id);
