...
impl ConcatenationScope {
  ...
  */
  ...
  pub fn register_re_export(&mut self, module: ModuleIdentifier, mode: ExportMode) {
    self.re_exports.entry(module).or_default().push(mode);
  }

  pub fn register_star_export(&mut self, source: String, module: Option<ModuleIdentifier>) {
    let star_exports = self.star_exports.entry(source).or_default();
