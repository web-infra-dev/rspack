...
pub trait Task: Debug + Send + Any + AsAny {
  ...
  async fn main_run(self: Box<Self>, _context: &mut MakeTaskContext) -> TaskResult {
    unreachable!();
  }
...
pub async fn run_task_loop_with_event<'a>(
  ...
  let mut queue: VecDeque<Box<dyn Task>> = VecDeque::from(init_tasks);
