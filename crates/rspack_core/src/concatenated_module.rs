...
pub(crate) const MODULE_REFERENCE_PLACEHOLDER_PREFIX: &str =
  "__rspack_module_reference_placeholder_";
pub(crate) const MODULE_REFERENCE_PREFIX: &str = "__rspack_module_ref";
pub(crate) const MODULE_REFERENCE_SUFFIX: &str = "__._";
...
pub struct ConcatenatedModuleInfo {
  ...
  pub rendered_init_fragments: Option<RenderedInitFragments>,
  pub generated_top_level_symbols: Vec<GeneratedTopLevelSymbol>,
  pub module_references: FxIndexMap<String, ConcatenatedModuleReference>,
  ...
  async fn code_generation(
    ...
    let (escaped_name_entries, escaped_identifier_entries) = module_to_info_map
      ...
      .map(|info| {
        ...
        let identifier_capacity = if faster_module_concatenation {
          0
        } else {
          identifier_capacity
        };
    ...
    for info in module_to_info_map.values_mut() {
      ...
      let module_name_parts: &[Atom] = readable_identifier.as_ref().map_or(&[], |identifier| {
        escaped_identifiers
          .get(identifier)
      ...
      match info {
        ...
        ModuleInfo::Concatenated(info) => {
          ...
          for (id, refs) in info.binding_to_ref.iter() {
            ...
            info
              .internal_names
              .insert(binding_name.clone(), final_name.clone());
            if binding_name != name && name == &*DEFAULT_EXPORT_ATOM {
              info.internal_names.insert(name.clone(), final_name.clone());
            }
            top_level_declarations.insert(final_name.clone());

            if final_name != *binding_name && !refs.is_empty() {
              // Update source
              ...
              for identifier in refs {
                ...
                if identifier.shorthand {
                  source.insert(high, format!(": {final_name}"), None);
                  continue;
                }

                source.replace(low, high, final_name.to_string(), None);
              }
            }
          }
          ...
          if let Some(import_map) = info.import_map.take() {
            for ((source, attr), imported) in import_map {
              let source_parts: &[Atom] = if faster_module_concatenation {
                &[]
              } else {
                escaped_identifiers
                  .get(&source)
          ...
          let namespace_object_name =
            if let Some(ref namespace_export_symbol) = info.namespace_export_symbol {
              ...
              Some(name_allocator.find_new_name("namespaceObject", module_name_parts))
            };
        ...
        ModuleInfo::External(info) => {
          let external_name: Atom = name_allocator.find_new_name("", module_name_parts);
          info.name = Some(external_name.clone());
          top_level_declarations.insert(external_name.clone());

          if info.deferred {
            let external_name = name_allocator.find_new_name("deferred", module_name_parts);
            info.deferred_name = Some(external_name.clone());
            top_level_declarations.insert(external_name.clone());

            let external_name_interop =
              name_allocator.find_new_name("deferredNamespaceObject", module_name_parts);
      ...
      if exports_type != BuildMetaExportsType::Namespace {
        let external_name_interop: Atom =
          name_allocator.find_new_name("namespaceObject", module_name_parts);
      ...
      {
        let external_name_interop: Atom =
          name_allocator.find_new_name("namespaceObject2", module_name_parts);
      ...
      if matches!(
        ...
        let external_name_interop: Atom =
          name_allocator.find_new_name("default", module_name_parts);
    ...
    let mut changes = module_to_info_map
      ...
      .filter_map(|info| {
        ...
        let mut refs = vec![];
        if info.module_references.is_empty() {
          for reference in info.global_scope_ident.iter() {
            let match_result =
              ConcatenationScope::match_module_reference(reference.id.sym.as_str());
          ...
          for (placeholder, reference) in &info.module_references {
            let referenced_info_id = reference.module;
            refs.push((
              ModuleReferenceReplacementTarget::Placeholder(placeholder.clone()),
              referenced_info_id,
              reference.options.ids.clone(),
    ...
    let mut module_reference_replacements: IdentifierMap<Vec<(String, String)>> =
