  ...
  fn create(self, splitter: &CodeSplitter, compilation: &Compilation) -> Vec<ChunkDesc> {
    ...
    match self {
      ...
      CreateChunkRoot::Block(origin, block_id, runtime) => {
        ...
        let mut chunks = vec![];
        let mut ctx = FillCtx::default();
        ctx.chunk_loading = if let Some(entry_option) = block
          .get_group_options()
...
impl CodeSplitter {
      ...
      for block_id in blocks {
        ...
        let child_runtime = if should_create {
          if let Some(root) = roots.get_mut(&block_id) {
            ...
            roots.insert(
              block_id,
              CreateChunkRoot::Block(*block.parent(), block_id, Some(rt.clone().into_owned())),
            );
            rt.clone()
          }
        } else {
  ...
  fn create_chunks(&mut self, compilation: &mut Compilation) -> Result<()> {
    let entries: &indexmap::IndexMap<String, EntryData> = &compilation.entries;
...
fn remove_available_modules(
  ...
  for parent in parents {
    let parent_available_modules =
      if let AvailableModules::Set(finished) = &available_modules[parent] {
        finished.clone()
      } else {
        remove_available_modules(parent, chunks, chunk_parents, available_modules);
        match &available_modules[parent] {
          AvailableModules::Set(finished) => finished.clone(),
          AvailableModules::Calculating => {
            // have cycle, just skip this branch
            continue;
          }
  ...
  if let Some(parents_available) = &parents_available {
    ...
    available_modules[curr] = AvailableModules::Set(parents_available | &result);
    *chunk.available_modules_mut() = result;
  } else {
    available_modules[curr] = AvailableModules::Set(chunk.available_modules().clone());
  }
}
      ...
      match task {
        ...
        Task::Enter((m, runtime)) => {
          ...
          module
            ...
            .filter(|conn| conn.is_active(&module_graph, Some(runtime)))
