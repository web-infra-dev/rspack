...
pub async fn runtime_requirements_pass(
    ...
    && !compilation.cgc_runtime_requirements_artifact.is_empty()
  {
    ...
    compilation
      .cgc_runtime_requirements_artifact
      .retain(|chunk, _| compilation.build_chunk_graph_artifact.chunk_by_ukey.contains(chunk));
    ...
    compilation.build_chunk_graph_artifact.chunk_by_ukey.keys().copied().collect()
  };
  compilation
  ...
  pub async fn process_chunks_runtime_requirements(
    ...
    let chunk_requirements = chunks
      ...
      .map(|chunk_ukey| {
        ...
        for mid in self.build_chunk_graph_artifact.chunk_graph.get_chunk_modules_identifier(chunk_ukey) {
    ...
    for &entry_ukey in &entries {
      let entry = self.build_chunk_graph_artifact.chunk_by_ukey.expect_get(&entry_ukey);
      let mut set = RuntimeGlobals::default();
