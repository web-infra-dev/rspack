...
impl CodeSplitter {
  ...
  pub(crate) fn invalidate_chunk_group(
    ...
    if let Some(name) = &chunk_group_name {
      ...
      compilation.build_chunk_graph_artifact.entrypoints.swap_remove(name);
    ...
    for parent in chunk_group.parents.iter() {
      let Some(parent_cg) = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.get_mut(parent) else {
    ...
    for chunk_ukey in chunk_group.chunks.iter() {
      let Some(chunk) = compilation.build_chunk_graph_artifact.chunk_by_ukey.get_mut(chunk_ukey) else {
        continue;
      };
      ...
      if chunk.remove_group(&chunk_group_ukey) && chunk.groups().is_empty() {
        ...
        if let Some(name) = chunk.name() {
          compilation.build_chunk_graph_artifact.named_chunks.remove(name);
        }
        compilation.build_chunk_graph_artifact.chunk_by_ukey.remove(chunk_ukey);
    ...
    // remove chunk group
    compilation.build_chunk_graph_artifact.chunk_group_by_ukey.remove(&chunk_group_ukey);
  ...
  pub(crate) fn remove_orphan(&mut self, compilation: &mut Compilation) -> Result<()> {
    let mut removed = vec![];
    for chunk_group in compilation.build_chunk_graph_artifact.chunk_group_by_ukey.values() {
  ...
  pub fn recover_from_cache(&mut self, cgi_ukey: CgiUkey, compilation: &mut Compilation) -> bool {
    ...
    let cg = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(&cgi.chunk_group);
    ...
    let group = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get_mut(&group_ukey);
  ...
  pub fn update_with_compilation(&mut self, compilation: &mut Compilation) -> Result<()> {
    ...
    for chunk in compilation.build_chunk_graph_artifact.chunk_by_ukey.keys() {
      ...
      for module_id in compilation.build_chunk_graph_artifact.chunk_graph.get_chunk_modules_identifier(chunk) {
    ...
    for (name, chunk_group) in &compilation.build_chunk_graph_artifact.entrypoints {
    ...
    compilation
      .build_chunk_graph_artifact.async_entrypoints
      .retain(|cg_ukey| compilation.build_chunk_graph_artifact.chunk_group_by_ukey.contains(cg_ukey));
    ...
    if !removed_entries.is_empty() {
      ...
      'outer: for (entry, data) in &compilation.entries {
        if !compilation.build_chunk_graph_artifact.entrypoints.contains_key(entry) {
          // new entry
    ...
    compilation.build_chunk_graph_artifact.entrypoints.sort_unstable_by(|a, _, b, _| {
      let a = compilation
        .entries
        .get_index_of(a)
  ...
  pub fn update_cache(&mut self, compilation: &Compilation) {
    ...
    for cgi in self.chunk_group_infos.values() {
      ...
      for block_id in blocks {
        ...
        let group = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(&cgi.chunk_group);
