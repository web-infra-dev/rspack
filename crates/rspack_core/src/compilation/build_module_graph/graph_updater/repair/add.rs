...
impl Task<TaskContext> for AddTask {
  ...
  async fn main_run(self: Box<Self>, context: &mut TaskContext) -> TaskResult<TaskContext> {
...

    {
      ...
      let module_graph =
        &mut TaskContext::get_module_graph_mut(&mut artifact.module_graph_partial);
        ...
        .is_some()
      {
        ...
        if context.compiler_options.experiments.lazy_barrel {
          if self.from_unlazy {
            artifact.affected_modules.mark_as_add(&module_identifier);
          }
            ...
            .is_some()
          {
            ...
            let pending_forwarded_ids = artifact
              .module_to_lazy_make
    ...
    Ok(vec![Box::new(BuildTask {
      ...
      runtime_template: context.runtime_template.clone(),
      fs: context.fs.clone(),
      forwarded_ids,
    })])
  }
