...
impl Task<TaskContext> for AddTask {
  ...
  async fn main_run(self: Box<Self>, context: &mut TaskContext) -> TaskResult<TaskContext> {
    ...
    let (mut module, reused) = if let Some(module_build_cache) = &context.module_build_cache {
      module_build_cache
        .restore(
          module,
    ...
    if reused {
      if let Some(cache) = &context.module_build_cache {
        cache.track(&mut module, false);
      }
