...
use crate::{
  ChunkContentHash, ChunkHashResult, ChunkUkey, Compilation, Logger, SharedPluginDriver,
  SourceType,
...
pub async fn create_hash(
  ...
  let other_chunk_runtime_module_hashes = rspack_futures::scope::<_, Result<_>>(|token| {
    other_chunks
      ...
      .for_each(|runtime_module_identifier| {
        let s = unsafe { token.used((&compilation, runtime_module_identifier)) };
  ...
  let other_chunks_hash_results = rspack_futures::scope::<_, Result<_>>(|token| {
    for chunk in other_chunks {
      let s = unsafe { token.used((&compilation, chunk, &plugin_driver)) };
  ...
  for runtime_chunk_ukey in runtime_chunks_map.keys().copied().collect::<Vec<_>>() {
    ...
    let groups =
      runtime_chunk.get_all_referenced_async_entrypoints(&compilation.chunk_group_by_ukey);
    for other in groups
      .into_iter()
  ...
  if remaining > 0 {
    let mut circular: Vec<_> = runtime_chunks_map
      ...
      .map(|(chunk_ukey, _)| compilation.chunk_by_ukey.expect_get(chunk_ukey))
      .collect();
  ...
  for runtime_chunk_ukey in runtime_chunks {
    ...
    let chunk = compilation.chunk_by_ukey.expect_get(&runtime_chunk_ukey);
    let chunk_hashes_changed = chunk.set_hashes(
      &mut compilation.chunk_hashes_artifact,
      chunk_hash_result.hash,
      chunk_hash_result.content_hash,
  ...
  for chunk_ukey in full_hash_chunks {
    ...
    let new_chunk_hash = {
      ...
      compilation.hash.hash(&mut hasher);
      hasher.digest(&compilation.options.output.hash_digest)
    };
    let new_content_hash: ChunkContentHash = {
  ...
  let results = rspack_futures::scope::<_, Result<_>>(|token| {
    compilation
      ...
      .for_each(|(runtime_module_identifier, runtime_module)| {
        let s = unsafe { token.used((&compilation, runtime_module_identifier, runtime_module)) };
        s.spawn(
