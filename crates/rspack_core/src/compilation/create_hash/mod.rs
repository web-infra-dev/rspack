...
impl Compilation {
  ...
  pub async fn create_hash(&mut self, plugin_driver: SharedPluginDriver) -> Result<()> {
    ...
    let other_chunk_runtime_module_hashes = rspack_futures::scope::<_, Result<_>>(|token| {
      other_chunks
        .iter()
        .flat_map(|chunk| self.build_chunk_graph_artifact.chunk_graph.get_chunk_runtime_modules_iterable(chunk))
    ...
    for runtime_chunk_ukey in runtime_chunks_map.keys().copied().collect::<Vec<_>>() {
      let runtime_chunk = self.build_chunk_graph_artifact.chunk_by_ukey.expect_get(&runtime_chunk_ukey);
      let groups = runtime_chunk.get_all_referenced_async_entrypoints(&self.build_chunk_graph_artifact.chunk_group_by_ukey);
      for other in groups
        .into_iter()
        .map(|group| self.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(&group))
    ...
    while i < runtime_chunks.len() {
      ...
      for other in referenced_by {
        if has_full_hash_modules {
          for runtime_module in self.build_chunk_graph_artifact.chunk_graph.get_chunk_runtime_modules_iterable(&other) {
    ...
    if remaining > 0 {
      let mut circular: Vec<_> = runtime_chunks_map
        ...
        .map(|(chunk_ukey, _)| self.build_chunk_graph_artifact.chunk_by_ukey.expect_get(chunk_ukey))
        .collect();
    ...
    for runtime_chunk_ukey in runtime_chunks {
      ...
      let chunk_hash_result = self
        ...
        .await?;
      let chunk = self.build_chunk_graph_artifact.chunk_by_ukey.expect_get(&runtime_chunk_ukey);
      let chunk_hashes_changed = chunk.set_hashes(
    ...
    for chunk_ukey in full_hash_chunks {
      ...
      let chunk = self.build_chunk_graph_artifact.chunk_by_ukey.expect_get(&chunk_ukey);
      let new_chunk_hash = {
  ...
  async fn process_chunk_hash(
    ...
    let mut hasher = RspackHash::from(&self.options.output);
    if let Some(chunk) = self.build_chunk_graph_artifact.chunk_by_ukey.get(&chunk_ukey) {
