...
impl Compilation {
  ...
  pub fn delete_asset(&mut self, filename: &str) {
    if let Some(asset) = self.assets.remove(filename) {
      ...
      self.build_chunk_graph_artifact.chunk_by_ukey.iter_mut().for_each(|(_, chunk)| {
  ...
  pub fn entrypoint_by_name(&self, name: &str) -> &Entrypoint {
    let ukey = self.build_chunk_graph_artifact.entrypoints.get(name).expect("entrypoint not found");
    self.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(ukey)
  }

  pub fn entrypoint_by_name_mut(&mut self, name: &str) -> &mut Entrypoint {
    let ukey = self.build_chunk_graph_artifact.entrypoints.get(name).expect("entrypoint not found");
    self.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get_mut(ukey)
  }

  pub fn get_chunk_graph_entries(&self) -> impl Iterator<Item = ChunkUkey> + use<'_> {
    let entries = self.build_chunk_graph_artifact.entrypoints.values().map(|entrypoint_ukey| {
      let entrypoint = self.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(entrypoint_ukey);
      entrypoint.get_runtime_chunk(&self.build_chunk_graph_artifact.chunk_group_by_ukey)
    });
    let async_entries = self.build_chunk_graph_artifact.async_entrypoints.iter().map(|entrypoint_ukey| {
      let entrypoint = self.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(entrypoint_ukey);
      entrypoint.get_runtime_chunk(&self.build_chunk_graph_artifact.chunk_group_by_ukey)
    });
  ...
  pub fn add_runtime_module(
    ...
    self.build_chunk_graph_artifact.chunk_graph.add_module(runtime_module_identifier);
