...
impl Compilation {
  ...
  async fn create_module_assets(&mut self, _plugin_driver: SharedPluginDriver) {
    ...
    for (identifier, module) in mg.modules() {
      ...
      {
        for chunk in self.build_chunk_graph_artifact.chunk_graph.get_module_chunks(identifier).iter() {
    ...
    for (chunk, asset_name) in chunk_asset_map {
      let chunk = self.build_chunk_graph_artifact.chunk_by_ukey.expect_get_mut(&chunk);
