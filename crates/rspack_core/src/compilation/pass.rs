...
pub trait PassExt: Send + Sync {
  ...
  ///
  /// The inner `run_pass` future is boxed so that `run()`'s async state
  /// machine stays small — without boxing, the compiler inlines the
  /// entire pass state (which can be large) into `run()`'s future,
  ...
  fn run_pass_with_cache<'a>(
    ...
    let boxed: Pin<Box<dyn Future<Output = Result<()>> + Send + 'a>> =
      Box::pin(self.run_pass(compilation));
    boxed
  }
