...
use crate::{
  ...
  PluginRenderRuntimeHookOutput, ProcessAssetStage, ProcessAssetsArgs, RenderManifestArgs,
  RenderRuntimeArgs, Resolver,
};
...
pub struct PluginDriver {
  ...
  pub registered_process_assets_stages: Vec<(usize, ProcessAssetStage)>,
...
impl PluginDriver {
  pub fn new(
    ...
    let apply_contexts = plugins
      ...
      .collect::<Vec<_>>();
    let mut registered_parser = HashMap::default();
    let mut registered_process_assets_stages = vec![];
    apply_contexts
      .into_iter()
      .enumerate()
      .for_each(|(plugin_index, mut apply_context)| {
        apply_context
          .registered_parser
          .into_iter()
          .for_each(|(module_type, parser)| {
            if registered_parser.contains_key(&module_type) {
              panic!("Duplicate parser for module type: {:?}", module_type);
            }
            registered_parser.insert(module_type, parser);
          });
        if apply_context.registered_process_assets_stages.is_empty() {
          // If no stage is registered, we will register a default stage
          // TODO: Will this behaviors cause any confusions?
          apply_context
            .registered_process_assets_stages
            .insert(ProcessAssetStage::Default);
        }
        registered_process_assets_stages.extend(
          apply_context
            .registered_process_assets_stages
            .into_iter()
            .map(|stage| (plugin_index, stage)),
        );
      });
    registered_process_assets_stages.sort_by_key(|(_, stage)| *stage);
  ...
  #[instrument(skip_all)]
  pub async fn process_assets(&self, compilation: &mut Compilation) -> PluginProcessAssetsOutput {
    for (plugin_index, stage) in &self.registered_process_assets_stages {
      let plugin = &self.plugins[*plugin_index];
      tracing::debug!("running process assets:{}", plugin.name());
      plugin
        .process_assets(
          PluginContext::new(),
          ProcessAssetsArgs { compilation, stage },
        )
        .await?;
    }
    Ok(())
