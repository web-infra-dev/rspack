...
pub struct ResolveContextImpl {
  ...
  pub file_dependencies: Option<Vec<InternedPath>>,

  /// Files that were not found on file system
  pub missing_dependencies: Option<Vec<InternedPath>>,
...
impl ResolveContext {
  ...
  // Accepts anything convertible to `InternedPath`. The conversion (which
  ...
  pub fn add_file_dependency<P: Into<InternedPath>>(&mut self, dep: P) {
    if let Some(deps) = &mut self.file_dependencies {
      deps.push(dep.into());
    }
  }

  pub fn add_missing_dependency<P: Into<InternedPath>>(&mut self, dep: P) {
