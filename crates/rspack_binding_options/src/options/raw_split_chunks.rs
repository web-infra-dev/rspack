...
impl From<RawSplitChunksOptions> for new_split_chunks_plugin::PluginOptions {
  fn from(raw_opts: RawSplitChunksOptions) -> Self {
    // let mut defaults = SplitChunksOptions {
    //   max_async_requests: raw_opts.max_async_requests,
    //   max_initial_requests: raw_opts.max_initial_requests,
    //   min_chunks: raw_opts.min_chunks,
    //   min_size: raw_opts.min_size,
    //   enforce_size_threshold: raw_opts.enforce_size_threshold,
    //   min_remaining_size: raw_opts.min_remaining_size,
    //   chunks: raw_opts.chunks.map(|chunks| match chunks.as_str() {
    //     "initial" => ChunkType::Initial,
    //     "async" => ChunkType::Async,
    //     "all" => ChunkType::All,
    //     _ => panic!("Invalid chunk type: {chunks}"),
