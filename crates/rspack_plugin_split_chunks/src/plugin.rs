...
use crate::{
  ...
  ChunksInfoMap, GetName, NormalizedFallbackCacheGroup, NormalizedOptions, SizeType,
  SplitChunkSizes, SplitChunksOptions,
};
...
impl SplitChunksPlugin {
  ...
  #[tracing::instrument(skip_all)]
  fn find_best_entry(&self, chunks_info_map: &mut ChunksInfoMap) -> ChunksInfoItem {
  ...
  fn optimize_chunks(
    ...
    split_chunks_span.in_scope(|| {
      while !chunks_info_map.is_empty() {
        let item = &mut self.find_best_entry(&mut chunks_info_map);
