...
impl SplitChunksPlugin {
  ...
  pub(super) async fn ensure_max_size_fit(
    ...
    let chunks_with_size_info_results = rspack_futures::scope::<_, Result<_>>(|token| {
      compilation_ref.build_chunk_graph_artifact.chunk_by_ukey.values().for_each(|chunk| {
        let s = unsafe {
          token.used((
            &*compilation,
            chunk,
    ...
    infos_with_results.into_iter().for_each(|(info, results)| {
      ...
      results.into_iter().enumerate().for_each(|(index, group)| {
        ...
        let chunk = compilation.build_chunk_graph_artifact.chunk_by_ukey.expect_get_mut(&info.chunk);
        ...
        if index != last_index {
          ...
          chunk.split(new_part, &mut compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
          *new_part.chunk_reason_mut() = chunk.chunk_reason().map(ToString::to_string);
          ...
          group.nodes.iter().for_each(|module| {
            compilation.build_chunk_graph_artifact.chunk_graph.add_chunk(new_part_ukey);
