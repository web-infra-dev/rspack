...
impl SplitChunksPlugin {
  ...
  pub(crate) fn get_corresponding_chunk(
    ...
    if let Some(chunk_name) = &module_group.chunk_name {
      if let Some(chunk) = compilation.build_chunk_graph_artifact.named_chunks.get(chunk_name) {
        ...
        *chunk
        ...
        compilation.build_chunk_graph_artifact.chunk_graph.add_chunk(new_chunk.ukey());
        new_chunk.ukey()
      }
  ...
  pub(crate) fn split_from_original_chunks(
    ...
    for original_chunk_ukey in original_chunks {
      ...
      original_chunk.split(new_chunk, &mut compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
      if let Some(mut mutations) = compilation.incremental.mutations_write() {
