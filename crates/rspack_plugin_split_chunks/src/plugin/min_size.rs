...
use crate::{SplitChunkSizes, SplitChunksPlugin, common::ModuleSizes, module_group::ModuleGroup};
...
pub fn remove_min_size_violating_modules<T: ModulesContainer>(
  ...
  let violating_source_types: Box<[SourceType]> = module_group
    ...
    .filter_map(|(module_group_ty, module_group_ty_size)| {
      let cache_group_ty_min_size = min_size
        .get(module_group_ty)
        .copied()
        .unwrap_or_default();
