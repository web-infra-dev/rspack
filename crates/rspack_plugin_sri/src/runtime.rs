...
impl RuntimeModule for SRIHashVariableRuntimeModule {
  ...
  async fn generate(&self, compilation: &Compilation) -> Result<String> {
    let Some(chunk) = compilation.build_chunk_graph_artifact.chunk_by_ukey.get(&self.chunk) else {
      return Err(error!(
    ...
    let include_chunks = chunk
      ...
      .iter()
      .filter_map(|c| {
        let chunk = compilation.build_chunk_graph_artifact.chunk_by_ukey.get(c)?;
        let id = chunk.id()?;
    ...
    for (source_type, variable_ref) in source_types {
      let chunk_with_source_type = all_chunks
        ...
        .map(|c| compilation.build_chunk_graph_artifact.chunk_by_ukey.expect_get(c).expect_id())
