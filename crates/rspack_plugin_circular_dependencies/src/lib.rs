...
impl<'a> CycleDetector<'a> {
  ...
  fn find_circular_modules(mut self) -> IdentifierSet {
    for module_index in 0..self.graph.modules.len() {
      if self.states[module_index].index.is_none() {
  ...
  fn connect(&mut self, module_index: usize) {
    ...
    while !visit_stack.is_empty() {
      let next_target = {
        ...
        if frame.next_edge_index < self.graph.edges[module_index].len() {
          let target_index = self.graph.edges[module_index][frame.next_edge_index];
      ...
      if let Some((module_index, target_index)) = next_target {
        if self.states[target_index].index.is_none() {
        ...
        } else if self.states[target_index].on_stack {
          self.states[module_index].low_link = self.states[module_index]
            .low_link
            .min(self.states[target_index].index.expect("indexed"));
        }
        continue;
      }
      ...
      if let Some(parent_index) = frame.parent_index {
        self.states[parent_index].low_link = self.states[parent_index]
          .low_link
          .min(self.states[module_index].low_link);
      }
    }
  }

  fn start_connect(&mut self, module_index: usize) {
    let index = self.next_index;
    self.next_index += 1;
    self.states[module_index] = NodeState {
      index: Some(index),
      low_link: index,
      on_stack: true,
    };
  ...
  fn finish_connect(&mut self, module_index: usize) {
    if self.states[module_index].low_link == self.states[module_index].index.expect("indexed") {
      let mut component = Vec::new();
      loop {
        let current = self.stack.pop().expect("root should be on the stack");
        self.states[current].on_stack = false;
