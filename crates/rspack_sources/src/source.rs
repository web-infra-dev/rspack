...
impl SourceMap<'static> {
  ...
  pub fn from_bytes(mut bytes: Vec<u8>) -> SourceResult<Self> {
    let fields = {
  ...
  pub fn from_json(s: String) -> SourceResult<Self> {
    ...
    Self {
      ...
      fields: SourceMapFields {
        ...
        file: self
          ...
          .file
          .as_ref()
          .map(|file| Cow::Owned(file.to_string())),
        sources: Cow::Owned(
          self
            .fields
            .sources
        ...
        sources_content: Cow::Owned(
          self
            .fields
            .sources_content
            .iter()
            .map(|source_content| Cow::Owned(source_content.to_string()))
            .collect(),
        ),
        names: Cow::Owned(
          self
        ...
        mappings: Cow::Owned(self.fields.mappings.to_string()),
        source_root: self
          .fields
          .source_root
          .as_ref()
          .map(|source_root| Cow::Owned(source_root.to_string())),
        debug_id: self
          .fields
          .debug_id
          .as_ref()
          .map(|debug_id| Cow::Owned(debug_id.to_string())),
        ignore_list: self
          .fields
          .ignore_list
          .as_ref()
          .map(|ignore_list| Cow::Owned(ignore_list.to_vec())),
      },
    }
  }
}
...
fn deserialize_source_map_fields<'a>(
  value: &'a BorrowedValue<'a>,
) -> SourceResult<SourceMapFields<'a>> {
  let object = value
    .as_object()
