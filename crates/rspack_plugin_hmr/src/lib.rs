  ...
  for (chunk_id, (old_runtime, old_module_ids)) in &old_chunks {
    ...
    for old_module_id in remaining_modules {
      ...
      let runtimes = compilation
        .build_chunk_graph_artifact.chunk_graph
        .get_module_runtimes(*module_identifier, &compilation.build_chunk_graph_artifact.chunk_by_ukey);
    ...
    if !new_modules.is_empty() || !new_runtime_modules.is_empty() {
      ...
      compilation.build_chunk_graph_artifact.chunk_by_ukey.add(hot_update_chunk);
      ...
      compilation.build_chunk_graph_artifact.chunk_graph.add_chunk(ukey);
      ...
      compilation.build_chunk_graph_artifact.chunk_graph.remove_chunk(&ukey);
      #[allow(clippy::unwrap_used)]
      let hot_update_chunk = compilation.build_chunk_graph_artifact.chunk_by_ukey.remove(&ukey).unwrap();
