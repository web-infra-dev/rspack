...
async fn process_assets(&self, compilation: &mut Compilation) -> Result<()> {
  ...
  for (chunk_id, (old_runtime, old_module_ids)) in old_chunks {
    ...
    // The extract-css filename this build emitted for a chunk whose extracted css
    // changed, matched by asset ownership (the extract-css plugin tags its own assets
    // with this type, the same convention rspack_plugin_sri relies on) rather than by a
    // `.css` suffix: a chunk can carry both native css and extract-css output, or
    // another plugin's own `.css`-suffixed asset, and picking "the first `.css` file"
    // could grab the wrong one. Threaded into the manifest so the runtime can request
    // a content-hashed stylesheet URL while `miniCssF` still names the previous one.
    let updated_mini_css_filename = current_chunk
      .filter(|_| mini_css_update == CssUpdate::Changed)
      .and_then(|chunk| {
        chunk
          .files()
          .iter()
...
struct HotUpdateContent {
  ...
  // extract-css filename freshly emitted for an updated chunk, keyed by chunk id. The
  // chunk filename runtime function (`miniCssF`) is only re-applied once the update is
  // applied, so while the extract-css HMR handler downloads the update it would still
  // resolve the previous, stale name of a content-hashed stylesheet.
...
fn css_manifest_json(
  ...
  // runtime can request the current (possibly content-hashed) URL rather than the one
  // `miniCssF` still resolves to during the download. Only the extract-css consumer
