...
impl<I1> SyncSeriesHook<I1> {
  ...
  pub fn tap(&mut self, tap: impl SyncSeries<I1> + Send + Sync + 'static) {
    self.taps.push(Box::new(tap));
  }
}

pub trait SyncSeries3<I1, I2, I3> {
  fn run(&self, input1: &mut I1, input2: &mut I2, input3: &mut I3) -> Result<()>;
  fn stage(&self) -> i32 {
    0
  }
}

pub struct SyncSeries3Hook<I1, I2, I3> {
  taps: Vec<Box<dyn SyncSeries3<I1, I2, I3> + Send + Sync>>,
  interceptors: Vec<Box<dyn Interceptor<Self> + Send + Sync>>,
}

impl<I1, I2, I3> Hook for SyncSeries3Hook<I1, I2, I3> {
  type Tap = Box<dyn SyncSeries3<I1, I2, I3> + Send + Sync>;
...
impl<I1, I2, I3> fmt::Debug for SyncSeries3Hook<I1, I2, I3> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(f, "SyncSeries3Hook(..)")
  }
}

impl<I1, I2, I3> Default for SyncSeries3Hook<I1, I2, I3> {
  fn default() -> Self {
    Self {
      taps: Default::default(),
      interceptors: Default::default(),
...
impl<I1, I2, I3> SyncSeries3Hook<I1, I2, I3> {
  pub fn call(&self, input1: &mut I1, input2: &mut I2, input3: &mut I3) -> Result<()> {
    let mut additional_taps = Vec::new();
    for interceptor in self.interceptors.iter() {
      additional_taps.extend(interceptor.call_blocking(self)?);
    }
    ...
    all_taps.sort_by_key(|hook| hook.stage());
    for tap in all_taps {
      tap.run(input1, input2, input3)?;
    }
    Ok(())
  }

  pub fn tap(&mut self, tap: impl SyncSeries3<I1, I2, I3> + Send + Sync + 'static) {
    self.taps.push(Box::new(tap));
  }
}
...
impl<I1, I2, I3, I4> SyncSeries4Hook<I1, I2, I3, I4> {
  ...
  pub fn tap(&mut self, tap: impl SyncSeries4<I1, I2, I3, I4> + Send + Sync + 'static) {
    self.taps.push(Box::new(tap));
  }
}
