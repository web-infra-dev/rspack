...
use rspack_core::{
  ...
  DependencyTemplate, ExternalModule, InitFragmentExt, InitFragmentKey, InitFragmentStage,
  NormalInitFragment,
};
  ...
  for module_id in entry_modules {
    ...
    if matches!(
      ...
      let has_unknown_exports = module.get_dependencies().iter().any(|dep_id| {
        ...
        let dep = mg.dependency_by_id(dep_id);
        let Some(exports) = dep.get_exports(mg, &compilation.module_graph_cache_artifact) else {
          return false;
        };
