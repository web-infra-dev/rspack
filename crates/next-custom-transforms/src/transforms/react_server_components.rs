...
impl ReactServerComponentValidator {
        ...
        for specifier in &import.specifiers {
          if invalid_apis.contains(&specifier.0.as_str()) {
            report_error(
              &self.app_dir,
              &self.filepath,
              RSCErrorKind::NextRscErrClientImport((specifier.0.to_string(), specifier.1)),
            );
          }
        }
      }
  ...
  fn assert_invalid_api(&self, module: &Module, is_client_entry: bool) {
    ...
    static RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"[\\/](page|layout)\.(ts|js)x?$").unwrap());
        ...
        match export_name {
          ...
          "dynamicParams" | "dynamic" | "fetchCache" | "runtime" | "revalidate" => {
            if self.dynamic_io_enabled {
              invalid_exports.insert(export_name.to_string(), InvalidExportKind::DynamicIoSegment);
              true
            } else {
              false
            }
        ...
        match kind {
          InvalidExportKind::DynamicIoSegment => {
            report_error(
              &self.app_dir,
              &self.filepath,
              RSCErrorKind::NextRscErrIncompatibleDynamicIoSegment(span, export_name.clone()),
            );
          }
            ...
            if is_client_entry {
              if has_gm_export || has_metadata_export {
                report_error(
                  &self.app_dir,
                  &self.filepath,
                  RSCErrorKind::NextRscErrClientMetadataExport((invalid_export_name.clone(), span)),
                );
              }
            } else {
