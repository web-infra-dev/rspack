...
use swc_core::{
  ...
  ecma::{
    ast::{
      op, ArrayLit, ArrowExpr, BinExpr, BlockStmt, BlockStmtOrExpr, Bool, CallExpr, Callee, Expr,
      ExprOrSpread, ExprStmt, Id, Ident, IdentName, ImportDecl, ImportNamedSpecifier,
      ImportSpecifier, KeyValueProp, Lit, ModuleDecl, ModuleItem, ObjectLit, Pass, Prop, PropName,
      ...
      if let Callee::Import(..) = &expr.callee {
        match &*expr.args[0].expr {
          ...
          Expr::Tpl(Tpl { exprs, quasis, .. }) if exprs.is_empty() => {
            self.dynamically_imported_specifier = Some((quasis[0].raw.clone(), quasis[0].span));
          }
          _ => {}
        }
      }
