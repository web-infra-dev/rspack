  ...
  fn should_retain_export_type(&self, export_type: ExportType) -> bool {
    !matches!(
      ...
      (
        ...
        ExportType::GetServerSideProps | ExportType::GetStaticProps | ExportType::GetStaticPaths,
...
impl Analyzer<'_> {
  ...
  fn within_removed_item<R>(&mut self, in_removed_item: bool, f: impl FnOnce(&mut Self) -> R) -> R {
          ...
          ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
            ...
            decl: Decl::Var(Box::new(VarDecl {
              ...
              decls: vec![VarDeclarator {
                span: DUMMY_SP,
                name: Pat::Ident(IdentName::new(data_marker.into(), DUMMY_SP).into()),
                init: Some(true.into()),
  ...
  fn fold_expr(&mut self, e: Expr) -> Expr {
    match e {
      Expr::Assign(assign_expr) => {
        let mut retain = true;
        let left = self.within_lhs_of_var(true, |this| assign_expr.left.clone().fold_with(this));
        ...
        Decl::Fn(f) => {
          if let Some(export_type) = self.state.export_type(&f.ident.to_id()) {
            if self.state.dropping_export(export_type) {
              tracing::trace!("Dropping an export specifier because it's an SSR/SSG function");
              return ModuleItem::Stmt(Stmt::Empty(EmptyStmt { span: DUMMY_SP }));
            }
          }
        }
