      ...
      Decl::Var(box var_decl) => {
        ...
        }) = var_decl.decls.first()
        {
          if EXPORTS_SET.contains(&name.sym.as_str()) {
            ...
            export_info.generate_image_metadata = Some(name.sym == "generateImageMetadata");
  ...
  fn visit_named_export(&mut self, named_export: &NamedExport) {
    for specifier in &named_export.specifiers {
      ...
      }) = specifier
      {
        ...
        if !export_info.generate_sitemaps.unwrap_or_default() && value.sym == "generateSitemaps" {
        ...
        if export_info.preferred_region.is_empty() && value.sym == "preferredRegion" {
          export_info.warnings.push(ExportInfoWarning::new(
            value.sym.to_string(),
            "it was not assigned to a string literal or an array of string literals".to_string(),
          ));
        }
      }
    }
