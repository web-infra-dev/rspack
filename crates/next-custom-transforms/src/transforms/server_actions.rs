...
pub struct Config {
  ...
  pub cache_kinds: FxHashSet<Arc<str>>,
}

#[derive(Clone, Debug)]
enum Directive {
  UseServer,
  UseCache { cache_kind: Arc<str> },
}
...
impl<C: Comments> VisitMut for ServerActions<C> {
  ...
  fn visit_mut_arrow_expr(&mut self, a: &mut ArrowExpr) {
    ...
    let directive =
      self.get_directive_for_function(if let BlockStmtOrExpr::BlockStmt(block) = &mut *a.body {
        Some(block)
      } else {
        None
      });
    ...
    if let Some(directive) = directive {
      ...
      if let Directive::UseCache { cache_kind } = directive {
        self.rewrite_expr_to_proxy_expr =
          Some(self.maybe_hoist_and_create_proxy_for_cache_arrow_expr(child_names, cache_kind, a));
      } else if !matches!(self.file_directive, Some(Directive::UseServer)) {
        self.rewrite_expr_to_proxy_expr =
  ...
  fn visit_mut_module_items(&mut self, stmts: &mut Vec<ModuleItem>) {
    ...
    for mut stmt in stmts.take() {
        ...
        match &mut stmt {
              ...
              Decl::Var(var) => {
                ...
                for ident in &idents {
                  self.exported_idents.push((
                    ident.clone(),
                    ident.sym.to_string(),
                    self.generate_server_reference_id(ident.sym.as_ref(), in_cache_file, None),
                  ));
                }
              ...
              for spec in &mut named.specifiers {
                ...
                }) = spec
                {
                  if let Some(export_name) = exported {
                    if let ModuleExportName::Ident(Ident { sym, .. }) = export_name {
                      // export { foo as bar }
                      self.exported_idents.push((
                        ident.clone(),
                        sym.to_string(),
                        self.generate_server_reference_id(sym.as_ref(), in_cache_file, None),
                      ));
                      ...
                      self.exported_idents.push((
                        ident.clone(),
                        str.value.to_string(),
                        self.generate_server_reference_id(str.value.as_ref(), in_cache_file, None),
                      ));
                    }
                  } else {
                    // export { foo }
          ...
          ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
            ...
            DefaultDecl::Fn(f) => {
              ...
              if !(is_cache_fn && self.config.is_react_server_layer) {
                let ref_id =
                  self.generate_server_reference_id("default", is_cache, Some(&f.function.params));

                if let Some(ident) = &f.ident {
                  // export default function foo() {}
                  self
                    .exported_idents
                    .push((ident.clone(), "default".into(), ref_id));
                } else {
                  // export default function() {}
                  // Use the span from the function expression
                  let span = f.function.span;

                  let new_ident = Ident::new(self.gen_action_ident(), span, self.private_ctxt);

                  f.ident = Some(new_ident.clone());

                  self
                    .exported_idents
                    .push((new_ident.clone(), "default".into(), ref_id));
            ...
            match &mut *default_expr.expr {
              ...
              Expr::Arrow(arrow) => {
                ...
                let (is_action_fn, is_cache_fn) =
                  has_body_directive(&if let BlockStmtOrExpr::BlockStmt(block) = &*arrow.body {
                    Some(block.clone())
                  } else {
                    None
                  });
                ...
                if !(is_cache_fn && self.config.is_react_server_layer) {
                  ...
                  *default_expr.expr = assign_arrow_expr(&new_ident, Expr::Arrow(arrow.clone()));
                }
              }
              Expr::Ident(ident) => {
                // export default foo
                self.exported_idents.push((
                  ident.clone(),
                  "default".into(),
                  self.generate_server_reference_id("default", in_cache_file, None),
                ));
              }
              Expr::Call(call) => {
                // export default fn()
                ...
                *default_expr.expr = assign_arrow_expr(&new_ident, Expr::Call(call.clone()));
              }
              _ => {
      ...
      if let Some(expr) = &self.rewrite_default_fn_expr_to_proxy_expr {
        ...
        new_stmt = ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultExpr(ExportDefaultExpr {
          span: DUMMY_SP,
          expr: expr.clone(),
        }));
        self.rewrite_default_fn_expr_to_proxy_expr = None;
      ...
      for (ident, export_name, ref_id) in self.exported_idents.iter() {
        if !self.config.is_react_server_layer {
          if export_name == "default" {
            ...
            let export_expr = ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
              span: DUMMY_SP,
              decl: Decl::Var(Box::new(VarDecl {
                ...
                decls: vec![VarDeclarator {
                  span: DUMMY_SP,
                  name: Pat::Ident(IdentName::new(export_name.clone().into(), ident.span).into()),
    ...
    match stmt {
      Stmt::Expr(ExprStmt {
        ...
        if value == "use cache" || value.starts_with("use cache: ") {
          if in_fn_body && !allow_inline {
            emit_error(ServerActionsErrorKind::InlineUseCacheInClientComponent { span: *span })
            ...
            if value == "use cache" {
              self.directive = Some(Directive::UseCache {
                cache_kind: Arc::from("default"),
              });
            } else {
              // Slice the value after "use cache: "
              let cache_kind = Arc::from(value.split_at("use cache: ".len()).1);
