...
pub fn make_paths_relative(context: &str, identifier: &str) -> String {
  SEGMENTS_SPLIT_REGEXP
    .split(identifier)
    .map(|s| absolute_to_request(context.to_string(), s.to_string()))
    .collect::<Vec<_>>()
    .join("")
}
...
#[cached]
pub fn absolute_to_request(context: String, maybe_absolute_path: String) -> String {
  ...
  if let Some(query_part) = query_part {
    concat_string!(relative_resource, query_part)
  } else {
    relative_resource
  }
}
