...
pub use context::{JsLoaderItem, LoaderContextFromJs};
use napi::{
  bindgen_prelude::*,
...
use crate::{
  COMPILER_REFERENCES, RspackResultToNapiResultExt, plugins::js_loader::context::LoaderContextToJs,
};
  ...
  let result = COMPILER_REFERENCES.with(|ref_cell| {
    if let Some(weak_reference) = ref_cell.borrow().get(&compiler_id) {
      ...
      let run_loader = compiler_object
        .get_named_property::<Function<LoaderContextToJs, Promise<LoaderContextFromJs>>>(
          "_runLoader",
        )?;
