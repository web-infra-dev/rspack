...
fn create_module_type_filter(
  ...
  match raw {
    ...
    Either::B(js_str) => {
      ...
      rspack_plugin_split_chunks::ModuleTypeFilter::String(type_str)
    }
  }
}
...
fn create_module_layer_filter(
  ...
  match raw {
    ...
    Either3::B(js_str) => {
      ...
      rspack_plugin_split_chunks::ModuleLayerFilter::String(test)
    }
    Either3::C(f) => {
      rspack_plugin_split_chunks::ModuleLayerFilter::Func(std::sync::Arc::new(move |layer| {
        let f = f.clone();
