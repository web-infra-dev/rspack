...
impl<'a> JavaScriptTransformer<'a> {
  ...
  fn parse_js(
    ...
    let program_result = match is_module {
      IsModule::Bool(true) => {
        parse_file_as_module(fm.as_ref(), syntax, target, comments, &mut errors)
          .map(Program::Module)
      }
      IsModule::Bool(false) => {
        parse_file_as_script(fm.as_ref(), syntax, target, comments, &mut errors)
          .map(Program::Script)
      }
      IsModule::Unknown => {
        parse_file_as_program(fm.as_ref(), syntax, target, comments, &mut errors)
      }
      IsModule::CommonJS => {
        parse_file_as_commonjs(fm.as_ref(), syntax, target, comments, &mut errors)
          .map(Program::Script)
      }
    };
