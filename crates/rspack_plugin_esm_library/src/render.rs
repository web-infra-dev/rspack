...
use rspack_core::{
  AssetInfo, Chunk, ChunkGraph, ChunkLinkContext, ChunkRenderContext, ChunkUkey, Compilation,
  ConcatenatedModuleInfo, ConcatenationScope, InitFragment, ModuleIdentifier, PathData, PathInfo,
  Ref, RuntimeGlobals, SourceType, get_js_chunk_filename_template, get_undo_path,
...
impl EsmLibraryPlugin {
  ...
  pub(crate) async fn render_chunk(
    ...
    for source in &chunk_link.raw_star_exports {
      final_source.add(RawStringSource::from(format!("export * from {source};\n",)));
    }
