...
async fn additional_chunk_runtime_requirements(
  ...
  let runtime_chunk = EsmLibraryPlugin::get_runtime_chunk(*chunk_ukey, compilation);

  if *chunk_ukey != runtime_chunk && !runtime_requirements.is_empty() {
    ...
    let link = chunk_link
      .get_mut(&runtime_chunk)
      .expect("should have link for chunk");
    link.static_exports.insert(RuntimeGlobals::REQUIRE.into());
  }
