...
use rspack_core::{
  BuildMetaDefaultObject, BuildMetaExportsType, ChunkGraph, ChunkInitFragments, ChunkLinkContext,
  ...
  ModuleInfo, NAMESPACE_OBJECT_EXPORT, NormalInitFragment, PathData, PrefetchExportsInfoMode, Ref,
  RuntimeGlobals, SourceType, SymbolRef, UsageState, UsedName, UsedNameItem, escape_name,
            ...
            match info {
              ...
              rspack_core::ModuleInfo::Concatenated(mut concate_info) => {
                ...
                let scope = codegen_res
                  ...
                  .as_ref()
                  .expect("should have concatenation scope");
                concate_info.star_exports = scope.star_exports.clone();
  ...
  */
  ...
  fn link_re_export(
    ...
    if let Some(info) = info.try_as_concatenated() {
      for (ref_module, re_exports) in &info.re_exports {
        for mode in re_exports {
          ...
          match mode {
            ...
            rspack_core::ExportMode::EmptyStar(_) | rspack_core::ExportMode::Unused(_) => {}
              ...
              if let ModuleInfo::External(ref_info) = ref_info {
                ...
                if ref_chunk != current_chunk {
                  ...
                  chunk_link.add_re_export(
                    ref_chunk,
                    exported.expect("should have exported").clone(),
                    mode.name.clone(),
                  );
                }
              }
            ...
            rspack_core::ExportMode::ReexportNamespaceObject(mode) => {
                ...
                .expect("should have info")
              {
                ModuleInfo::External(ref_info) => {
                  ...
                  entry_imports.entry(*ref_module).or_default();
                  Self::add_export(current_chunk, namespace, mode.name.clone(), exports, true);
                }
                ModuleInfo::Concatenated(ref_info) => {
                  ...
                  if ref_chunk != current_chunk {
                    ...
                    chunk_link.add_re_export(
                      ref_chunk,
                      exported.cloned().expect("should have exported"),
                      mode.name.clone(),
                    );
                  }
                }
            ...
            rspack_core::ExportMode::NormalReexport(mode) => {
              for item in &mode.items {
                ...
                if let Some(UsedNameItem::Inlined(inlined)) = export_info.get_used_name(None, None)
                {
                  let chunk_link = link.get_mut(&current_chunk).expect("should have link");
                  ...
                  Self::add_export(current_chunk, new_name, item.name.clone(), exports, true);
                  continue;
                };
                ...
                let ref_info = if let Some(target) = export_info.get_max_target().values().next() {
                  ...
                  let module = conn.module_identifier();
                ...
                match ref_info {
                  ...
                  ModuleInfo::Concatenated(ref_info) => {
                    ...
                    if ref_chunk != current_chunk {
                      ...
                      chunk_link.add_re_export(
                        ref_chunk,
                        exported.cloned().expect("should have export"),
                        item.name.clone(),
                      );
                    }
                  }
  ...
  fn link_imports_and_exports(
        ...
        match entry_info {
          ModuleInfo::Concatenated(info) => {
            if let Some(export_map) = info.export_map.as_ref() {
              for (export_name, local) in export_map {
                ...
                let exported = Self::add_export(
                  ...
                  export_name.clone(),
                  &mut exports,
                  entry_module_chunk == entry_chunk_ukey,
                );
            ...
            for (name, export_info) in exports_info.exports() {
              let Some(UsedNameItem::Inlined(inlined)) = export_info.get_used_name(None, None)
              else {
                continue;
              };
              ...
              let new_name = find_new_name(
                ...
                escaped_identifiers
                  .get(&get_cached_readable_identifier(
                    &entry_module,
                    &module_graph,
                    &compilation.module_static_cache_artifact,
                    context,
              ...
              Self::add_export(entry_chunk_ukey, new_name, name.clone(), &mut exports, true);
            }
          }
          ...
          for (ref_string, ref_atom) in all_refs.iter() {
            ...
            if let Ref::Symbol(symbol_binding) = &mut binding {
              ...
              if from_other_chunk && !ref_external {
                ...
                symbol_binding.symbol = exported.cloned().expect("should have exported");
              }
            }
    ...
    for (chunk_ukey, chunk_link) in link.iter_mut() {
      for m in &chunk_link.hoisted_modules {
        let info = concate_modules_map
          .get(m)
          .expect("should have module info")
          .as_concatenated();
        for (source, module) in &info.star_exports {
          chunk_link.raw_star_exports.insert(source.clone());
          if let Some(module) = module {
            let imports = imports.get_mut(chunk_ukey).expect("should have imports");
            if let Some(imported_symbols) = imports.get(module)
              && imported_symbols.is_empty()
            {
              imports.swap_remove(module);
            }
          }
        }
      }
