...
pub(crate) fn ensure_entry_exports(compilation: &mut Compilation) {
  ...
  for entrypoint_ukey in compilation.build_chunk_graph_artifact.entrypoints.values() {
    let entrypoint = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(entrypoint_ukey);
  ...
  compilation
    ...
    .for_each(|(ukey, _)| {
      let modules = compilation.build_chunk_graph_artifact.chunk_graph.get_chunk_modules_identifier(ukey);
  ...
  for entry_chunk_ukey in dirty_chunks {
    ...
    let entrypoint = compilation.build_chunk_graph_artifact.chunk_group_by_ukey.expect_get(&entrypoint_ukey);
    if entrypoint.get_runtime_chunk(&compilation.build_chunk_graph_artifact.chunk_group_by_ukey) == entry_chunk_ukey {
    ...
    entry_chunk.split(new_chunk, &mut compilation.build_chunk_graph_artifact.chunk_group_by_ukey);
  }
}
