...
impl RuntimeModeRenderer for RspackExportRuntimeRenderer {
  ...
  fn render_runtime(&self, context: RuntimeRenderContext<'_>) -> ConcatSource {
    ...
    let mut add_definition = |runtime_global: RuntimeGlobals, definition: RawStringSource| {
      ...
      .contains(RuntimeGlobals::MODULE_FACTORIES)
    {
      add_definition(
        ...
        render_module_factories_definition(context.runtime_template),
      );
    }
    if context
      .runtime_requirements
      .contains(RuntimeGlobals::MODULE_CACHE)
    {
      add_definition(
        ...
        render_module_cache_definition(context.runtime_template),
      );
    }
    if context
      .runtime_requirements
      .contains(RuntimeGlobals::INTERCEPT_MODULE_EXECUTION)
    {
      add_definition(
        RuntimeGlobals::INTERCEPT_MODULE_EXECUTION,
        render_intercept_module_execution_definition(context.runtime_template),
      );
    }
    source
  }
