    ...
    let program = self.run(|| {
      ...
      match result {
        ...
        let help_msg = formatdoc!{"
          ...
          Please visit https://plugins.swc.rs/versions/from-core/{swc_core_version_strip_prefix} to verify the `swc_core` version of the SWC Wasm plugin and ensure it falls within the compatible range.
