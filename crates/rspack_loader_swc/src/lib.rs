...
impl SwcLoader {
  ...
  fn loader_impl(&self, loader_context: &mut LoaderContext<RunnerContext>) -> Result<()> {
    ...
    if let Some(isolated_dts) = isolated_dts? {
      ...
      set_build_info(
        loader_context.context.build_data.build_info.get_mut(),
    ...
    if let Some(rsc) = rsc_meta.borrow_mut().take() {
      ...
      loader_context.context.build_data.build_info.get_mut().rsc = Some(rsc);
      if let Some(code) =
        to_server_entry(module, loader_context.context.build_data.build_info.read())?
      {
