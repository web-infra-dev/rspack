...
fn template_replace(s: &str, ctx: &ModuleFilenameTemplateFnCtx) -> String {
  ...
  for i in memchr::memchr2_iter(b'[', b']', s) {
    ...
    match s[i] {
      b'[' => {
        ...
        let s = str::from_utf8(&s[pos..i]).unwrap();
        buf.push_str(s);
        pos = i;
        state = true;
      }
      b']' if state => {
        ...
        match_ignore_case!(&s[pos..next_pos];
            ...
            b"[all-loaders]" | b"[allloaders]" => if s[next_pos..].starts_with(resource_tag) {
                next_pos += resource_tag.len();
                buf.push_str(&ctx.identifier);
            } else {
                buf.push_str(&ctx.all_loaders);
            },
            b"[loaders]" => if s[next_pos..].starts_with(resource_tag) {
                next_pos += resource_tag.len();
  ...
  // always utf8
  #[allow(clippy::unwrap_used)]
  let s = str::from_utf8(&s[pos..]).unwrap();
  buf.push_str(s);
  buf
}
