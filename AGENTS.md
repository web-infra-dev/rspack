...
| Command | Description |
| --- | --- |
| `pnpm run setup` | Install JS dependencies, bootstrap the workspace, and build initial Rust bindings. |
| `pnpm run build:binding:dev` | Compile napi bindings in development mode (debug symbols, no strip). |
| `pnpm run build:cli:dev` | Full dev build that covers bindings and CLI/runtime bundles. |
| `pnpm run build:js` | Build all JS packages; pair with `pnpm --filter <pkg> build` for targeted builds. |
| `pnpm run lint:js` | Run Biome lint checks across JS/TS sources. |
| `pnpm run lint:type` | Execute TypeScript/Rslint type checking. |
| `pnpm run format:js` | Format JS/TS via Prettier; `pnpm run format:rs` runs `cargo fmt`. |
| `pnpm run test:unit` | Execute JavaScript unit tests. |
| `pnpm run test:e2e` | Run scenario-driven E2E suites under `tests/e2e`. |
| `pnpm run test:webpack` | Replay the webpack compatibility corpus from `tests/rspack-test`. |
| `pnpm --filter @rspack/core dev` | Watch-mode build for `@rspack/core` (works for any package). |
| `cargo test -p <crate>` | Run Rust tests for a single crate; append `-- --nocapture` for verbose logs. |
| `cargo bench -p rspack_benchmark` | Execute Criterion benchmarks defined in `xtask/benchmark`. |
| `cargo fmt && pnpm run format:js` | Format Rust + JS sources before committing. |
| `pnpm exec rspack --version` | Quick sanity check that the CLI resolves and bindings load. |
