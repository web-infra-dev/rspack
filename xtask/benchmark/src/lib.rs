...
static GLOBAL: NeverGrowInPlaceAllocator<rspack_allocator::MiMalloc> =
  NeverGrowInPlaceAllocator::new(rspack_allocator::MiMalloc);
...
///
/// A thin wrapper around [`rspack_allocator::MiMalloc`] allocator. It passes through `alloc`
/// and `dealloc` methods to [`rspack_allocator::MiMalloc`], but does not implement
/// [`GlobalAlloc::realloc`].
///
