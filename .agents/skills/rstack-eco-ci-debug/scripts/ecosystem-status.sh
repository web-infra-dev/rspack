...
Options:
  ...
  --repo <path>       Local rstack-ecosystem-ci checkout for git refs.
                      Can also be set with RSTACK_ECOSYSTEM_CI_PATH.
  --source <ref>      JSON file path or git ref. Defaults to origin/data:<ecosystem>.json.
...
while [[ $# -gt 0 ]]; do
  case "${1}" in
    ...
    --ecosystem | --stack)
      if [[ $# -lt 2 || -z "${2:-}" ]]; then
        echo "ecosystem-status.sh: --ecosystem requires a name." >&2
        exit 1
      fi
      ecosystem="${2}"
      shift 2
    ...
    --repo | --ecosystem-ci-repo)
      if [[ $# -lt 2 || -z "${2:-}" ]]; then
        echo "ecosystem-status.sh: --repo requires a path." >&2
        exit 1
      fi
      repo_path="${2}"
      shift 2
    ...
    --source)
      if [[ $# -lt 2 || -z "${2:-}" ]]; then
        echo "ecosystem-status.sh: --source requires a JSON path or git ref." >&2
        exit 1
      fi
      if [[ "${source_set}" == true ]]; then
        echo "ecosystem-status.sh: only one source can be provided." >&2
        exit 1
      fi
      source_ref="${2}"
      source_set=true
    ...
    --)
      ...
      if [[ $# -gt 0 ]]; then
        echo "ecosystem-status.sh: unexpected extra arguments: $*" >&2
        exit 1
      fi
      ;;
    -*)
      echo "ecosystem-status.sh: unknown option ${1}." >&2
      usage >&2
      exit 1
      ;;
    *)
...
if [[ -z "${ecosystem}" ]]; then
  echo "ecosystem-status.sh: --ecosystem is required." >&2
  usage >&2
  exit 1
fi
...
if [[ "${source_set}" == false ]]; then
  source_ref="origin/data:${ecosystem}.json"
fi

if [[ ! -f "${source_ref}" && -n "${repo_path}" && ! -e "${repo_path}/.git" ]]; then
  echo "ecosystem-status.sh: --repo must point to a local rstack-ecosystem-ci checkout." >&2
  exit 1
fi

if ! command -v node >/dev/null 2>&1; then
  echo "ecosystem-status.sh requires node in PATH." >&2
  exit 1
fi

if [[ ! -f "${source_ref}" ]] && ! command -v git >/dev/null 2>&1; then
  echo "ecosystem-status.sh requires git in PATH when reading a git ref." >&2
  exit 1
fi
...
ECOSYSTEM_NAME="${ecosystem}" node -e '
const fs = require("node:fs");

const ecosystem = process.env.ECOSYSTEM_NAME;
let rows;
try {
  rows = JSON.parse(fs.readFileSync(0, "utf8"));
} catch (error) {
  console.error(`Failed to parse ${ecosystem} status JSON: ${error.message}`);
  process.exit(1);
}

if (!Array.isArray(rows) || rows.length === 0) {
  console.error(`${ecosystem} status JSON must be a non-empty array.`);
  process.exit(1);
}
