...
When macro expansion comes from an external proc macro such as `#[napi]`, review both the local usage pattern and the upstream generated shape. A valid optimization path may be a local usage change, such as directionality flags, or an upstream improvement, such as making the macro generate shared helpers, shared trampolines, smaller descriptors, or less duplicated conversion glue.
...
When a large generic group comes from an external crate or standard-library helper, do not list it as a standalone optimization target. Instead, trace why Rspack instantiates many variants and decide whether the local call boundary can be made less generic or whether the upstream abstraction should be improved. The useful conclusion should be about an Rspack-owned adapter, wrapper, callback, DTO, or option path that can use `dyn trait`, type erasure, primitive descriptors, or shared trampolines to reduce the number of external generic instantiations, or about an upstream change to a dependency such as `napi` when the repeated code is generated there.
...
Final analysis output should focus on optimization opportunities, not raw noise. Include a point only when there is a plausible Rspack-owned change that can reduce binary size without unacceptable performance or maintenance cost.

For each included opportunity, state:

- the evidence from the reports
- the likely mechanism, such as macro-generated glue, repeated conversion code, monomorphized adapters, duplicate dependency versions, or always-linked optional code
- the concrete optimization path, such as directionality flags, shared helpers, trampolines, stubs, `dyn trait`, type erasure, primitive proxy descriptors, or an upstream dependency improvement
- why the path is safe for performance, especially when it sits on a JS/N-API, config, diagnostic, or other cold boundary

Exclude signals that do not have a realistic optimization path. Examples include hot Rust-only loops that should keep static dispatch, expected standard-library `Vec`/`Option`/`Result` helpers with no local wrapper to erase, derive output that is necessary and not repeated enough to extract, duplicate dependency versions already tracked by `deny.toml` with no current unification path, and areas that have already been optimized by an existing shared helper, common object, trampoline, stub, or primitive-proxy design. Mention excluded categories only briefly when they explain why a noisy report entry is not actionable.

For external generic expansion or external proc-macro output, avoid reporting the external symbol group by itself. Convert it into an optimization question: can Rspack reduce the number of instantiations by type-erasing the boundary, sharing a trampoline, or passing primitive descriptors into a non-generic helper? If the repeated code is generated by the dependency, can the upstream implementation be changed to generate a shared helper, trampoline, or erased adapter? If both answers are no, exclude it from the optimization list.

When previous work has already extracted the common path, such as a shared runtime-module helper or the hook common-data pattern from PR #14633, do not present the same area as an open opportunity unless the new reports show remaining repeated generated code beyond that existing optimization. Treat already-optimized areas as exclusions and point to the existing mechanism briefly.
