...
For local ownership analysis, build the binding with allocative enabled. This instrumentation is disabled by default:
...
Install Inferno (`cargo install inferno --locked`), then render the snapshot from the Rspack checkout:
...
The report contains separate `memory.svg` and `counts.svg` charts, `counts.csv`, and coverage diagnostics. Counts describe current module graph entries, not all process objects. Byte weights include traversed owned heap allocations and container capacity, but exclude inaccessible third-party internals, V8 memory, and some container/allocator metadata. Check the warnings before comparing runs; the total is neither RSS nor peak memory.

Unset `ALLOCATIVE` when rebuilding normally. Setting `ALLOCATIVE=0` still enables it because the build script checks whether the environment variable is nonempty.
