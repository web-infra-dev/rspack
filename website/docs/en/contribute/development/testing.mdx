---
description: 'Because Rspack uses a mix of Rust and Node.js code, different testing strategies are used for each'
---
...
- **When filtering test cases is needed**: Add `-t`. Pattern matching supports regex, see [rstest](https://rstest.rs/guide/basic/test-filter#filter-by-test-name) for details.
...
| Default Configuration | [NormalProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/normal.ts) |
| Run Output            | `Yes`                                                                                                              |
...
| Default Configuration | [ConfigProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/config.ts) |
| Run Output            | `Yes`                                                                                                              |
...
| Default Configuration | [HotProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/hot.ts) |
| Run Output            | `Yes`                                                                                                        |
...
| Default Configuration | [WatchProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/watch.ts) |
| Run Output            | `Yes`                                                                                                            |
...
| Default Configuration | [StatsProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/stats-output.ts) |
| Run Output            | `No`                                                                                                                    |
...
| Default Configuration | [`None`](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/stats-api.ts) |
| Run Output            | `No`                                                                                                         |
...
| Default Configuration | [DiagnosticProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/diagnostic.ts) |
| Run Output            | `No`                                                                                                                       |
...
| Output Directory      | None                                                                                                           |
| Default Configuration | [HashProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/hash.ts) |
| Run Output            | No                                                                                                             |
...
| Entry File            | `Compiler.test.mjs`                                                                                        |
| --------------------- | ---------------------------------------------------------------------------------------------------------- |
| Case Directory        | [`tests/compilerCases`](https://github.com/web-infra-dev/rspack/tree/main/tests/rspack-test/compilerCases) |
| Output Directory      | None                                                                                                       |
| Default Configuration | [None](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/compiler.ts)  |
| Run Output            | No                                                                                                         |

Compiler cases support both existing single files and self-contained directories. Single-file cases use `tests/rspack-test/fixtures` as their default build context. A directory case uses its own directory and must contain `test.config.mjs`, `test.config.cjs`, or `test.config.js` (in that priority order). Only the config is discovered; sibling inputs, loaders, and helpers are not tests.

The config exports one `TCompilerCaseConfig` or an array. Ordinary cases use `options`, `compiler`, `build`, and `check` to customize the managed compiler. For GC tests or scenarios that manage their own compilers, use `run` instead:

```js title="object-lifecycle/test.config.mjs"
/** @type {import('@rspack/test-tools').TCompilerCaseConfig[]} */
export default [
  {
    name: 'hooks',
    description: 'should collect hook closures and captured objects',
    async run() {
...
`run` cannot be combined with `options`, `compiler`, `build`, `check`, `compilerCallback`, or `error`. The framework does not create a compiler or retain Stats for these cases; close owned compilers in `finally`. Both forms support `description`, `skip`, and an optional `name`. Names must be unique within the config and contain lowercase letters, digits, and single separating hyphens. Existing unnamed cases retain their index-based identifiers.

Use the full directory/scenario path to select a named case:

```bash
pnpm --dir tests/rspack-test run test -t 'compilerCases/object-lifecycle/hooks'
```

Import GC and compiler helpers from `@rspack/test-tools/helper/lifecycle`. GC scenarios run in the existing test process, which enables `--expose-gc`. For clean libuv measurements and natural-exit checks, use `runNodeCase(moduleURL, args)` from `@rspack/test-tools/helper/node-case`: the module default-exports an async function, receives JSON arguments, and returns a JSON result (or `null` for an empty return). Each child has a 20-second timeout; success requires natural process exit. See `compilerCases/libuv-handles` for examples.
...
| Output Directory      | None                                                                                                      |
| Default Configuration | [None](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/defaults.ts) |
| Run Output            | No                                                                                                        |
...
| Output Directory      | None                                                                                                             |
| Default Configuration | [ErrorProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/error.ts) |
| Run Output            | No                                                                                                               |
...
| Output Directory      | None                                                                                                           |
| Default Configuration | [HookProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/hook.ts) |
| Run Output            | No                                                                                                             |
...
| Default Configuration | [TreeShakingProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/treeshaking.ts) |
| Run Output            | No                                                                                                                           |
...
| Default Configuration | [BuiltinProcessor](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack-test-tools/src/case/builtin.ts) |
| Run Output            | No                                                                                                                   |
