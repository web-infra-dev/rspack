...
Inside the hook callback, use `module.originalSource()` to read the module's source. Modules without a source return `null`. Access the module only while the callback is active, following the [object lifetime rules](/api/javascript-api/architecture#compilation-and-object-lifetimes).
