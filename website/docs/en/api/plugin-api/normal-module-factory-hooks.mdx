---
description: 'NormalModuleFactory is used by the Compiler to generate modules (NormalModule)'
---
...
Called when resolving a module request fails, for example when the requested module cannot be found. This hook allows recovering from the failure: a tap may modify `resolveData` (such as rewriting `resolveData.request`) and return `{ retry: true }` to retry the resolution once with the modified data.

- If no tap returns `{ retry: true }`, the original resolution error is reported as usual.
- The resolution is retried at most once. If the retried resolution also fails, the new error is reported and `resolveError` is not called again.
- This hook is only called for module-not-found failures of ordinary path resolution. It is not called for other resolver errors (such as invalid `package.json` or package `exports` violations), module specifiers with a scheme (URI), loader resolution failures, or other kinds of errors.

The `error` argument contains the resolution error, including its `message`.
  ...
  (compilation, { normalModuleFactory }) => {
    normalModuleFactory.hooks.resolveError.tap(
      ...
      (resolveData, error) => {
        ...
        // redirect the failed request to a fallback module and retry
