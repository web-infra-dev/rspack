---
description: 'Compiler hooks allow Rspack plugins to intervene at specific stages of the build process'
---
...
This hook is only triggered when calling [compiler.run()](/api/javascript-api/#compilerrun) (which is used by the `rspack build` command), and will not be executed in watch mode. You can use the [watchRun](#watchrun) hook in watch mode.
...
This hook is only triggered when calling [compiler.watch()](/api/javascript-api/#compilerwatch), and will not be called in non-watch mode. You can use the [run](#run) or [beforeRun](#beforerun) hook in non-watch mode.
...
Called after the compilation has been finished and sealed, and before Rspack
decides whether to emit the assets ([`shouldEmit`](#shouldemit)).

At this point the assets have been generated and are available on
`compilation.assets`, but nothing has been written to the output directory yet,
so a plugin can still fail the build here without leaving stale files behind.
...
Called before emitting assets. Return `false` to skip emitting assets, or `true` to emit them. Either value stops subsequent taps from running. Return `undefined` or omit the return value to continue to the next tap. If no tap returns a boolean, Rspack emits the assets.
...
compiler.hooks.shouldEmit.tap('MyPlugin', (compilation) => {
  // Skip emitting assets if the compilation has errors.
