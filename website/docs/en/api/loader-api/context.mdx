...
Here, `./some-file.js` is the value passed to the `file` field. If this field is not provided, the current module's identifier will be used as the error output marker, which in this example is `./loader.js!`.
