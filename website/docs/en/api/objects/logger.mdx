...
Rspack logger is available to [loaders](/guide/features/loader) and [plugins](/guide/features/plugin). Emitting as part of the [Stats](/api/objects/stats) and configured by the user in [rspack configuration](/config/).
...
1. [compilation.getLogger](/api/objects/compilation#getlogger): the content will be stored within the stats, use this when logging is related to the compilation.
