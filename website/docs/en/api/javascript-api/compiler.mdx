...
- **Type:** `{ platforms?: string[]; esVersion?: number }`
...
- `platforms`: An array of platform strings with versions (e.g., `['chrome 87', 'firefox 78']` or `['node 18']`).
...
export default function myLoader(source) {
  ...
  const targets = options.targets ?? this._compiler.target.platforms;
