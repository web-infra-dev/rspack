...
- Direct use of this API requires manual configuration of `compiler.modifiedFiles` and `compiler.removedFiles`, otherwise the memory cache will not be invalidated properly.
...
Files modified when triggering compilation, Rspack will use it to invalidate some caches.

### removedFiles

- **Type:** `Set<string>`

Files removed when triggering compilation, Rspack will use it to invalidate some caches.
