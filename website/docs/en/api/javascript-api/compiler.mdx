...
1. This API only supports one compilation at a time. Please call `compiler.close` in the `compiler.run` callback and wait for it to finish before executing `compiler.run` again. Concurrent compilations will damage the output files.
2. Rspack cache invalidation detection relies on `modifiedFiles` and `removedFiles`. When cache is enabled, they need to be passed to Rspack via the options parameter.
