---
description: '@rspack/browser is a version of Rspack specifically designed for browser environments, without relying on WebContainers or any particular platform'
---
...
Run `@rspack/browser` inside a dedicated Web Worker, because `Atomics.wait` is not allowed on the browser main thread.

Except for the browser-specific file system and Worker setup, the rest of the API is consistent with the [JavaScript API](/api/javascript-api/) of `@rspack/core`.

```ts title="main.ts"
const worker = new Worker(new URL('./rspack.worker.ts', import.meta.url), {
  type: 'module',
});
...
```ts title="rspack.worker.ts"
import { rspack, builtinMemFs } from '@rspack/browser';

builtinMemFs.volume.fromJSON({
  '/src/index.js': 'console.log("hello from rspack")',
});

rspack({ entry: '/src/index.js' }, (err, stats) => {
  self.postMessage({
    type: 'done',
    output: builtinMemFs.volume.readFileSync('/dist/main.js', 'utf-8'),
  });
});
```
...
`@rspack/browser` supports Rspack's [ModuleFederationPlugin](/plugins/module-federation-plugin). With this feature, you can pre-bundle complex dependency modules as provider projects deployed to CDN, and then use them in consumer projects that are bundled online in the browser.
...
3. All [ModuleFederationPlugin.shared](/plugins/module-federation-plugin#shared) configurations must set `import: false`, meaning that consumer projects bundled online cannot bundle shared modules themselves; shared modules must be provided by pre-bundled provider projects.
4. [ModuleFederationPlugin.implementation](/plugins/module-federation-plugin#implementation) option is not supported.
