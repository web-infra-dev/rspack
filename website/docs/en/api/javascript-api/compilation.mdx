---
description: 'The Compilation object is one of the core objects used in the Rspack build process'
---
...
import FileSystemDependenciesType from '../../types/file-system-dependencies.mdx';
...
The following code will add a new asset named `asset-name.js` and will not be compressed because `minimized` is set to `true`:
...
The following code replaces the content of `main.js`:

```js
compiler.hooks.thisCompilation.tap('MyPlugin', (compilation) => {
  const { RawSource } = compiler.rspack.sources;
  compilation.hooks.processAssets.tap('MyPlugin', () => {
    const updatedSource = new RawSource(`console.log("Updated");`);
...
<Collapse>
  <CollapsePanel
    className="collapse-code-panel"
    header="FileSystemDependencies.ts"
    key="FileSystemDependencies"
  >
    <FileSystemDependenciesType />
  </CollapsePanel>
</Collapse>
...
<Attribution url="https://webpack.js.org/api/compilation-object/" />
