...
## javascript minify and transform

Rspack supports JavaScript minify and transform. You can use Rspack's JavaScript API to compile and transform JavaScript code. Here's a simple example demonstrating how to transform JavaScript code using Rspack:
...
Rspack uses SWC under the hood to transform and minify JavaScript code. The options accepted by these APIs are passed to SWC. You can learn more about the configuration options in the SWC official documentation:
...
declare namespace rspack {
  namespace experimental {
    declare const swc: {
      ...
      minify(code: string, options?: MinifyOptions): Promise<TransformOutput>;
    };
