...
`Compiler.compilation` uses a compiler-local ownership cell containing either `Arc<Compilation>` or `UniqueArc<Compilation>`. Hooks and passes keep their existing `&Compilation` and `&mut Compilation` parameters. The cell is not shared with worker threads; scheduling boundaries publish owned Arc readers and recover unique ownership before mutation.

The compiler's emit scheduler performs this handoff between `emit` and `afterEmit`: workers own Arc readers while writing assets and running `assetEmitted`; once all workers finish, the scheduler recovers UniqueArc before propagating errors or calling `afterEmit`. A retained reader prevents recovery and returns an error. Build/rebuild preserves the allocation by replacing its contents. Weak references can upgrade only in a shared phase and refer to the current compilation, not a historical snapshot. Other scoped tasks retain their existing borrowed access. JavaScript native-backed handles have not been migrated to Weak and still follow the lifetime rules above.
