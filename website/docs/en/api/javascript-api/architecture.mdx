...
A `Chunk` handle belongs to the `Compilation` that supplied it. Passing a Chunk from another compiler or an earlier build to `compilation.chunks.has()` returns `false`; passing it to a ChunkGraph method throws an error. Internal reuse during an incremental build does not extend the lifetime of the earlier JavaScript handle.
