...
The native loader runner keeps its `LoaderContext` in a `Box` and passes content and mappings together as a `BoxSource`. Consumers that require owned text use `Source::into_source_value`: uniquely owned raw and mapped leaves move their backing allocation, while shared sources copy only when necessary. This avoids borrowing a buffer and then copying it to construct another source. Builtin loaders can preserve source graphs: for example, a refresh loader appends runtime code with `ConcatSource`, and lightningcss returns a `SourceMapSource` without first flattening its map.

When execution reaches JavaScript, a native `JsLoaderContext` class directly owns an `Option<Box<LoaderContext>>` for that invocation. Its getters materialize content and source maps only when the JavaScript runner reads them.

The JavaScript loader-context wrapper reads one plain `JsLoaderContextState` object (`#[napi(object)]`), accesses its mutable fields through JavaScript getters and setters, and passes the same state object back in one call before returning. Loader metadata is read once when the wrapper is created; per-loader data and execution flags travel in the state object. Reused loader objects read the latest state after each native loader. The state carries an optional output only when JavaScript produces one. A pitch that produces no output leaves the native source graph intact.
...
```text
Native loader: BoxSource
  ↓ move boxed LoaderContext
JsLoaderContext: state snapshot + lazy source getters
  ↓
JavaScript loader-context wrapper: getters/setters access state
  ↓ pass the whole state object back
Native runner: recover boxed LoaderContext and continue
```

JavaScript returns the same class instance on success or with a captured error. The return-value conversion takes its boxed context on the JavaScript thread before moving it back to the native runner. The instance is left with `None`, revoking further access without a shared ownership slot or per-access locking. This is an internal bridge; loaders continue to use the webpack-compatible `this` context, string/Buffer content, source-map objects, and callbacks.
