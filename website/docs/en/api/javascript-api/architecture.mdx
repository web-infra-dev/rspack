...
For example, two JavaScript loaders on either side of a builtin loader can each set `this.data` during pitch. Each loader reads its own data during its normal stage, while content passes from one loader to the next. Pitch data travels back in `loaderData`, dependency changes in `state`, and the loader's request and type remain metadata. Content and source maps keep their existing Buffer-based transfer representation; this separation does not change when loader hooks run or how the native context is owned.
