...
When execution reaches JavaScript, a native `JsLoaderContext` class directly owns an `Option<Box<LoaderContext>>` for that invocation. Its getters materialize content and source maps only when the JavaScript runner reads them. The JavaScript wrapper caches each getter result, including missing values, for that entry. A new wrapper on the next entry reads fresh native values; output produced in JavaScript takes precedence over cached input.
...
```text
Native loader: String/Buffer + separate source map
  ↓ move boxed LoaderContext
JsLoaderContext: state snapshot + lazy content/source-map getters
  ↓
