...
Normally, unknown `import.meta` metadata is replaced with `undefined` by default. When using [ESM output](/guide/features/esm), [`module.parser.javascript.importMeta`](/config/module-parser-javascript#moduleparserjavascriptimportmeta) defaults to `'preserve-unknown'`, so unknown metadata is preserved for runtime evaluation.
...
:::warning
`import.meta.resolve()` is currently experimental. To use it, you must enable [`module.parser.javascript.importMetaResolve`](/config/module-parser-javascript#moduleparserjavascriptimportmetaresolve). Rspack also plans to [align this API with the standard behavior](https://github.com/openjs-foundation/bundler-collab-space/issues/8) in the future, so its behavior may change. Use it with caution.
:::
...
`import.meta.webpackContext` is the ESM equivalent of [`require.context()`](/api/runtime-api/module-methods#requirecontext). It allows you to dynamically import a set of modules.
