...
Normally, unknown `import.meta` metadata is replaced with `undefined` by default. When using [ESM output](/guide/features/esm), [`module.parser.javascript.importMeta`](/config/module#moduleparserjavascriptimportmeta) defaults to `'preserve-unknown'`, so unknown metadata is preserved for runtime evaluation.
