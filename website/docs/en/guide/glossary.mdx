...
In Rspack, first-class module types refer to module types that can be supported without relying on loaders or plugins, such as CSS, JSON, Assets, etc. However, module types that require loaders or plugins to support, such as JavaScript, HTML, Markdown, YAML, etc., are not first-class citizen modules.
