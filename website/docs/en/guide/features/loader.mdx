...
A loader performs pre-processing of modules, transforming them into Rspack supported types, such as using [sass-loader](https://github.com/webpack-contrib/sass-loader) to convert Sass files to CSS, or using [builtin:swc-loader](/guide/features/builtin-swc-loader) to convert JSX files to JavaScript. Whereas the configuration [Rule.type](/config/module#ruletype) specifies how to post-process modules that are already built-in supported by Rspack.
