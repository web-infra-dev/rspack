...
[less-loader](https://github.com/webpack-contrib/less-loader) can transform Less files to Rspack-supported CSS Modules types, so you can set the type to `'css'` to instruct Rspack to use the CSS handling method that is natively supported for post-processing.
