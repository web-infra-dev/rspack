...
- You need to exclude the `core-js` package via [exclude](/config/module#ruleexclude), because if `core-js` is compiled by SWC, it will not work properly.
- If you need to reference non-ES modules, you need to add [isModule: 'unknown'](https://swc.rs/docs/configuration/compilation#ismodule) to allow SWC to recognize the module type.
