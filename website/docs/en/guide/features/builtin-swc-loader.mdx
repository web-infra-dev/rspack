...
### Auto Detect Syntax transpilation

By setting `autoParser: true` , `builtin:swc-loader` will automatically configure `jsc.parser.syntax` based on file extensions:

- For `.ts` and `.tsx` files (case-insensitive), TypeScript transpilation will be automatically enabled
- For `.jsx` and `.tsx` files (case-insensitive), JSX transpilation will be automatically enabled
