...
1. [output.module](/config/output#outputmodule): Set to `true` to generate ESM format output. When this option is enabled, it affects the following configurations:
   ...
   3. Affects [output.chunkFilename](/config/output#outputchunkfilename) default value indirectly: It is derived from `output.filename`, so the default is `'[name].mjs'` with ESM output. A custom `output.filename` can change this; when it is a function, `output.chunkFilename` defaults to `'[id].js'`.
