...
Unlike application-oriented `output.module: true`, `modern-module` switches Rspack onto a dedicated ESM library rendering path. It automatically enables `output.module` and moves chunk loading to `import`, so you usually do not need to manually set `chunkFormat: 'module'` or `chunkLoading: 'import'`. If your library also needs ESM worker chunk loading, configure `workerChunkLoading: 'import'` explicitly. `modern-module` uses its own linking and scope-hoisting pipeline instead of the regular module-concatenation plugin. However, [`optimization.concatenateModules`](/config/optimization#optimizationconcatenatemodules) still controls whether that pipeline can hoist CommonJS modules with statically analyzable exports: set it to `false` or `{ commonjs: false }` to disable CommonJS hoisting.
...
1. [output.module](/config/output#outputmodule): Set to `true` to generate ESM format output. When this option is enabled, it affects the following configurations:
   ...
   9. Affects [output.environment.dynamicImport](/config/output#outputenvironment) default value: Enabled when `output.module` is `true`
   10. Affects [output.environment.dynamicImportInWorker](/config/output#outputenvironment) default value: Enabled when `output.module` is `true`
   11. Affects [output.environment.module](/config/output#outputenvironment) default value: Enabled when `output.module` is `true`
...
6. [optimization.concatenateModules](/config/optimization#optimizationconcatenatemodules): For `modern-module`, controls whether CommonJS modules with statically analyzable exports can participate in its dedicated scope-hoisting pipeline. Set it to `false` or `{ commonjs: false }` to disable CommonJS hoisting.
