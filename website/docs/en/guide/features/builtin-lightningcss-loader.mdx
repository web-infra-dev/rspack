...
The `builtin:lightningcss-loader` uses the builtin `lightningcss` from Rspack to process `css`, which can replace the syntax lowering functionality in `postcss-loader` to make the build faster.
...
You can use the `lightningcss.LoaderOptions` type exported by `@rspack/core` to enable type hints:
