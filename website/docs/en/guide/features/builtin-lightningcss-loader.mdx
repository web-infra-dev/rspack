...
::: warning
Please note that Lightning CSS strictly requires standards-compliant CSS input. Using `builtin:lightningcss-loader` to process non-standard CSS may result in styles being ignored or unexpected output (Undefined Behavior). Please ensure your CSS input conforms to the specification.
:::
