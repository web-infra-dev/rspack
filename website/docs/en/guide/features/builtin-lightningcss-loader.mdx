...
targets`supports multiple formats, it can be set as a`browserslist`query string, a`browserslist`query result, or the`targets` of lightningcss.
...
- Setting as lightningcss's `targets`, which relies on lightningcss's internal `browserslistToTargets` method to convert a `browserslist` query result to lightningcss's `targets` format:
