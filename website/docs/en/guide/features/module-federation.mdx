...
In Rspack 2.0, `ModuleFederationPlugin` also further improves shared dependency Tree Shaking. You can use `shared.treeShaking` to optimize shared dependencies based on actual usage. For related options, see [ModuleFederationPlugin](/plugins/webpack/module-federation-plugin#shared).
