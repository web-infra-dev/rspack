---
description: 'Module Federation is an architectural pattern for JavaScript application decomposition (similar to microservices on the server-side), allowing you to share...'
---
...
In Rspack 2.0, `ModuleFederationPlugin` also further improves shared dependency Tree Shaking. You can use `shared.treeShaking` to optimize shared dependencies based on actual usage. For related options, see [ModuleFederationPlugin](/plugins/module-federation-plugin#shared).

You can use it through Rspack's [ModuleFederationPlugin](/plugins/module-federation-plugin) without installing any additional plugins.
...
You can use it through Rspack's [ModuleFederationPluginV1](/plugins/module-federation-plugin-v1).
