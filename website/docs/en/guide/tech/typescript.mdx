---
description: 'Use TypeScript with Rspack, covering setup, configuration, integration details, and framework-specific development patterns.'
---
...
It's possible to use the type of Rspack specific features in your TypeScript code, such as [import.meta.glob](/api/runtime-api/module-variables#importmetaglob).
...
1. Add the TypeScript reference directive to declare:
   ...
   ```ts title="src/index.ts"
   const modules = import.meta.glob('./dir/*.js'); // without reference declared above, TypeScript will throw an error
   ```
