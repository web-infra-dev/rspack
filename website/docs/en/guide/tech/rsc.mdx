---
description: 'Use React Server Components with Rspack, covering setup, configuration, integration details, and framework-specific development patterns.'
---
...
Add `builtin:swc-loader` for `.js`, `.mjs`, `.jsx`, `.ts`, and `.tsx` files and enable `rspackExperiments.reactServerComponents` so Rspack parses and transforms RSC directives such as `"use client"`, `"use server"`, and `"use server-entry"`.
