...
The [rspack-rsc-examples](https://github.com/SyMind/rspack-rsc-examples) repository contains full examples of building React Server Component applications using Rspack.
...
Add `builtin:swc-loader` to process `.jsx` and `.tsx` files. The `rspackExperiments.reactServerComponent`s option must be enabled to turn on Rspack's support for RSC syntax.
...
In the Server Compiler, Rspack uses [layer](/config/experiments#experimentslayers) to identify modules that belong to the Server Component (RSC) environment.
