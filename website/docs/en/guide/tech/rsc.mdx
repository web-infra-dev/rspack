...
## Module Federation with RSC

Rspack supports first-class Module Federation + RSC integration behind `ModuleFederationPlugin.experiments.rsc`.

Server-side requirements:

- `target` must include `async-node`
- `ModuleFederationPlugin.experiments.asyncStartup` must be `true`
- `ModuleFederationPlugin.runtimePlugins` must include `@module-federation/node/runtimePlugin`
...
Host/remote demo examples are available in the Rspack repo:

- [examples/rsc-mf-host](https://github.com/web-infra-dev/rspack/tree/main/examples/rsc-mf-host)
- [examples/rsc-mf-remote](https://github.com/web-infra-dev/rspack/tree/main/examples/rsc-mf-remote)
