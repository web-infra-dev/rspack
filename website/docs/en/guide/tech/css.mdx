...
By default, built-in CSS support uses the `link` export type, which emits CSS as stylesheet output. You can configure `module.parser.css.exportType` or the parser options for `css/auto`, `css/global`, and `css/module` to export CSS through JavaScript instead.
