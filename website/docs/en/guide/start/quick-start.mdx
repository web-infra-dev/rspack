...
- [Rspack StackBlitz Example](https://stackblitz.com/~/github.com/rspack-contrib/rspack-stackblitz-example)
...
## Preview projects online with StackBlitz

Rspack >= 1.4 includes a Wasm binding, which makes Rspack available in StackBlitz(WebContainers). Currently, if you want to preview your Rspack projects on StackBlitz, you need to specify the CPU architecture when installing dependencies. Different package managers have different ways to do it.
...
In the future, we will optimize the workflow to enable users to preview Rspack projects seamlessly. Additionally, we are planing to implement browser-based version of Rspack.
