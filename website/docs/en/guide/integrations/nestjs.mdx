...
Start with the common [Node.js application configuration](/guide/integrations/node), then add the NestJS settings described below. You can also use the [NestJS example](https://github.com/rstackjs/rstack-examples/tree/main/rspack/nestjs) in `rstack-examples` as a reference. It includes a complete `rspack.config.mjs`, development script, production build script, and HMR entry.
...
Install the common Node.js build dependencies from the [Node.js guide](/guide/integrations/node#install-dependencies), then install the NestJS runtime packages:

<PackageManagerTabs command="add @nestjs/common @nestjs/core @nestjs/platform-express reflect-metadata rxjs" />

## Basic concepts

NestJS applications run in Node.js, so most configurations can follow the common [Node.js configuration](/guide/integrations/node). The main NestJS-specific requirement is decorator metadata, which NestJS uses at runtime.
...
See [Node.js - Configure development HMR](/guide/integrations/node#configure-development-hmr) for the shared Rspack entry, `writeToDisk`, `RunScriptWebpackPlugin`, and externals allowlist settings used with this bootstrap code. If your project needs ESM output, see [Node.js - Use ESM output](/guide/integrations/node#use-esm-output).
...
If a NestJS dependency includes a Node.js native addon (`.node` module), follow [Node.js - Native node modules](/guide/integrations/node#native-node-modules) to emit the addon with `asset/resource` and let Node.js load it at runtime.
