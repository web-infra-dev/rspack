...
`sideEffectsFree` allows you to manually specify function names as side-effect-free in specific modules via the Rspack configuration file. This is useful for third-party libraries where source code cannot be modified (e.g., packages in `node_modules`).

:::note
Works only for function declarations at top level scope
:::
...
If a function name specified in the `sideEffectsFree` configuration is not found in the target module, Rspack will emit a warning.
