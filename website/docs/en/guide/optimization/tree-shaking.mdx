...
This feature is currently experimental and controlled by `experiments.pureFunctions`, which is enabled by default. It also depends on [optimization.sideEffects](/config/optimization#optimizationsideeffects) (`sideEffects` is enabled by default in production mode):
...
:::warning
This option is experimental and only takes effect when `experiments.pureFunctions` is enabled, which is the default.
:::
