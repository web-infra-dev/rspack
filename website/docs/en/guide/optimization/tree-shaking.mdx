...
This feature is currently experimental and needs to be enabled via configuration, and it depends on [optimization.sideEffects](/config/optimization#optimizationsideeffects) (`sideEffects` is enabled by default in production mode):
...
`pureFunctions` allows you to manually specify function names as side-effect-free in specific modules via the Rspack configuration file. This is useful for third-party libraries where source code cannot be modified (e.g., packages in `node_modules`).
    ...
    rules: [
      {
        ...
        parser: {
          pureFunctions: ['isString', 'isNumber'],
        },
      },
    ],
  },
...
If any function name is not found in the pureFunctions configuration, Rspack will give you a warning.
