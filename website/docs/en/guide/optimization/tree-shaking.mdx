---
description: 'Rspack supports tree shaking, a term commonly used in the JavaScript ecosystem for removing unused code, also known as "dead code"'
---
...
:::tip Check the code before bundling

If unused exports remain, inspect the JavaScript emitted by your loaders or other compilers, not just the original source. For example, `export const unused = 1` may have become `exports.unused = 1` before Rspack receives it. Preserve ES module syntax at that earlier step before investigating the tree shaking options. Changing the final bundle's output format does not change what the loader passes into Rspack.

For background on why module format affects static analysis, see this [ESM and CommonJS comparison](https://frontendatlas.com/javascript/trivia/js-esm-vs-cjs).
...
- [innerGraph](/config/optimization#optimizationinnergraph): Tracks variable usage to more accurately determine if exports are actually used.
...
`/*#__NO_SIDE_EFFECTS__*/` is a feature that follows the community [JavaScript Compiler Hints Spec](https://github.com/javascript-compiler-hints/compiler-notations-spec/blob/main/no-side-effects-notation-spec.md). Unlike `/*#__PURE__*/` which marks call sites, `/*#__NO_SIDE_EFFECTS__*/` is used to mark **function definitions**, declaring that the function has no side effects when called with arguments.

### Configuration

This feature is currently experimental and controlled by `experiments.pureFunctions`, which is enabled by default in production mode. It also depends on [optimization.sideEffects](/config/optimization#optimizationsideeffects) being `true` (`sideEffects` is `true` by default in production mode):
...
For configuration details, see [`experiments.pureFunctions`](/config/experiments#experimentspurefunctions) and [`module.parser.javascript.pureFunctions`](/config/module-parser#javascriptpurefunctions).

:::warning
This option is experimental and only takes effect when `experiments.pureFunctions` is enabled, which is the default in production mode.
:::
