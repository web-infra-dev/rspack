...
`pureFunctions` lets you manually mark top-level exported functions in matched modules as side-effect-free for advanced tree shaking. This is useful for third-party libraries where source code cannot be modified, such as packages in `node_modules`.
