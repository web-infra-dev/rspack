...
If `shared.js` still appears duplicated after lowering `minSize`, check `splitChunks.minSizeReduction`, `splitChunks.minChunks`, and request limits such as `maxAsyncRequests` / `maxInitialRequests` before reaching for `enforce`.
