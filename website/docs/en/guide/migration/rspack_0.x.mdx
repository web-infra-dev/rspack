---
description: 'The document lists all breaking changes from Rspack 0.7 to 1.0'
---
...
The default value of [splitChunks.cacheGroups.\{cacheGroup\}.reuseExistingChunk](/plugins/split-chunks-plugin#splitchunkscachegroupscachegroupreuseexistingchunk) has been changed from `true` to `false`.
...
Now, we have removed it and replaced it with [rspack.LightningCssMinimizerRspackPlugin](/plugins/lightning-css-minimizer-rspack-plugin) to handle the same functionality.
