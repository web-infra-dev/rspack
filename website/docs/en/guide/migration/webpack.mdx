...
-   "serve": "webpack serve",
-   "build": "webpack build",
+   "serve": "rspack serve -c webpack.config.js",
+   "build": "rspack build -c webpack.config.js",
  }
}
```

:::tip
Rspack commands are similar to webpack commands and can specify the configuration file with `-c` or `--config`.
Unlike webpack, if you do not explicitly specify a configuration file, Rspack defaults to using `rspack.config.js`.
:::

## Updating configuration

Rspack does not currently support all webpack configurations, and some configurations might affect the build output.
To ensure the correctness of the build output, Rspack enables strict configuration validation by default.
However, Rspack also offers lenient modes to facilitate gradual migration. You can enable these modes by setting the `RSPACK_CONFIG_VALIDATE` environment variable:
...
## Migrating webpack built-in plugins
...
For example, replacing the [HotModuleReplacementPlugin](/plugins/webpack/hot-module-replacement-plugin):
...
Using `builtin:swc-loader` offers better performance compared to the external `swc-loader`, as it avoids frequent communication between JavaScript and Rust.
