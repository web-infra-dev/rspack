...
This document lists all breaking changes from Rspack v1.x to v2.0. You can refer to this document to migrate your project.

> See [Breaking changes in Rspack v2.0](https://github.com/web-infra-dev/rspack/discussions/9270) for more details.

## Upgrade Node.js

Rspack 2.0 has upgraded the minimum supported Node.js version from 18.x to **20.19.0** to natively support loading ESM modules via `require()` at runtime.

Meanwhile, `@rspack/core` and `@rspack/cli` are now published as pure ESM packages, and their CommonJS build artifacts have been removed.

## Dependency Updates

`@rspack/dev-server` is now an optional dependency. You need to manually install it when using `rspack serve` or `rspack preview` commands.
