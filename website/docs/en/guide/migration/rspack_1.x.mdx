...
### Moved builtin:swc-loader's rspackExperiments.import to top-level transformImport

The `rspackExperiments.import` option (also known as `rspackExperiments.pluginImport`) of `builtin:swc-loader` has been moved to a top-level [transformImport](/guide/features/builtin-swc-loader#transformimport) option. The old `rspackExperiments.import` is deprecated but still works for backward compatibility.
...
### JavaScript `resolve.extensions` no longer includes `.wasm` by default

Rspack 2.0 no longer includes `.wasm` in the default `resolve.extensions` used for JavaScript requests.
