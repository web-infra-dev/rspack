...
[@rspack/dev-server](https://npmjs.com/package/@rspack/dev-server) 2.0 has been released, with the following changes:
...
- Some [devServer](/config/dev-server) options have changed, including `devServer.proxy` and `devServer.watchFiles`. See the [upgrade guide](https://github.com/rstackjs/rspack-dev-server/blob/main/docs/migrate-v1-to-v2.md) for more details.
...
The built-in webpack-bundler-analyzer has been removed, and the `--analyze` flag is no longer available.

If you need to analyze bundle outputs, you can use [Rsdoctor](/guide/optimization/use-rsdoctor), which provides more powerful analysis capabilities.
...
The default value of [experiments.asyncWebAssembly](/config/experiments#experimentsasyncwebassembly) has been changed from `false` to `true`. Async WebAssembly support is now enabled by default, and Rspack automatically injects built-in `type: 'webassembly/async'` rules for common `.wasm` files and `application/wasm` requests, so you usually don't need to add a manual `module.rules` entry.
...
In Rspack 2.0, `builtin:swc-loader` no longer supports reading `.swcrc` files. You need to migrate your SWC configuration to the loader options in `rspack.config.js`:
...
The `optimization.removeAvailableModules` configuration has been removed. This option had no actual effect in Rspack and can be safely removed from your configuration:
...
`experiments.SubResourceIntegrityPlugin` has been moved to the top-level [SubresourceIntegrityPlugin](/plugins/rspack/subresource-integrity-plugin):
...
Rspack 2.0 no longer includes `.wasm` in the default [resolve.extensions](/config/resolve#resolveextensions) used for JavaScript requests.

In practice, this means extension-less requests such as `import './module'` or `require('./module')` will no longer try `./module.wasm` by default.

Explicit imports like `import './module.wasm'` are not affected.

If you need to preserve the previous behavior, add `.wasm` back to the corresponding `resolve.byDependency` entries:
...
Rspack 2.0 no longer includes `webpack` in the default CSS `@import` resolve conditions.

If you need to preserve the previous behavior, add `webpack` back explicitly:
...
If you need source maps in production, we recommend setting `devtool` to `source-map` or `hidden-source-map`:
...
The resolve's [roots](/config/resolve#resolveroots) configuration default value has been changed from `[ context ]` to `[]`.

If you rely on server-relative URL resolution that expects paths to resolve relative to the `roots` directories, explicitly configure the `roots` option:
...
The `@module-federation/runtime-tools` package has been changed from `dependencies` to optional `peerDependencies` (declared via `peerDependencies` + `peerDependenciesMeta.optional`). If you are using [ModuleFederationPlugin](/plugins/webpack/module-federation-plugin), ensure that this package is explicitly installed in your project.
