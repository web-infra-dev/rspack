...
If you need to analyze bundle outputs, it is recommended to use [Rsdoctor](/guide/diagnostics/use-rsdoctor), which provides more powerful analysis capabilities.

## Module compilation changes

### Changed default value of module.parser.javascript.exportsPresence

The default value of [module.parser.javascript.exportsPresence](/config/module-parser#javascriptexportspresence) has been changed from `warn` to `error`. When detecting non-existent exports, an error will now be thrown directly instead of just a warning.
...
`module.parser.javascript.strictExportPresence` has been removed. You can use [module.parser.javascript.exportsPresence](/config/module-parser#javascriptexportspresence) to control the behavior when exports don't exist.

### Changed default value of module.parser.javascript.requireAlias

The default value of [module.parser.javascript.requireAlias](/config/module-parser#javascriptrequirealias) changed from `true` to `false`. If your code relies on renamed `require` calls being parsed and bundled, explicitly enable it:
...
- Use [module.parser.javascript.pureFunctions](/config/module-parser#javascriptpurefunctions) to manually mark top-level exported functions in matched third-party modules such as packages in `node_modules`.
...
The top-level `profile` configuration and `stats.profile` configuration have been removed. Use [Rsdoctor](/guide/diagnostics/use-rsdoctor) for performance analysis:
...
`WarnCaseSensitiveModulesPlugin` has been renamed to [CaseSensitivePlugin](/plugins/case-sensitive-plugin), which covers all of its functionality.
...
The `sri` configuration of [HtmlRspackPlugin](/plugins/html-rspack-plugin) has been removed. If you need SRI functionality, use the [SubresourceIntegrityPlugin](/plugins/subresource-integrity-plugin) plugin.
...
The following [LightningCssMinimizerRspackPlugin](/plugins/lightning-css-minimizer-rspack-plugin) options have changed:
...
The `@module-federation/runtime-tools` package has changed from `dependencies` to optional `peerDependencies` (declared via `peerDependencies` + `peerDependenciesMeta.optional`). If you are using [ModuleFederationPlugin](/plugins/module-federation-plugin), ensure that this package is explicitly installed in your project.
...
> For more details, refer to [CSS](/guide/languages/css)
...
- `experiments.lazyCompilationMiddleware`: moved to the top-level [lazyCompilationMiddleware](/guide/advanced/lazy-compilation#integrating-with-custom-server).
- `experiments.lazyBarrel`: removed; the [Lazy barrel](/guide/advanced/lazy-barrel) optimization is now enabled by default.
- `experiments.typeReexportsPresence`: removed; use [module.parser.javascript.typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) directly to control the behavior when re-exporting types.
...
- `experiments.SubResourceIntegrityPlugin`: moved to the top-level [SubresourceIntegrityPlugin](/plugins/subresource-integrity-plugin).
