...
The `rspackExperiments.import` option of `builtin:swc-loader` has been moved to a top-level [transformImport](/guide/features/builtin-swc-loader#transformimport) option. The old `rspackExperiments.import` is deprecated but still works for backward compatibility.
