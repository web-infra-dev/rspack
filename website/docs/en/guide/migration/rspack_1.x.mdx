...
The default value of [module.parser.javascript.exportsPresence](/config/module-parser-javascript#moduleparserjavascriptexportspresence) has been changed from `warn` to `error`. When detecting non-existent exports, an error will now be thrown directly instead of just a warning.
...
`module.parser.javascript.strictExportPresence` has been removed. You can use [module.parser.javascript.exportsPresence](/config/module-parser-javascript#moduleparserjavascriptexportspresence) to control the behavior when exports don't exist.

### Changed default value of module.parser.javascript.requireAlias

The default value of [module.parser.javascript.requireAlias](/config/module-parser-javascript#moduleparserjavascriptrequirealias) changed from `true` to `false`. If your code relies on renamed `require` calls being parsed and bundled, explicitly enable it:
...
- Use [module.parser.javascript.pureFunctions](/config/module-parser-javascript#moduleparserjavascriptpurefunctions) to manually mark top-level exported functions in matched third-party modules such as packages in `node_modules`.
...
- `experiments.typeReexportsPresence`: removed; use [module.parser.javascript.typeReexportsPresence](/config/module-parser-javascript#moduleparserjavascripttypereexportspresence) directly to control the behavior when re-exporting types.
