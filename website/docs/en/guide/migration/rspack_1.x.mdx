...
### Remove webpack-bundler-analyzer

Starting from Rspack 2.0, the built-in webpack-bundler-analyzer integration and the `--analyze` CLI flag have been removed.
...
This only affects packages that expose different `exports` entries for `webpack` and `style`.
