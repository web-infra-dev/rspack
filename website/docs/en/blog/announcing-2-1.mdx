...
Rspack stores one persistent cache subdirectory for each compiler path. When `cache.version`, cache-related configuration, or the Rspack version changes, Rspack invalidates and overwrites that compiler's existing cache instead of retaining multiple versions.
