...
Rspack stores one persistent cache subdirectory for each compiler path. When `cache.version` changes, Rspack invalidates and overwrites that compiler's existing cache instead of retaining multiple versions.

`cache.maxVersions` remains accepted for backward compatibility, but is deprecated and has no effect. Explicitly configuring it emits a warning.

Rspack 2.1 adds an automatic cleanup mechanism for persistent cache. [`cache.maxAge`](/config/cache#maxage) controls how long a compiler cache can remain unaccessed. The default value is `7 * 24 * 60 * 60` (7 days).
...
When a compiler cache has not been accessed for longer than this limit, Rspack removes its subdirectory. Set `maxAge` to `Infinity` if cache cleanup is managed externally.
