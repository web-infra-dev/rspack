---
description: 'Rspack 2.1 is now available, featuring React Compiler Rust support, import.meta.glob, Source Phase Imports, persistent cache cleanup, and multiple performance and output optimizations.'
...
- Performance improvements
  - [React Compiler Rust version](#react-compiler)
...
- New features
  ...
  - [Support for `createRequire`](#create-require)
  - [Rspack magic comments](#magic-comments)
  - [Support for Source Phase Imports](#source-phase-imports)
  - [New persistent cache cleanup mechanism](#cache-cleanup)
- Output optimization
...
In the past, React Compiler was mainly integrated through `babel-loader`, which introduced additional Babel transform overhead and increased project build time. With [React Compiler being ported to Rust](https://github.com/facebook/react/pull/36173), SWC has also completed the integration. Rspack 2.1 can now enable React Compiler directly through the built-in SWC loader.
...
This feature is already available in Vite and Turbopack. With Rspack support, developers can use a more consistent and familiar syntax across different ecosystem tools, reducing the cognitive cost of switching between them.
It also allows framework and library authors who support multiple toolchains to reuse more similar implementations.
See the [`import.meta.glob` documentation](/api/runtime-api/module-variables#importmetaglob) for the complete usage.
...
CSS Modules capabilities also continue to improve, covering more CSS Modules features.

For detailed configuration, see the [Built-in CSS support documentation](/guide/tech/css#built-in-css-support), the [`module.generator` CSS options](/config/module-generator#cssauto), and the [`module.parser` CSS options](/config/module-parser#cssauto).
...
- **Custom sources**: besides `true`, you can customize the specifier and module source using a string in the `"<specifier> from <module>"` form, such as `"createRequire from module"`.
...
Rspack 2.1 adds the `rspack` prefix to [magic comments](/api/runtime-api/module-methods#magic-comments).
New code can prefer the `rspack*` form to pass compilation hints. The existing `webpack*` form remains compatible, so existing projects do not need to migrate.

```js
import(/* rspackChunkName: "dashboard" */ './dashboard');
```

See the [magic comments documentation](/api/runtime-api/module-methods#magic-comments) for the complete usage.
...
Unlike normal WebAssembly imports, source phase import does not instantiate the Wasm module during import. Instead, it returns a compiled [`WebAssembly.Module`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/Module). This lets you control instantiation yourself, such as instantiating the same Wasm module multiple times with different imports, or reusing the same compiled result across multiple Web Workers to avoid repeated compilation cost.
...
Rspack's persistent cache is isolated by cache version. When `cache.version`, cache-related configuration, or the Rspack version changes, Rspack creates a new cache version in the cache directory to avoid reusing incompatible cache across different configurations. However, during long-term development, frequent branch switching, or CI workflows that reuse the working directory, old cache versions may continue to accumulate and take up more disk space.
...
For `export const` in non-cyclic modules, however, the exported value will not change after module initialization, so it does not need to be read through a getter every time. In production builds, Rspack 2.1 combines this with circular module information: when Rspack confirms that the current module is not part of a circular dependency, `export const` is defined on the namespace object as a readonly value, reducing generated code and runtime getter call overhead.
...
// Rspack 2.1: const exports in non-cyclic modules are defined as readonly values.
...
We are working with the TanStack team to improve Rsbuild support in TanStack Start, and we have already made incremental progress: [TanStack Start now officially supports Rsbuild](https://tanstack.com/blog/start-adds-rsbuild-support). Developers can now use Rsbuild to build TanStack Start applications and get framework capabilities including RSC.

Our core goal for RSC is to provide general-purpose RSC build capabilities, allowing upper-level frameworks to integrate RSC based on their own routing, rendering, and server runtime solutions while reusing unified build infrastructure.
...
Rslib adds a fast type generation mode based on `isolatedDeclarations`. After enabling [dts.isolated](https://rslib.rs/config/lib/dts#dtsisolated), Rslib uses SWC's `fast_dts` capability during the Rspack build to directly emit declaration files for TypeScript modules in the dependency graph.
...
We released [rspack-merge](https://github.com/rstackjs/rspack-merge), an npm package for merging Rspack configurations. It covers needs from composing base configurations to merging loader and plugin rules.
...
`rspack-merge` is written in TypeScript, ships modern ESM output, and keeps zero runtime dependencies, making configuration merging simple, reliable, and lightweight enough.
...
Rspress now provides more installable AI skills, including:
