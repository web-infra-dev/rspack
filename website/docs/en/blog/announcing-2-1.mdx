...
For more configuration options, see the [Rspack React Compiler guide](/guide/integrations/react#react-compiler).
...
Rspack 2.1 adds [`CircularCheckRspackPlugin`](/plugins/circular-check-rspack-plugin), which replaces the deprecated [`CircularDependencyRspackPlugin`](/plugins/circular-dependency-rspack-plugin).
...
Rspack 2.1 further improves [built-in CSS support](/guide/languages/css#built-in-css-support). The new `css/global` module type allows CSS Modules to work in a "global by default, opt into `:local` when needed" mode. Together with `css/module` and `css/auto`, it covers more scope organization patterns.
