---
description: 'Rspack 1.6 has been released with better ESM output, enhanced tree shaking, support for the import defer syntax, stabilized layers feature, and default barrel file optimization.'
...
To address these issues once and for all, we introduced an experimental plugin called [EsmLibraryPlugin](https://v1.rspack.rs/plugins/rspack/esm-library-plugin), purpose-built for constructing clean and efficient ESM libraries:
...
Starting from Rspack 1.6, the layer feature has become stable enough that the experimental flag [experiments.layers](/guide/advanced/layer) has been deprecated. You can now use the layer feature directly without the experimental flag.

For more details and practical examples, check out our new [Module layers guide](/guide/advanced/layer).
