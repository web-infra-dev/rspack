...
Optimizing ESM output has long been one of the key challenges faced by Rspack. Previously, we relied on [concatenateModules](/config/optimization#optimizationconcatenatemodules) to optimize ESM outputs, but that approach had several limitations:
