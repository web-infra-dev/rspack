...
The new EsmLibraryPlugin is now available as an experimental feature, providing better and more static output for library builds. Compared to the previous ModernModule approach, the new ESM output is not limited by ConcatenatedModule and Rspack's own runtime constraints. For example, even if the Entry Module doesn't meet Scope Hoisting standards, it won't cause the output to be unusable or result in syntax errors.
...
In version 1.6.0, we deprecated the experimental switch for [experiments.layers](/config/experiments#experimentslayers), and you can now directly start using the layer feature.
...
You have multiple ways to specify layers for modules:

- Specify via [Entry](/config/entry#entrydescriptionlayer)
...
- Specify via [Rule.layer](/config/module#rulelayer)
...
You can filter specific layers through issuerLayer. In the following example, we use different downgrade strategies based on different layers of modules:
...
In subsequent stages, you can also place Modules into different Chunks based on different layers:
...
In the above example, we separated server and client code into different output directories through layers.
