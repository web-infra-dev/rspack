...
We're excited to announce Rspack 1.6!
...
The new [EsmLibraryPlugin](/plugins/rspack/esm-library-plugin) is now available as an experimental feature, specifically designed for building JavaScript libraries with cleaner and more efficient output.

**What is ESM library building?**

ESM (ES Modules) is the modern JavaScript module system that allows you to organize code using `import` and `export` statements. When you want to publish a JavaScript library for others to use, you need to bundle your source code into an appropriate format.

**Previous challenges**

ESM library building has always been a challenging area for Rspack. Previously, we used [concatenateModules](/config/optimization#optimizationconcatenatemodules) to optimize ESM output, but this approach had some issues:

- **Impure output**: The generated files contained a lot of Rspack's internal runtime code, rather than just the functionality you actually wanted to export
- **Error-prone**: In certain situations, modules couldn't be properly merged, causing unexpected errors in the final library files

**The new solution**

To completely solve these problems, we introduced the EsmLibraryPlugin. Here's how it works:

- **Full control over bundling**: Instead of relying on Rspack's default runtime, it properly connects all modules during the compilation phase
- **Better code splitting**: Output after code splitting is more clean and can be easily optimized

> Below you can see the difference of the output after code splitting between current output and EsmLibraryPlugin:
...
The EsmLibraryPlugin is still in the experimental stage, you can try it out by following the configuration below.
...
- Building 10 000 React components is **11% faster**
...
For more information, see [Rstest - Code Coverage](https://rstest.rs/config/test/coverage).
...
👉 [Playground Document](https://rsdoctor.rs/guide/start/playground)

👉 [All-in-one JSON](https://rsdoctor.rs/config/options/output#mode-brief)
