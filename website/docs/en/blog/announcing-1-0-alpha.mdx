...
Rspack 1.0 enables the `optimization.concatenateModules` by default during production builds. This enables module concatenation optimization, also known as scope hoisting.
...
With concatenateModules enabled, the size of Rspack's output is reduced by about 4% to 10% (before Gzip).
