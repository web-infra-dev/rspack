---
description: 'Rspack 1.0 alpha is now available on npm! Before releasing Rspack 1.0 stable version, we will test for 1~2 months to improve the API stability and reliability of v1.0 and to verify its impact on downstream projects.'
...
Rspack 1.0 aligns with the webpack [experiment.css](/config/deprecated-options#experimentscss) default value, making it easier to migrate from webpack to Rspack.
...
3. Use [experiment.css](/config/deprecated-options#experimentscss), an experimental feature introduced in webpack 5 that provides native CSS support.
