...
Rsbuild v0.7 has been released!

This is the last minor release before the Rsbuild v1.0. After this, the Rspack team will focus on the development of v1.0 and aim to launch the Rspack v1.0 alpha version soon.
...
Rspack v0.7 supports lazy compilation, which is very helpful for improving the build performance of multi-entry projects or large projects.
...
Mainly in the CSS dependency analysis part, the switch from `swc-css` to `css-module-lexer` not only fixes the boundary problem of CSS Modules, but also brings faster bundling speed to experiments.css.
