...
This feature is still in the experimental stage. If you encounter problems during use, please give us feedback through [GitHub Issues](https://github.com/web-infra-dev/rspack/issues).

### Faster Native CSS bundling speed
...
Mainly in the CSS dependency analysis part, the switch from `swc-css` to `css-module-lexer` not only fixes the boundary problem of css modules, but also brings faster bundling speed to experiments.css. For relevant changes, please see PR: [rspack#6594](https://github.com/web-infra-dev/rspack/pull/6594).

After actual testing, `bootstrap.css` bundling speed increased by 4 times:
...
Rspack is committed to being compatible with plugins in the webpack ecosystem to use the excellent features that have been accumulated and verified in the community.

[Plugin Compatibility List](/guide/compatibility/plugin) in continuous updates, you can view the current compatibility status of some community plugins and their alternatives through the plugin compatibility list. Currently compatible plugins include `@vanilla-extract/webpack-plugin` and `moment-locales-webpack-plugin`, while `workbox-webpack-plugin` and `webpack-manifest-plugin` provide alternatives.

In addition, [Plugin API support](/api/plugin-api/index) and plugin compatibility are a matter that needs to be continued. If you find other Rspack-incompatible plugins, please feel free to submit them through [GitHub Issues](https://github.com/web-infra-dev/rspack/issues) and we will support or provide an alternative as soon as possible.
...
0.7.0 has removed the `builtins.treeShaking` (oldTreeShaking) and `experiments.rspackFuture.newTreeshaking` (new tree shaking switch) configurations, taking the old tree shaking functionality completely offline.
