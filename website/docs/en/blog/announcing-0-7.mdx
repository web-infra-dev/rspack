---
description: 'Rspack 0.7 has been released, featuring support for lazy compilation, which can significantly improve the dev startup performance of large applications. It also introduces a brand-new css-module-lexer, increasing CSS bundling speed by 4 times.'
...
Now, you can enable the lazy compilation feature in Rspack through the [experiments.lazyCompilation](/config/lazy-compilation) configuration:
...
In v0.7, we have refactored the internal implementation of the [experiments.css](/config/deprecated-options#experimentscss).
...
In Rspack 0.7, we have partially refactored the internal implementation of [experiments.css](/config/deprecated-options#experimentscss).
...
Rsbuild v0.7 has been released with Rspack v0.7, please read [Announcing Rsbuild v0.7](https://v0.rsbuild.rs/community/releases/v0-7) to learn more.
