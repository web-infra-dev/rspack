...
In previous versions, the upgrade cost for SWC Wasm plugins was relatively high. This was because the SWC AST structure would change across versions, causing existing plugins to fail after an SWC upgrade. Plugin authors needed to adapt their plugins, and users needed to upgrade dependencies, which affected project stability and the upgrade experience.
...
In Rspack 1.7, we upgraded the used SWC version and adopted these compatibility mechanisms. From now on, in most scenarios, SWC upgrades will not break existing plugins built with older SWC versions.
...
Rsbuild now supports printing size differences to visually observe if the build output size has changed.
