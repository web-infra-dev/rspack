---
description: 'Rspack 1.3 has been released with support for detecting circular dependencies, building HTTP imports, and referencing AMD modules. It introduces a new lazy compilation middleware, while also improving code splitting performance, output bundle size, and memory usage.'
...
Rspack 1.3 introduces a built-in plugin [CircularDependencyRspackPlugin](/plugins/circular-dependency-rspack-plugin) to detect circular dependencies between runtime modules.
