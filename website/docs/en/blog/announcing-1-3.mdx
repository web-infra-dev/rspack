...
- New features
  ...
  - [`experiments.buildHttp`](#experimentsbuildhttp): downloading the HTTP/HTTPS resources to the local at build time automatically and then bundle them
...
In Rspack you can import HTTP/HTTPS resources by [externalsPresets.web](/config/externals#externalspresetsweb) or [externalsPresets.webAsync](/config/externals#externalspresetswebasync), which simply externals the HTTP/HTTPS resources and let the browser (or other platform) to fetch them at runtime.
...
2. Rolling out [mimalloc](https://github.com/microsoft/mimalloc) v3 on macOS. This mitigates some memory consumption issue on macOS during rebuilding. According to some community and internal projects, this would lift the RSS for rebuilding, based on the size of each project, varying from 10% to 85%.
