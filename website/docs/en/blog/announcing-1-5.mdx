---
description: 'Rspack 1.5 has been released, introducing barrel file optimization and constant inlining optimization, also adding a built-in file system watcher, a virtual modules plugin, and a Rust extension mechanism, while dropping support for Node.js 16.'
date: 2025-08-26 11:00:00
sidebar: false
authors:
  - name: Rspack Team
...
To address this, Rspack 1.5 introduces the experimental [lazyBarrel](/guide/optimization/lazy-barrel) feature. This optimization automatically detects side-effect free barrel files and defers building their re-exports until actually needed. Modules are only resolved and built when actually needed, significantly reducing unnecessary module resolution and build costs.
...
For more details, see the [experiments.lazyBarrel documentation](/guide/optimization/lazy-barrel).
...
> For more details, see the [experiments.inlineConst documentation](/config/optimization#optimizationinlineexports). This feature is planned to be enabled by default in v1.6.
...
> For more details, see the [experiments.inlineEnum documentation](/config/optimization#optimizationinlineexports).
...
Rspack 1.5 introduces the [experiments.typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) option, which improves the detection of TypeScript type exports. With this option enabled, Rspack can correctly recognize type re-exports across modules, preventing false warnings.

> For more details, see the [experiments.typeReexportsPresence documentation](/config/module-parser#javascripttypereexportspresence).
...
To address this, Rspack 1.5 adds a built-in [VirtualModulesPlugin](/plugins/virtual-modules-plugin).
...
After extensive validation, the [experiments.lazyCompilation](/config/lazy-compilation) option has been promoted from an experimental feature to a stable feature, and is now available at the top level of the Rspack configuration:
...
The Rspack [experiments.topLevelAwait](/guide/migration/rspack_1.x#other-experimental-configuration-changes) option was used to control support for top-level await, and it has always been enabled by default.
...
- Enabled [lazyBarrel](/guide/optimization/lazy-barrel) to optimize the build performance of barrel files and reduce unnecessary module resolution.
- Enabled [inlineEnum](/config/optimization#optimizationinlineexports) to inline TypeScript enums, reducing the bundle size after compilation.
- Enabled [typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) to correctly detect TypeScript type re-exports, improving type handling accuracy.
...
- **Mock API**: Rstest now includes a comprehensive [mock API](https://rstest.rs/api/runtime-api/rstest/mock-modules), enabling developers to replace actual module implementations in test environments, with full support for mocking ES modules.
