...
- New features
  ...
  - [@rspack/browser Released](#rspackbrowser-released)
  - [Extending Rspack with Rust](#extending-rspack-with-rust)
  - [Const Inline](#const-inline)
  - [Support for type Re-exports](#support-for-type-re-exports)
...
- Misc
  - [End of Supporting for Node.js 16](#end-of-supporting-for-nodejs-16)
...
While this pattern simplifies module imports, it can introduce performance overhead during the build process: when importing a single module from a barrel file, Rspack must resolve and build **all** modules referenced in the barrel file, even if only a small subset is actually used.

To address this, Rspack 1.5 introduces the experimental [lazyBarrel](/config/experiments#experimentslazybarrel) feature. This optimization automatically detects **side-effect-free barrel files** and applies deferred build optimizations to their `reexports`. Modules are only resolved and built when truly needed, significantly reducing unnecessary module resolution and build costs.
This is particularly effective in projects with a large number of barrel files, delivering substantial build performance improvements.
...
In real-world testing, enabling barrel file optimization yielded clear performance gains across applications of different scales:
...
| Module Resolutions | 39,675 | 20,071 次 |        -49% |
| Module Builds      |  9,358 |  5,062 次 |        -46% |
...
Barrel file optimization is enabled by default in Rsbuild 1.5, and we plan to make it the default in Rspack 1.6 for all projects.
For more details, see the [experiments.lazyBarrel documentation](https://chatgpt.com/config/experiments#experimentslazybarrel).

### Faster file system watcher

Previously, Rspack relied on the [watchpack](https://github.com/webpack/watchpack) file system watcher to track file changes. However, we identified performance bottlenecks in `watchpack`. For example, each file change would trigger a new instance creation, consuming significant CPU and memory in large projects (see [#7490](https://github.com/web-infra-dev/rspack/issues/7490)).
`
To address this, we redesigned and reimplemented a native file system watcher in Rust, offering the following benefits:

- **High performance**: HMR speed improvements of up to 50%
- **Incremental updates**: Only processes files that actually change
- **Persistent runtime**: Runs continuously throughout development without repeated reinitialization
...
With the release of [@rspack/browser](https://www.npmjs.com/package/@rspack/browser), you can now **use Rspack directly in any modern browser**—without relying on WebContainers or specific platforms.
`@rspack/browser` is designed specifically for pure browser environments, providing core bundling capabilities for complex web projects. It offers a lightweight way to reproduce issues, share configurations, and helps developers get started with Rspack through interactive online demos.
...
`@rspack/browser` is currently in an experimental stage and may introduce breaking changes.
We will continue to enhance its capabilities for online bundling scenarios. In the meantime, Welcome to try it out at the [Rspack Playground](https://playground.rspack.rs/).
...
You can get started quickly with the [official template](https://github.com/rspack-contrib/rspack-binding-template).
For more details, see the [design rationale](https://rspack-contrib.github.io/rspack-rust-book/custom-binding/getting-started/rationale.html).

⚠️ Please note: adopting this approach introduces additional maintenance costs and is recommended only when extreme performance optimization is required.
...
> For more details, see the [experiments.inlineConst documentation](/config/experiments#experimentsinlineconst).
> This feature is planned to be enabled by default in v1.6.
...
Implemented in Rust, this plugin moves the storage and management of virtual modules to the Rust layer, effectively reducing module read and parse overhead. As a result, it delivers significantly better performance when handling large volumes of virtual modules.
...
Below is a comparison of bundle size optimization achieved in a demo project using the new Module Federation plugin:
...
Since version 1.4, we have implemented a series of optimizations to reduce Rspack’s installation size, which have already delivered significant results:
...
- Reducing the install size by **5 MB** through adjustments to [compiler parameters](https://github.com/web-infra-dev/rspack/pull/11077)
- Optimizing upstream dependencies [wasmer](https://github.com/wasmerio/wasmer/pull/5621/files) and [SWC](https://github.com/swc-project/swc/pull/10638), resulting in nearly **3 MB** saved
- Leveraging [feature flags](https://github.com/web-infra-dev/rspack/pull/10965) to cut down close to **2 MB**
- Improving the [browserslist-rs](https://github.com/browserslist/browserslist-rs/pull/32) data structure, saving around **2 MB**

Rspack now integrated artifact size checks into its daily [code quality checks](https://github.com/web-infra-dev/rspack/blob/main/.github/actions/binary-limit/action.yml), and we will continue to closely monitor size changes in future releases.
...
### End of supporting for Node.js 16

Since Node.js 16 reached its end of life on September 11, 2023, and many community npm packages (such as `webpack-dev-server`, `css-loader`, `sass-loader`, etc.) have stop supporting for Node.js 16, **Rspack 1.5 will no longer support Node.js 16** to reduce maintenance complexity.
...
We welcome you to try it out and share feedback to help us refine and improve Rslint together!

### Rsbuild 1.5

Out-of-the-box usability has always been a core design principle of Rsbuild. In Rsbuild 1.5, we have enabled several of Rspack’s latest features by default, delivering improved build performance, including:
...
Rsbuild 1.5 also introduces the new [output.module](https://rsbuild.rs/zh/config/output/module) option, which allows generating build outputs in the ES modules format.
...
In addition, we are actively designing and developing a new ESM output generation strategy, aiming to deliver ESM output quality comparable to tools like esbuild and Rollup, while keeping webpack-compatible interop behavior to ensure correctness.
See [interop tests](https://jserfeng.github.io/interop-test/by-test-case) for details.
...
Read more in the [Rsdoctor 1.2 Release Note](https://rsdoctor.rs/zh/blog/release/release-note-1_2).
...
- **Mock Module Support**: Rstest now includes a comprehensive mock API, enabling developers to replace actual module implementations in test environments, with full support for mocking ES modules.

![Mock Modules](https://assets.rspack.rs/rspack/assets/rspack-v1-5-rstest-mock-modules.png)
