...
To address this, Rspack 1.5 introduces the experimental [lazyBarrel](/config/experiments#experimentslazybarrel) feature. This optimization automatically detects **side-effect-free barrel files** and applies deferred build optimizations to their re-exports. Modules are only resolved and built when truly needed, significantly reducing unnecessary module resolution and build costs.
...
In real-world testing, enabling barrel file optimization yielded clear performance gains across applications at different scales:
...
Previously, Rspack relied on the [watchpack](https://github.com/webpack/watchpack) file system watcher to track file changes. However, we identified performance bottlenecks in `watchpack`. For example, each file change triggers a new instance creation, consuming significant CPU and memory in large projects (see [#7490](https://github.com/web-infra-dev/rspack/issues/7490)).
...
`@rspack/browser` is designed specifically for pure browser environments, providing core bundling capabilities for complex web projects. It offers a lightweight way to reproduce issues, share configurations, and help developers get started with Rspack through interactive online demos.
...
This plugin is implemented in Rust and moves the storage and management of virtual modules to the Rust layer. This effectively reduces module read and parse overhead. As a result, it delivers significantly better performance when handling large volumes of virtual modules.
