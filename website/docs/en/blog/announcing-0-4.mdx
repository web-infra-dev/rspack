---
description: 'Rspack 0.4 is out, removing support for some builtin features.'
...
Checkout [here](/guide/integrations/react#fast-refresh) for more details.
...
For `builtins.html`, it can be directly migrated to [HtmlRspackPlugin](/plugins/html-rspack-plugin):
...
For `builtins.copy`, it can be directly migrated to [CopyRspackPlugin](/plugins/copy-rspack-plugin).

For the original `builtins.minifyOptions`, we provide [SwcJsMinimizerRspackPlugin](/plugins/swc-js-minimizer-rspack-plugin):
