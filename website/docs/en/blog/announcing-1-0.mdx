...
18 months ago, we open-sourced Rspack 0.1 and received a lot of feedback and contributions from the community. During this time, 170 contributors have joined in the development of Rspack, submitting over 5000 pull requests and more than 2000 issues, helping Rspack to release over 80 versions. And Rspack's weekly downloads on npm have exceeded 100,000 🎉.
...
Here is a comparison of build performance between Rspack 0.1 and Rspack 1.0 from the [benchmark](https://github.com/rspack-contrib/performance-compare). As shown, Rspack has significantly improved build performance while also adding many new features:

![Rspack benchmark](https://assets.rspack.dev/rspack/assets/rspack-v1-0-benchmark.png)

Note that the current architecture and implementation of Rspack still have a lot of room for optimization. After the 1.0 release, we plan to further improve the performance by several folds to better support large-scale application.

### Better compatibility

When 0.1 was first released, Rspack had not yet implemented many webpack APIs and hooks, so it was only compatible with a limited number of webpack plugins and loaders. This required us to fork some community libraries to adapt them to Rspack, such as the early versions of [@rspack/plugin-html](https://www.npmjs.com/package/@rspack/plugin-html), [@rspack/plugin-minify](https://www.npmjs.com/package/@rspack/plugin-minify), and [@rspack/plugin-node-polyfill](https://www.npmjs.com/package/@rspack/plugin-node-polyfill).
...
For example, in a real-world medium web application, the bundle size of Rspack 1.0 was optimized from 6600KB to 5900KB compared to Rspack 0.1, which is equivalent to webpack. In the future, Rspack will continue to explore more advanced solutions to optimize bundle size.
