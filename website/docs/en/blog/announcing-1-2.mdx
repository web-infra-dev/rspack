...
- New features
  - [Persistent cache](#persistent-cache): Experimental feature improves hot start performance of large projects by more than **60%**.
  - [New code splitting algorithm](#new-code-splitting-algorithm): Experimental feature that significantly improve the code splitting performance after it is enabled.
  - [Yarn pnp](#yarn-pnp)
- Performance improvements
  - [Watch scope change](#watch-scope-change): Ignore `.git` and `node_modules` monitoring by default.
...
| 32C64G Linux (2.4GHz) | Number of modules | Normal dev | Cold dev     | Hot dev       |
|-----------------------|-------------------|------------|--------------|---------------|
...
The default value of `watchOptions.ignored` is changed to `/[\\/](?:.git|node_modules)[\\/]/`, which will significantly reduce the number of watched files and bring performance improvements.
