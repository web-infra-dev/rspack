...
- New features
  ...
  - [Incremental builds by default](#incremental-builds-enabled-by-default)
...
由于自定义的 `inputFileSystem` 是通过 JavaScript 实现的，可能导致性能下降。为了缓解这个问题，`useInputFileSystem` 允许你传入一个正则表达式数组，过滤哪些文件需要从自定义的 `inputFileSystem` 读取，避免因替换原生文件系统而导致的性能劣化。
...
Rspack 1.4 进一步优化了 tracing 功能，基于 [perfetto](https://perfetto.dev/) 来提供更强大的性能分析工具，可以用于快速定位构建性能的瓶颈。

你可以通过 `RSPACK_PROFILE` 环境变量开启 tracing 功能：
...
> 更多用法请参考 [Tracing 文档](/contribute/development/tracing)。
...
- `create-rspack` 现在提供 [Biome v2](https://biomejs.dev/blog/biome-v2/) 作为代码校验和格式化的工具。
