...
The plugin accepts two positional arguments.

### resourceRegExp

- **Type:** `RegExp`

Matches the module request during `beforeResolve` and the resolved resource during `afterResolve`.

### newResource

- **Type:** `string | ((data: ResolveData) => void)`

A replacement request or a function that updates the supplied resolve data. Relative replacement requests are resolved relative to the original resource.

<Attribution url="https://webpack.js.org/plugins/normal-module-replacement-plugin/" />
