...
Maps identifiers or property paths to the code fragments that replace them at compile time. Each key in the object is one definition, such as `DEBUG` or `process.env.NODE_ENV`.
...
<Attribution url="https://webpack.js.org/plugins/define-plugin/" />
