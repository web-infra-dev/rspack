...
An array preserves scope order. For example, `['custom', 'default']` uses `"custom"` as the primary scope when initializing a legacy container. Enhanced containers receive the complete ordered scope list. The scopes keep separate shared registries. An entry whose initial chunks consume a shared module from an ordered scope array starts asynchronously, since the scopes are initialized before the consume is installed; module-library output awaits that initialization.
...
This option is available with Module Federation 1.5, which uses the enhanced runtime. When using `ContainerPlugin` directly, set `enhanced: true`; Module Federation 1.0 does not support this option. Omitting `layer` leaves it unspecified. A layer is a non-empty string; an empty string is rejected.
...
- request: Override the module request matched by this shared configuration. When `import` is omitted, it defaults to `request`, or to the configuration key when `request` is omitted. Requires the enhanced runtime (Module Federation 1.5); set `enhanced: true` when using `ConsumeSharedPlugin` or `ProvideSharedPlugin` directly.
- layer: Select the layer of the shared provider and fallback module. Matching layers take precedence over fallback configurations without a layer. A layer is a non-empty string; an empty string is rejected. Requires the enhanced runtime.
- issuerLayer: Restrict consumption to modules imported from this layer. This is independent of the shared module's `layer`. A non-empty string; requires the enhanced runtime.
...
- Shares with a scalar scope and no layer retain the legacy `id`, which is not guaranteed to be unique. Within a manifest, use `name`, `shareScope`, and `layer` together to distinguish shared entries, preserving scope-array order.
...
- `disableAssetsAnalyze`: Disables asset analysis. Shared and exposed modules are described from configuration, with empty asset lists.
