...
The `name` option is required when exposing modules. If no modules are exposed, you can omit `name`; the Module Federation runtime uses [`output.uniqueName`](/config/output#outputuniquename) instead.
