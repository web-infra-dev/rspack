...
When asset analysis finds multiple concrete providers for a shared name, both files include an optional `shared[].providers` array. Each record contains `version`, a context-relative `import`, and `assets`. Existing shared fields remain unchanged. The array is omitted for zero or one provider and when asset analysis is disabled. Provider-specific fallback entry metadata and layer-aware grouping are outside this draft; see the [provider metadata RFC](https://github.com/module-federation/core/issues/5082).
