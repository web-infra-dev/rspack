---
description: 'ModuleFederationPlugin enables Module Federation in Rspack'
---
...
`ModuleFederationPlugin` enables Module Federation in Rspack.

This plugin implements the Module Federation 1.5 specification. If you want to learn more about the core concepts, usage, and best practices of Module Federation, please refer to the [Module Federation Guide](/guide/advanced/module-federation).

## Install dependencies

This plugin depends on the runtime package [@module-federation/runtime-tools](https://www.npmjs.com/package/@module-federation/runtime-tools), which provides runtime support for Module Federation. Please install it first:
...
An array preserves scope order. For example, `['custom', 'default']` uses `"custom"` as the primary scope when initializing a legacy container. Enhanced containers receive the complete ordered scope list. The scopes keep separate shared registries.

An array must contain at least one scope. Multiple scopes require the enhanced runtime used by `ModuleFederationPlugin`, or `enhanced: true` when using the lower-level federation plugins. Without the enhanced runtime, a single-element array is treated as a string and arrays with multiple entries are rejected. These rules also apply to `remotes.*.shareScope` and `shared.*.shareScope`.
...
Assign a [module layer](/guide/advanced/layer) to an exposed module. This allows the same module request to be exposed in different layers without assigning its layer through `module.rules`:

```js
new rspack.container.ModuleFederationPlugin({
  name: 'app',
  exposes: {
    './server': { import: './src/module.js', layer: 'server' },
    './client': { import: './src/module.js', layer: 'client' },
  },
});
```

This option is available with Module Federation 1.5, which uses the enhanced runtime. When using `ContainerPlugin` directly, set `enhanced: true`; Module Federation 1.0 does not support this option. Omitting `layer` leaves it unspecified. An empty string (`layer: ''`) is preserved as an explicit layer value and is distinct from an omitted layer.
...
- request: Override the module request matched by this shared configuration. When `import` is omitted, it defaults to `request`, or to the configuration key when `request` is omitted.
- layer: Select the layer of the shared provider and fallback module. Matching layers take precedence over fallback configurations without a layer. An empty string is an explicit layer and differs from an omitted value.
- issuerLayer: Restrict consumption to modules imported from this layer. This is independent of the shared module's `layer`.
...
When a shared key, scope, and layer has multiple concrete providers, its shared entry includes an optional `providers` array. Each provider records its `version`, resolved `import` relative to the build context, and `assets`. The existing shared entry and its fields are retained for older readers. Single-provider entries omit `providers`; readers must also accept manifests without this field.

Layer-aware builds add optional identity fields to both outputs:

- `layer` identifies the layer of an exposed or shared entry; entries without a layer omit it.
- Shared entries include `shareScope` for non-default scopes or ordered scope arrays. An omitted scope means the scalar `"default"` scope.
- Shares with a scalar scope and no layer retain the legacy `id`, which is not guaranteed to be unique. Within a manifest, use `name`, `shareScope`, and `layer` together to distinguish shared entries, preserving scope-array order and the distinction between an omitted layer and an empty string.
- Exposes include `requiredShared` when shared names alone are insufficient. Each requirement contains `name` and optional `shareScope` and `layer`; the stats output also retains the key-only `requires` list. Inferred requirements are omitted when asset analysis is disabled.

Readers must accept older manifests without these optional fields.
...
- `disableAssetsAnalyze`: Disables asset analysis. Shared and exposed modules are described from configuration, with empty asset lists. Resolved provider details are omitted.
