...
- `layer` identifies the layer of an exposed or shared entry; unlayered entries omit it.
...
- Scalar, unlayered shares retain the legacy `id`. `identityId` disambiguates colliding legacy IDs and shared keys beginning with the reserved `shared:` prefix. Use `identityId ?? id` as the shared entry's identity and treat it as opaque.
