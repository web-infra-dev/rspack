...
This option is available with Module Federation 1.5, which uses the enhanced runtime. When using `ContainerPlugin` directly, set `enhanced: true`; Module Federation 1.0 does not support this option. Omitting `layer` leaves it unspecified.
...
- layer: Select the layer of the shared provider and fallback module. Matching layers take precedence over fallback configurations without a layer.
