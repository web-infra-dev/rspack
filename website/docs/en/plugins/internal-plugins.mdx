---
description: 'Low-level Rspack plugins for target presets, runtime templates, child compilers, and higher-level plugins.'
---
...
This page catalogs low-level plugins that Rspack primarily uses to implement target presets, runtime templates, child compilers, and higher-level plugins.

:::warning
Most application builds should use the corresponding high-level configuration instead. Use these plugins directly only when building a custom compiler, configuring a child compiler, or inspecting Rspack internals.
:::
