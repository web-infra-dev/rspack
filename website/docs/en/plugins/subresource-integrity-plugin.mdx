---
description: 'The rspack.SubresourceIntegrityPlugin is a plugin for enabling Subresource Integrity in Rspack'
---
...
The plugin supports integration with [`HtmlRspackPlugin`](/plugins/html-rspack-plugin) and [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin). It will automatically set the `integrity` and `crossorigin` attributes for the injected tags.
...
When the HTML plugin([`HtmlRspackPlugin`](/plugins/html-rspack-plugin) or [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin)) is used, the `integrity` and `crossorigin` attributes will be set automatically.

The SubresourceIntegrityPlugin will interact with [`HtmlRspackPlugin`](/plugins/html-rspack-plugin) by default:
...
With [`HtmlRspackPlugin`](/plugins/html-rspack-plugin), the grammar of the template is a bit different with `.ejs`(see [here](/plugins/html-rspack-plugin#supported-ejs-syntax)), you can inject them like this:
