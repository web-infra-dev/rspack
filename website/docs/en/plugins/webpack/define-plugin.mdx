---
description: 'The DefinePlugin replaces variables in your code with other values or expressions at compile time'
---
...
Use `JSON.stringify` when you want to inject a literal value rather than a code fragment.

Common cases include:

- Injecting string constants such as environment names, versions, or API endpoints.
- Injecting objects or arrays that should be read directly in application code.
- Injecting string values from environment variables such as `process.env.*`.

This is because string values passed to `DefinePlugin` are inserted directly as code and do not get quotes added automatically.
...
When `experiments.env` is enabled, `import.meta.env` can be defined with `DefinePlugin`:
...
When only individual fields are accessed, Rspack replaces each access with its value:
...
When the whole `import.meta.env` object is read, Rspack generates an object containing all defined fields:

```js title="src/index.js"
const env = import.meta.env;

console.log(typeof env); // 'object'
console.log(Boolean(env)); // true
...
console.log(Boolean(env));
console.log(env.MODE);
```

When `experiments.env` is enabled, [`EnvironmentPlugin`](/plugins/webpack/environment-plugin), `NODE_ENV`, and `DefinePlugin` keys under `import.meta.env.*` are collected into `import.meta.env`.
