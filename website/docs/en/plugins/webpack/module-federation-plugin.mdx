...
Restrict which remote entry origins can be loaded at runtime.

If this option is not set, the existing loading behavior stays the same. Once `allowedRemoteOrigins` is configured, only matching network remote entry URLs can be loaded.
