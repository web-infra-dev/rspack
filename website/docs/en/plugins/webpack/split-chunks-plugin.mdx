...
When output contains multiple small modules that collectively exceed the minSize threshold, developers may still want to prevent these modules from being split into separate chunks. The `minSizeReduction` parameter specifies the minimum total size reduction required for module splitting.

Assuming the following scenario, a module has a size of `40KB`, this module is owned by 2 chunks, with configuration `minSizeReduction: 100`, potential reduction in size for each parent chunk after splitting: `40KB`, total reduction amount `40KB × 2 = 80KB < 100KB`, therefore no split is triggered
