...
In extracted CSS flows such as `mini-css-extract-plugin` or Rspack's [Built-in CSS support](/guide/tech/css#built-in-css-support), changing chunk groups with splitChunks can also change final CSS order. See [Deep dive into webpack CSS order issue](https://github.com/orgs/web-infra-dev/discussions/12) for background. If CSS order is the primary concern, also review [CssChunkingPlugin](/plugins/rspack/css-chunking-plugin).
