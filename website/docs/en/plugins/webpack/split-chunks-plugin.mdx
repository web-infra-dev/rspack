...
- the shared candidate is too small to satisfy `minSize`
- the split does not satisfy `minSizeReduction`
- the candidate does not satisfy `minChunks`
- `maxAsyncRequests` or `maxInitialRequests` rejects the split
