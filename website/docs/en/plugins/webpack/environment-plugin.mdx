...
When [`experiments.env`](/config/experiments#experimentsenv) is enabled, the `EnvironmentPlugin` also defines each environment variable under `import.meta.env.*` in addition to `process.env.*`:
