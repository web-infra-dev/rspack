...
:::warning Deprecated
`CircularDependencyRspackPlugin` is deprecated. Use [`CircularCheckRspackPlugin`](/plugins/circular-check-rspack-plugin), which has better performance and a more reasonable API design.
:::
