...
### splitChunks.optimizeForSize

- **Type:** `boolean`
- **Default:** `false`

Spend additional build time looking for shared modules that can be extracted to reduce output size. Enable this option when smaller production assets are worth a longer build:
...
This option discovers additional extraction candidates for unnamed cache groups. Extraction still respects the cache group's module and chunk filters, priorities, size limits, and request limits, and preserves module execution semantics. It can change chunk boundaries and filenames. The size benefit and build-time cost depend on the project; measure both before enabling it.

Configure this option on `splitChunks`, not on individual cache groups.
