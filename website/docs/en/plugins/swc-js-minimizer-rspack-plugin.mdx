---
description: 'This plugin is used to minify JavaScript files using SWC'
---
...
:::tip
When `optimization.minimizer` is set, the default minimizers are disabled, so we need to add [LightningCssMinimizerRspackPlugin](/plugins/lightning-css-minimizer-rspack-plugin) to minify CSS files.
:::
