---
description: 'rspack.HtmlRspackPlugin is a high-performance HTML plugin implemented in Rust'
---
...
:::warning
`rspack.HtmlRspackPlugin` does not support the full [EJS syntax](https://ejs.co/#docs), it only supports a subset of the EJS syntax. If you need the full EJS syntax support, you can use `html-webpack-plugin` directly.
...
type HtmlRspackPluginOptions = {
  ...
  template?: string;
  templateContent?:
    string | ((params: Record<string, any>) => string | Promise<string>);
  templateParameters?:
