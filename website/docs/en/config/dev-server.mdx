...
This option allows us to provide a custom client implementation. This allows specifying how the browser or other client communicates with the `devServer`.
...
- **Type:** `'http' | 'https' | 'http2' | string | object`
- **Default:** `'http'`
...
:::tip
If you use `server: 'https'` or `server: 'http2'` without providing `server.options.key` and `server.options.cert`, install [`selfsigned`](https://www.npmjs.com/package/selfsigned) in your project so dev server can automatically generate a local certificate.
:::
...
- `boolean`: `false` disables static file serving; `true` serves the default `public` directory with the default options.
- `string`: shorthand for `static.directory`.
- `object`: configure one static directory.
- `array`: configure multiple static directories.

Set it to `false` to disable static file serving:
...
Tells the server which directory to read static files from, the value is an absolute path.

`directory` only controls which folder is read from disk. It does not decide where Rspack compilation assets are served from. The URL prefix for these static files is controlled by [`static.publicPath`](#publicpath).
...
Allows configuring lower-level options for serving files. See the [express.static](https://expressjs.com/en/api.html#express.static) documentation for the available options.
...
Tells the server at which URL to serve the content from [`static.directory`](#directory).
...
Controls whether files in [`static.directory`](#directory) are watched for changes. By default, changes to static files trigger a full page reload. Set it to `false` to disable watching.
...
You can also pass an object to configure advanced watch options. See the [`chokidar`](https://github.com/paulmillr/chokidar) documentation for the available options.
...
- **Type:** `string | string[] | object | (string | object)[]`

This option allows you to configure a list of directories/files to watch for file changes, implemented based on [`chokidar`](https://github.com/paulmillr/chokidar).
...
- For more advanced options, see the [`chokidar`](https://github.com/paulmillr/chokidar) documentation.
- If [`devServer.liveReload`](#devserverlivereload) is set to `false`, changes to these watched files will not refresh the browser.

:::

## devServer.webSocketServer

- **Type:** `boolean | 'ws' | string | object`

This option allows you to provide custom WebSocket server implementation.
...
<Attribution url="https://webpack.js.org/configuration/dev-server/" />
