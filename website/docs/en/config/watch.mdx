...
You can also mix glob patterns and regular expressions in an array:
...
When using glob patterns, Rspack matches them with glob semantics, so make sure to get yourself familiar with glob matching before you use glob patterns for watchOptions.ignored.
