---
description: 'Rspack configuration overview covering core options, output behavior, optimization, development, and tooling settings.'
---
...
See [Extending Configurations](./extends) for details on how to extend configurations from other files or packages.
...
- When the runtime supports native TypeScript, Rspack CLI tries the native loader first and falls back to [Jiti](https://github.com/unjs/jiti) if loading fails.
...
If your config depends on TypeScript transforms that native loading and Jiti do not perform, you can still use [ts-node](https://npmjs.com/package/ts-node) as an external loader.
