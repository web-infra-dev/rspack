...
Rspack supports extending configurations from other files or packages using the `extends` property. This allows you to create a base configuration and extend it for different environments or use cases.

### Basic Usage

You can extend a configuration from another file by specifying the path to the file in the `extends` property:
...
### Extending from Node Modules

You can also extend configurations from packages installed in your node_modules:
...
Configurations can have their own `extends` property, allowing for nested configuration inheritance:
