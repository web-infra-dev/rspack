---
description: 'Use optimization config to custom optimization strategy'
---
...
<PropertyType
  type="false | 'natural' | 'named' | 'deterministic' | 'compact-hashed' | 'size' | 'total-size'"
  defaultValueList={[
...
| `'compact-hashed'` | Use the shortest available prefix of a stable lowercase alphanumeric hash.                                                                     |
...
| `false`            | Disable Rspack's built-in chunk id algorithm, allowing a custom plugin to provide chunk ids.                                                   |
...
The `compact-hashed` option uses the shortest available prefix of a stable lowercase alphanumeric hash as the chunk id, balancing long-term caching, bundle size, and runtime performance.

:::warning Deprecated alias
`compat-hashed` was available in Rspack 2.2.0. It is deprecated and was renamed to `compact-hashed` in Rspack 2.2.1. It remains available as a compatibility alias.
:::
...
:::tip
The `compact-hashed` option is available in Rspack 2.2.1 and later.
:::
...
Customize the minimizer. By default, [`rspack.SwcJsMinimizerRspackPlugin`](/plugins/swc-js-minimizer-rspack-plugin) and [`rspack.LightningCssMinimizerRspackPlugin`](/plugins/lightning-css-minimizer-rspack-plugin) are used.
...
<PropertyType
  type="false | 'natural' | 'named' | 'deterministic' | 'compact-hashed' | 'hashed'"
  defaultValueList={[
    { defaultValue: "'deterministic'", mode: 'production' },
    { defaultValue: "'named'", mode: 'development' },
    { defaultValue: "'natural'", mode: 'none' },
  ]}
/>
...
| `compact-hashed` | Use the shortest available prefix of a stable alphanumeric hash.                                                               |
...
We recommend using `named` in development and `deterministic` or `compact-hashed` in production.

Use `process.env.NODE_ENV` to determine the current environment:
...
The `compact-hashed` option uses the shortest available prefix of a stable alphanumeric hash as the module id, balancing long-term caching, bundle size, and runtime performance.
...
:::warning
`compact-hashed` module ids are case-sensitive and may contain uppercase letters. On case-insensitive file systems, avoid using `[id]` as the only distinguishing placeholder in [Asset module filenames](/config/output#outputassetmodulefilename) or [WebAssembly module filenames](/config/output#outputwebassemblymodulefilename). Prefer `[contenthash]`, or include it alongside `[id]`.
:::
...
export default {
  optimization: {
    moduleIds: isDev ? 'named' : 'compact-hashed',
  },
};
```
...
`optimization.nodeEnv` controls the value used to replace `process.env.NODE_ENV` at compile time. Rspack performs this replacement with [DefinePlugin](/plugins/define-plugin).
...
See available options for configuring this behavior in the [SplitChunksPlugin](/plugins/split-chunks-plugin) page.
