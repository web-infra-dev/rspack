...
<PropertyType
  type="false | 'natural' | 'named' | 'deterministic' | 'compat-hashed' | 'size' | 'total-size'"
  defaultValueList={[
...
| `'compat-hashed'` | Use the shortest available prefix of a stable lowercase alphanumeric hash.                                                                     |
...
The `compat-hashed` option uses the shortest available prefix of a stable lowercase alphanumeric hash as the chunk id, balancing long-term caching, bundle size, and runtime performance.
...
:::tip
The `compat-hashed` option is available in Rspack 2.2.0 and later.
:::
...
<PropertyType
  type="false | 'natural' | 'named' | 'deterministic' | 'compat-hashed' | 'hashed'"
  defaultValueList={[
    { defaultValue: "'deterministic'", mode: 'production' },
    { defaultValue: "'named'", mode: 'development' },
...
| `compat-hashed` | Use the shortest available prefix of a stable alphanumeric hash.                                                               |
...
We recommend using `named` in development and `deterministic` or `compat-hashed` in production.
...
The `compat-hashed` option uses the shortest available prefix of a stable alphanumeric hash as the module id, balancing long-term caching, bundle size, and runtime performance.

:::warning
`compat-hashed` module ids are case-sensitive and may contain uppercase letters. On case-insensitive file systems, avoid using `[id]` as the only distinguishing placeholder in [Asset module filenames](/config/output#outputassetmodulefilename) or [WebAssembly module filenames](/config/output#outputwebassemblymodulefilename). Prefer `[contenthash]`, or include it alongside `[id]`.
:::
...
export default {
  optimization: {
    moduleIds: isDev ? 'named' : 'compat-hashed',
  },
};
```
