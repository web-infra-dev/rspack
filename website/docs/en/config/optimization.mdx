...
<PropertyType
  type="false | 'natural' | 'named' | 'deterministic' | 'compact' | 'hashed'"
  defaultValueList={[
    { defaultValue: "'deterministic'", mode: 'production' },
    { defaultValue: "'named'", mode: 'development' },
  ]}
...
| `compact`       | Use stable alphanumeric ids with a denser identifier space. Equivalent to using `CompactModuleIdsPlugin` with default options. |
...
The `compact` option provides the same long-term stability while encoding ids with letters followed by alphanumeric characters. Its denser id space can reduce repeated module id bytes in bundles with many modules. It is opt-in; production mode continues to default to `deterministic`.
