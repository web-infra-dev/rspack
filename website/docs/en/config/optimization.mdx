...
Passing an options object also enables module concatenation. The `commonjs` option controls whether CommonJS modules with statically analyzable exports can be concatenated, and defaults to `true`. Set `commonjs` to `false` to concatenate only ECMAScript modules.

When [`output.library.type`](/config/output#outputlibrarytype) is `'modern-module'`, Rspack uses its dedicated linking and scope-hoisting pipeline. In that pipeline, `optimization.concatenateModules` only controls CommonJS hoisting: `false` or `{ commonjs: false }` prevents CommonJS modules from being scope-hoisted.
