...
<PropertyType
  type="'natural' | 'named' | 'deterministic' | 'compact' | 'size' | 'total-size'"
  defaultValueList={[
...
| `'compact'`       | Stable alphanumeric ids with a denser identifier space. Equivalent to using `CompactChunkIdsPlugin` with default options.                      |
...
The `compact` option starts with one-character chunk ids and extends them when collisions occur. It is opt-in; production mode continues to default to `deterministic`.
