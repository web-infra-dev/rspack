...
Rspack memoizes the result per distinct set of inputs the function actually reads, so a function that only reads `request` runs once per distinct request instead of once per import. Functions that use `getResolve` are never memoized.
