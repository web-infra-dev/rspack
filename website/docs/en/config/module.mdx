...
### module.parser["css/auto"].exportType

- **Type:** `'link' | 'text' | 'css-style-sheet' | 'style'`
- **Default:** `'link'`

Controls how CSS modules are exported to JavaScript.

- `'link'`: emits CSS as a CSS asset and keeps the normal CSS export behavior.
- `'text'`: exports the generated CSS text as the default export.
- `'css-style-sheet'`: exports a `CSSStyleSheet` instance as the default export.
- `'style'`: injects the generated CSS into the document with a `<style>` element at runtime and keeps the normal CSS export behavior.

When using CSS modules with `'text'` or `'css-style-sheet'`, named exports for local class names are still available.
...
Same as [`module.parser["css/auto"].exportType`](#moduleparsercssautoexporttype).
...
### module.parser["css/module"].exportType
