...
### module.parser.javascript.sideEffectsFree

<ApiMeta stability={Stability.Experimental} />

<PropertyType type="string[]" />

Manually mark top-level exported functions in matched modules as side-effect-free for advanced tree shaking.
...
export default {
  ...
  module: {
    parser: {
      javascript: {
        sideEffectsFree: ['isString', 'default'],
      },
    },
  },
};
...
:::warning
This option only takes effect when `experiments.advancedTreeShaking` is enabled, and Rspack emits a warning if a configured exported function name cannot be found in the matched module.
:::
