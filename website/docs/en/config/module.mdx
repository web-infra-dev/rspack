...
When set to `true`, Rspack will bundle `new URL()` targets as separate entry chunks (single-file output).

This option creates an `AsyncDependenciesBlock` with special `EntryOptions` that:
...
When combined with `url: 'new-url-relative'`, relative URLs are generated:
