...
- **Type:** `{ maxSize: number } | (source: Buffer, context: { filename: string, module: Module }) => boolean`
...
When a function is given, returning true tells Rspack to inject the module into the bundle as Base64 encoded string, otherwise module file will be emitted into the output directory.
