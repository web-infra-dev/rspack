---
description: 'The plugins option is used to register a set of Rspack or webpack plugins to customize the build process'
---
...
For example, [`DefinePlugin`](/plugins/define-plugin) allows you to replaces variables in your code with other values or expressions at compile time. You can access it via `rspack.DefinePlugin` and create a plugin instance with `new`:
...
For example, enable [HotModuleReplacementPlugin](/plugins/hot-module-replacement-plugin) only in the development environment:
