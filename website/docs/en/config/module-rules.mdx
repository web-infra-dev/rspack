...
### Referencing JavaScript modules by URL

<ApiMeta stability={Stability.Experimental} />

You can use `new URL('./module.js', import.meta.url)` to get the URL of a bundled JavaScript module. Configure rules with `dependency: 'url'` and the corresponding module `type`:
...
Creating the URL does not execute the script; you can use the URL to load it later, for example through `<script src>`.

By default, files referenced by `new URL()` are treated as assets, even if their extension is `.js`. A rule containing only `test` does not change this behavior: explicitly set `dependency: 'url'` and `type` as shown above to bundle them as modules.
