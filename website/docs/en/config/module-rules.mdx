...
### Referencing JavaScript and CSS modules by URL

You can use `new URL('./module.js', import.meta.url)` or `new URL('./style.css', import.meta.url)` to get the URL of a bundled JavaScript or CSS module. Configure rules with `dependency: 'url'` and the corresponding module `type`:
...
Creating the URL does not execute the script or apply the stylesheet; you can use these URLs to load them later, for example through `<script src>` and `<link rel="stylesheet" href>`.

By default, files referenced by `new URL()` are treated as assets, even if their extensions are `.js` or `.css`. A rule containing only `test` does not change this behavior: explicitly set `dependency: 'url'` and `type` as shown above to bundle them as modules.
