...
- `'css' | 'css/auto' | 'css/global' | 'css/module'`: CSS module, see [Built-in CSS support](/guide/tech/css#built-in-css-support).
...
Enables caching for an individual loader. This option only takes effect when [`experiments.newCache`](/config/experiments) is enabled.
...
- The incoming source map is not included in the cache identity. Do not enable this option for a loader whose result depends on that map.
