---
description: 'module.rules defines how Rspack processes different types of modules during the build'
---
...
For more information about layers, see the [Module layers guide](/guide/advanced/layer).
...
For specific parser options and the corresponding module type, you can refer to [`module.parser`](/config/module-parser#parser).
...
For specific generator options and the corresponding module type, you can refer to [`module.generator`](/config/module-generator#generator).
...
- `'json'`: JSON data module, see [JSON](/guide/languages/json).
- `'css' | 'css/auto' | 'css/global' | 'css/module'`: CSS module, see [Built-in CSS support](/guide/languages/css#built-in-css-support).
...
- Loader options must be an object. Function-valued options are supported and execute on the main thread.
- Compilation and module APIs use proxies to access their main-thread implementations. Loader hooks also run on the main thread. `additionalData` retains main-thread object identity when passing through parallel and builtin loaders.
...
When `maxWorkers` is omitted, Rspack uses the number of CPUs minus one, with a minimum of one worker. Workers initialize asynchronously when parallel loader rules are registered; compilation waits for initialization before making modules. The loader worker pool is shared within the current process, so use a consistent `maxWorkers` value if multiple loaders configure `parallel`.
