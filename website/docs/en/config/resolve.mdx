---
description: 'Used to configure the Rspack module resolution logic'
---
...
Rspack's resolver supports the [`NODE_PATH`](https://nodejs.org/api/modules.html#loading-from-the-global-folders) environment variable. After searching all `node_modules` directories, the resolver will also look through directories listed in `NODE_PATH` (semicolon-separated on Windows, colon-separated on POSIX).

:::warning

- `NODE_PATH` makes module resolution depend on an external environment variable, which can reduce build determinism.
- Modules in `node_modules` always take precedence over `NODE_PATH` directories.
- Avoid relying on `NODE_PATH` in CI and production builds unless you explicitly need this behavior.
  :::
