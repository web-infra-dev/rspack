...
When enabled, the resolver also searches [`NODE_PATH`](https://nodejs.org/api/modules.html#loading-from-the-global-folders) entries after `node_modules` directories. This follows the Node.js behavior of loading modules from global folders specified via the `NODE_PATH` environment variable.

:::caution
Enabling this option makes module resolution depend on the external `NODE_PATH` environment variable, which can reduce build determinism and may unexpectedly pull in globally installed modules. Keep `NODE_PATH` controlled or disabled in CI and production builds unless you explicitly need this behavior.
:::
```js title="rspack.config.mjs"
export default {
  resolve: {
