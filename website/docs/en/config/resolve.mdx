...
If you also set `resolve.byDependency.esm.extensions` or another dependency-specific `extensions` option under [resolve.byDependency](#resolvebydependency), the matching `byDependency` value is applied afterward. Without `'...'`, it replaces the current top-level `resolve.extensions` value. With `'...'`, it inserts the current top-level `resolve.extensions` value at that position.
