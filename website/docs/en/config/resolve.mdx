...
If you also set `resolve.byDependency[type].extensions`, the matching `byDependency` value is applied afterward. Without `'...'`, it replaces the top-level `resolve.extensions` value. With `'...'`, it inserts the top-level `resolve.extensions` value at that position.
