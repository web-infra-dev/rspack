...
Whether to perform the rebuild incrementally to speed up the rebuild and HMR. Two configuration ways are provided:

1. Preset: including `boolean | 'none' | 'safe' | 'advance' | 'advance-silent'`
   - `false | 'none'`: Turn off, and it will not be enabled for any stage.
   - `'safe'`: Enable incremental for `make` and `emitAssets` stages, This is also the current default behavior of Rspack.
   - `true | 'advance-silent'`: Enable incremental for all stages to maximize the performance of rebuilds and HMR. After these stages become stable in the future, we will set this option as the default behavior of Rspack.
   - `'advance'`: The same as the above, but it will throw a warning to prompt the user in cases where incremental is not friendly. This option serves as a supplement to `true | 'advance-silent'` to help users troubleshoot configurations that may potentially affect the performance of incremental rebuilds.
2. Detailed object configuration: `Incremental`, which enables fine-grained control over whether the incremental of each stage is enabled.
...
Incremental will only improve the speed of subsequent rebuilds and have no impact on the initial build. However, the initial build when the persistent cache is available will also be regarded as a rebuild, so as to use the incremental feature to speed up the build:
