...
When enabled, Rspack collects `import.meta.env` definitions from [`DefinePlugin`](/plugins/webpack/define-plugin), [`EnvironmentPlugin`](/plugins/webpack/environment-plugin), and [`optimization.nodeEnv`](/config/optimization#optimizationnodeenv), and replaces `import.meta.env` references at compile time:

- Accessing a defined field, such as `import.meta.env.MODE`, is replaced with its value.
- Reading the whole `import.meta.env` object is replaced with an object literal containing all defined fields.
- `typeof import.meta.env` evaluates to `'object'`.
...
See [DefinePlugin - import.meta.env](/plugins/webpack/define-plugin#importmetaenv) for more details. Per-module evaluation can be disabled with [`module.parser.javascript.importMeta.env`](/config/module-parser#javascriptimportmeta).
