...
rspack will use the input file system wrapped by `VirtualModulesPlugin`; other than that, `src/disk_file.js` will use the native file system to read files.
In normal project, there are not much files need to be access by the input file system, config `experiments.useInputFileSystem` properly with keep Rspack running fast.
