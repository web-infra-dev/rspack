...
Controls the faster module concatenation implementation, which is enabled by default. It reuses identifier information collected while parsing modules, avoiding an additional parse of inner-module sources during concatenated-module code generation. Set it to `false` to fall back to the legacy implementation.
