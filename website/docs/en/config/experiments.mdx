...
This capability is enabled by default. To disable pure-function analysis, set this option to `false`. To manually mark identifiers in matched modules — exported functions in a third-party module, or imports/locals in your own files — configure [`module.parser.javascript.pureFunctions`](/config/module-parser#javascriptpurefunctions) through `module.rules[i].parser` so the rule only applies to the intended modules.
