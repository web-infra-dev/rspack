...
Selects the experimental new Cache backend. This option does not enable Cache and does not affect
Incremental compilation: [`cache`](/config/cache) controls whether Cache is enabled, while
[`incremental`](/config/incremental) independently controls artifact reuse during rebuilds.

- `false` selects the legacy Cache backend.
- `true` selects the new Cache backend.
