...
Use this option to turn on the capability itself. To manually mark identifiers in matched modules — exported functions in a third-party module, or imports/locals in your own files — configure [`module.parser.javascript.pureFunctions`](/config/module-parser#javascriptpurefunctions) through `module.rules[i].parser` so the rule only applies to the intended modules.
