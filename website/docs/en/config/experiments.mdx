...
- **Type:** `boolean | BuildHttpOptions`
- **Default:** `false`

```ts
type BuildHttpOptions = {
  /**
...
Enables loading of modules via HTTP/HTTPS requests. This allows importing modules directly from URLs.
...
The `buildHttp` feature is useful for:

- Importing modules from CDNs or other HTTP sources
- Loading shared libraries from remote locations
- Working with remote assets without downloading them locally

When using a custom HTTP client implementation, you need to implement the `http_client` function that handles HTTP requests and returns a Promise that resolves to an object with status, headers, and body.
