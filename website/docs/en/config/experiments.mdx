...
A module is considered side-effect-free when it is marked with [`"sideEffects": false` in package.json or flagged by module.rule.sideEffects](/config/optimization#optimizationsideeffects).
