...
To reference bundled JavaScript modules with `new URL()`, configure [`module.rules[].dependency`](/config/module-rules#rulesdependency) as `'url'` and set the corresponding module `type`.
