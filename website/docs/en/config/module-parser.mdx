...
By default, injected CSS uses asset URLs resolved at build time. With `output.publicPath: 'auto'`, these URLs remain relative to the document. Set `runtimePublicPath: true` to use the runtime public path instead, including values assigned to `__webpack_public_path__`.

If you set `__webpack_public_path__` dynamically, set it in a separate module that runs before importing the styles. Explicit [asset generator public paths](/config/module-generator#assetpublicpath) take precedence. Inline data URLs and external URLs are unchanged.
