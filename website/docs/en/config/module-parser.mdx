...
:::warning
This option only takes effect when `experiments.pureFunctions` is enabled, which is the default. Rspack emits a warning if a configured name does not appear as a top-level binding in the matched module.
:::
