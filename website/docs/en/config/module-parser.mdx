...
`import.meta.resolve("./asset.txt")` returns the emitted asset URL as a string, like `new URL("./asset.txt", import.meta.url).href`. It uses the `url` dependency category, so assets are handled by the default asset rules. Setting `module.parser.javascript.url` to `"relative"` produces a relative URL.

If a matching module rule sets the target type to JavaScript, Rspack emits a standalone JavaScript entry and returns its URL. Resolving the URL does not execute that entry. For example, this can provide a script URL to an API that loads the script later.

Only calls with one statically evaluable string argument are bundled. Other calls are preserved for the runtime.
