...
Use the runtime public path for emitted asset URLs when `exportType` is `'style'`. This option has no effect on other export types.

By default, injected CSS uses asset URLs resolved at build time. With `output.publicPath: 'auto'`, these URLs remain relative to the document. Set `runtimePublicPath: true` to use the runtime public path instead, including values assigned to `import.meta.rspackPublicPath`.

If you set `import.meta.rspackPublicPath` dynamically, set it in a separate module that runs before importing the styles. Explicit [asset generator public paths](/config/module-generator#assetpublicpath) take precedence. Inline data URLs and external URLs are unchanged.
...
Same as [`module.parser["css/auto"].runtimePublicPath`](#cssautoruntimepublicpath).
...
<Attribution url="https://webpack.js.org/configuration/module/" />
