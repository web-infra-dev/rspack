...
- Rspack-specific keys: `env`, `glob`, `resolve`, `rspackBaseUri`, `rspackHash`, `rspackInitSharing`, `rspackNonce`, `rspackPublicPath`, `rspackRsc`, `rspackShareScopes`, `rspackUniqueId`, and `rspackVersion`.
...
`env` only takes effect when [`experiments.env`](/config/experiments#experimentsenv) is enabled. Setting it to `false` preserves `import.meta.env` in this module for runtime evaluation instead of replacing it at compile time.
