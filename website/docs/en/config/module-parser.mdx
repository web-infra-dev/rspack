...
Referencing bundled JavaScript modules with `new URL()` is experimental. To use this feature, configure [`module.rules[].dependency`](/config/module-rules#rulesdependency) as `'url'` and set the corresponding module `type`.
