...
Controls the snapshot strategy for paths recorded in `compilation.fileDependencies`. `snapshot.dependencies` is configured on the top-level `snapshot` option.
...
When a strategy object does not set `hash: true`, `timestamp` defaults to `true`. When `hash: true` is set without `timestamp: true`, Rspack uses hash-only mode.
...
Controls the snapshot strategy for paths recorded in `compilation.contextDependencies`. `snapshot.contextDependencies` is configured on the top-level `snapshot` option.
...
Move path-related snapshot options into `cache.snapshot`. Keep dependency strategy options on the top-level `snapshot` option as `snapshot.dependencies` and `snapshot.contextDependencies`.
