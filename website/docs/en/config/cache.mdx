...
- **Default:** No count limit; the existing 7-day age-based cleanup still applies
...
Enabling `maxVersions` uses compiler-scoped cache version directories. Existing unscoped versions continue to use the 7-day age cleanup and are not counted toward the limit.
