...
Move path-related snapshot options into `cache.snapshot`. Rspack currently does not expose webpack's `snapshot.module` and `snapshot.contextModule` strategy options in JavaScript configuration.
