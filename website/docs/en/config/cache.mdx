...
Rspack will automatically clean up cache folders that have not been accessed longer than `cache.storage.maxAge`.
:::
...
Enabling `maxGenerations` uses compiler-scoped cache version directories. Existing unscoped versions continue to use age-based cleanup and are not counted toward the limit.
