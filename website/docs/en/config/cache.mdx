...
Messages about cache enablement, valid build dependencies, successful recovery, and snapshot dependency changes use the `log` level. Set `stats.logging` to `'log'` or `'verbose'`, or enable `stats.loggingDebug` as shown above, to display them.
