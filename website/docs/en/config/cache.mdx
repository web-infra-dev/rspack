import PropertyType from '@components/PropertyType';

# Cache

Rspack will cache snapshots and intermediate products during the build process and use them in the next build to improve the speed of the build.

:::info Cache Type
Rspack currently only supports memory cache, persistent cache is experimental, for more details refer to [Persistent Cache](/config/experiments#persistent-cache).
:::

This configuration currently only supports setting whether to enable caching.

<PropertyType
  type="boolean"
  defaultValueList={[
    { defaultValue: 'false', mode: 'production' },
    { defaultValue: 'true', mode: 'development' },
  ]}
/>

## Usage

You can disable the cache directly in `rspack.config.js`:

```ts title="rspack.config.js"
module.exports = {
  cache: false,
};
```
