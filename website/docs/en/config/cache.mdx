...
Rspack stores filesystem cache in compiler-scoped directories. Cleanup options only affect cache entries for the current compiler, even when multiple compilers share the same `cache.storage.directory`.

Rspack will automatically clean up cache folders for the current compiler that have not been accessed longer than `cache.storage.maxAge`.
:::

#### cache.storage.maxAge

- **Type:** `number` (integer from `0` to `4294967295`, in seconds)

- **Default:** `7 * 24 * 60 * 60` (7 days)

The maximum age of cache versions. Versions that have not been accessed within this time are removed during cache cleanup. Set it to `0` to disable age-based cleanup.
...
- **Type:** `number` (integer from `0` to `4294967295`)

- **Default:** No generation count limit; age-based cleanup still applies

The maximum number of cache generations retained for each compiler. When the limit is exceeded, Rspack removes the least recently accessed generations for that compiler. Other compilers using the same storage directory are not counted toward the limit. Set it to `0` to remove previous generations during cleanup.
...
Each compiler is counted independently, so compilers that share `storage.directory` do not consume each other's limit.

Filesystem cache entries are scoped by compiler, so this limit only counts generations for the current compiler.

:::warning
Concurrent builds of the same compiler share this limit and may remove one another's older cache versions.
:::

### Inspect persistent cache logs

Persistent cache logs are emitted through the `rspack.persistentCache` logger. To display them in the build stats, enable `stats.logging`:
...
The logs include key persistent cache events, such as whether persistent cache is enabled, whether build dependencies are valid, whether cache recovery succeeded, and why the persistent cache was invalidated. They may also include logs from other loggers.

If you only want to print logs from `rspack.persistentCache`, set `stats.logging` to `false` to disable logs from other loggers, and use `stats.loggingDebug` to include this logger:
