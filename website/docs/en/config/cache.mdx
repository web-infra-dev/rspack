...
- **Type:** `{ hash?: boolean; timestamp?: boolean }`

- **Default:** `{ timestamp: true }` in development and none mode, `{ hash: true, timestamp: true }` in production mode

Controls the snapshot strategy used when building modules. `snapshot.module` is configured on the top-level `snapshot` option.

- `hash: true` compares file content hashes.
- `timestamp: true` compares file timestamps.
- When both are enabled, Rspack checks timestamps first and compares hashes only after a timestamp change, so unchanged content can still reuse the cache.

Following webpack, module snapshots use timestamp mode unless `hash: true` is set. When `hash: true` is set without `timestamp: true`, Rspack uses hash-only mode.
...
Controls the snapshot strategy used when building context modules, such as modules created by dynamic imports or requires with a context. `snapshot.contextModule` is configured on the top-level `snapshot` option.

- `hash: true` compares recursive content hashes.
- `timestamp: true` compares recursive timestamp hashes, including directory entries and child timestamps.
- When both are enabled, Rspack checks timestamps first and compares hashes only after a timestamp change, so unchanged content can still reuse the cache.

Following webpack, context module snapshots use timestamp mode unless `hash: true` is set. When `hash: true` is set without `timestamp: true`, Rspack uses hash-only mode.
...
The maximum time, in seconds, that cache can remain unused. During cleanup, Rspack removes cache that has not been accessed for longer than `maxAge`.
...
This is a generational cleanup policy that controls how many cache generations can be retained in the current cache directory. When the number of generations exceeds the limit, Rspack removes the least recently accessed cache.
...
Move path-related snapshot options into `cache.snapshot`. Keep `snapshot.module` and `snapshot.contextModule` on the top-level `snapshot` option.
