...
Rspack supports the webpack-compatible top-level `snapshot` option for persistent cache. The path options can also be configured under `cache.snapshot` for existing Rspack configurations. When both are configured, top-level `snapshot.*` values take precedence over `cache.snapshot.*`.

`snapshot.contextModule` must be configured on the top-level `snapshot` option.
...
- **Default:** `{ hash: true, timestamp: true }`

Controls the snapshot strategy used when building context modules, such as modules created by dynamic imports or requires with a context.

- `hash: true` compares recursive content hashes.
- `timestamp: true` compares recursive timestamps.
...
At least one of `hash` or `timestamp` should be enabled. Disabling both makes context module snapshots always invalid.
...
4. Keep webpack-compatible `snapshot` at the top level

Rspack supports the webpack-compatible top-level `snapshot` option for persistent cache, including `snapshot.contextModule`.
