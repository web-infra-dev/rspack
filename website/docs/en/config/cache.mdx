...
- **Type:** `{ type: 'filesystem'; directory?: string; maxVersions?: number }`
...
- **Type:** `number` (integer from `1` to `4294967295`)

- **Default:** No limit

The maximum number of cache versions retained for each compiler. When the limit is exceeded, Rspack removes the least recently accessed versions for that compiler. Other compilers using the same storage directory are not counted toward the limit.
...
Compiler retention groups are identified by the compiler context, its position in the compiler tree, and [config.name](/config/other-options#name). This keeps compilers that share `storage.directory` from counting each other's cache versions.

Cache versions created by Rspack releases that predate `maxVersions` do not contain compiler identity metadata. Those legacy versions continue to use the existing 7-day age cleanup instead of being counted toward this limit.

:::warning
Using a low limit for concurrently running builds with the same compiler name and storage directory can remove an older build's cache while that build is still running. Versions accessed at the same time are preserved and can temporarily exceed the limit until a later build performs cleanup.
:::
