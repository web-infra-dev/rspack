...
1. Presets: including `boolean | 'none' | 'safe' | 'advance' | 'advance-silent'`
   ...
   - `'safe'`: Enable incremental for the `buildModuleGraph` and `emitAssets` stage.
...
Starting from v1.4.0, Rspack enables incremental builds using `'advance-silent'` mode by default. In previous versions, it only activated incremental builds for the `buildModuleGraph` phase by default with `'safe'` mode.
