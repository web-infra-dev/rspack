...
The format of chunks, The default value of this option depends on the [`target`](/en/config/target.html) and [`output.module`](#outputmodule) setting. In general, the `'array-push'` format is used when the target is web or webworker, the `'module'` format is used when it is ESM, and the `'commonjs'` format is used when it is node.js.
...
- **Type:** `string`
- **Default:** inferred from [`output.uniqueName`](/config/output.html#outputuniquename)
...
See [`output.devtoolModuleFilenameTemplate`](/en/config/output.html#outputdevtoolmodulefilenametemplate).

## output.devtoolModuleFilenameTemplate

- **Type:** `string = 'webpack://[namespace]/[resource-path]?[loaders]'` | `function (info) => string`

This option is only used when [`devtool`](/en/config/devtool.html) uses an option that requires module names.
...
If multiple modules would result in the same name, [`output.devtoolFallbackModuleFilenameTemplate`](/en/config/output.html#outputdevtoolfallbackmodulefilenametemplate) is used instead for these modules.

## output.devtoolNamespace

This option determines the module's namespace used with the [`output.devtoolModuleFilenameTemplate`](/en/config/output.html#outputdevtoolmodulefilenametemplate). When not specified, it will default to the value of: [`output.uniqueName`](/en/config/output.html#outputuniquename). It's used to prevent source file path collisions in sourcemaps when loading multiple libraries built with Rspack.
...
- **Type:** `string`
- **Default:** It defaults to [`output.library`](/config/output.html#outputlibrary) name or the package name from package.json in the context, if both aren't found, it is set to an ''.
...
Output JavaScript files as module type. Disabled by default as it's an experimental feature. `output.module` is an experimental feature and can only be enabled by setting [`experiments.outputModule`](/en/config/experiments.html#experimentsoutputmodule) to true.
...
Only used when [chunkLoading](/config/output.html#outputchunkloading) is `"jsonp"` or `"import-scripts"`. This global variable is used for loading hot update chunks.
