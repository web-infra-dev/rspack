...
By default, injection only occurs when `__rspack_version__` and `__rspack_unique_id__` are detected in the code. Set `force` to `true` to always inject them:
...
#### Disable force injection

If you enable force injection selectively, setting `force` back to `false` restores on-demand injection:
