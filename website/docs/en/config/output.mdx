---
description: 'The top-level output key contains a set of options instructing Rspack on how and where it should output your bundles, assets, and anything else you bundle...'
---
...
- Module type is `'asset'` and asset is set to satisfy [rules[].parser.dataUrlCondition](/config/module-parser#assetdataurlcondition)
...
When this option is omitted, Rspack derives it from [`output.filename`](#outputfilename):

- If `output.filename` is a string containing `[name]`, `[id]`, `[chunkhash]`, or `[contenthash]`, Rspack reuses that string. In particular, `[name]` is not replaced with `[id]`.
- Otherwise, Rspack inserts `[id].` before the basename, preserving the directory and query string.
- If `output.filename` is a function, the default is `'[id].js'`. Rspack does not call that function to derive `output.chunkFilename`.

| `output.filename`     | Default `output.chunkFilename` |
| --------------------- | ------------------------------ |
| `'[name].js'`         | `'[name].js'`                  |
| `'[name].mjs'`        | `'[name].mjs'`                 |
| `'scripts/bundle.js'` | `'scripts/[id].bundle.js'`     |
| A function            | `'[id].js'`                    |

To choose a different template explicitly:
...
The method to load chunks (methods included by default are `'jsonp'` (web), `'import'` (ESM), `'import-scripts'` (webworker), `'require'` (sync node.js), `'async-node'` (async node.js), but others might be added by plugins). The default value will be determined based on the configuration of [`target`](/config/target) and [`chunkFormat`](#outputchunkformat).

:::tip

The default value of this option depends on the [`target`](/config/target) and [`chunkFormat`](#outputchunkformat) setting. For more details, search for `"chunkLoading"` [in the Rspack defaults](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack/src/config/defaults.ts).
...
For a string `output.chunkFilename`, Rspack replaces its `.js`, `.mjs`, or `.cjs` extension with `.css`, preserving any query string. For a function, the default is `'[id].css'`; the function is not called to derive the CSS filename.
...
- **Type:** `string | ((pathData: PathData, assetInfo?: AssetInfo) => string)`
- **Default:** Derived from [`output.filename`](#outputfilename) when it is a string, otherwise `'[id].css'`.
...
For a string `output.filename`, Rspack replaces its `.js`, `.mjs`, or `.cjs` extension with `.css`, preserving any query string. If none of these extensions is present, the string is unchanged.

| `output.filename`        | Default `output.cssFilename` |
| ------------------------ | ---------------------------- |
| `'scripts/[name].js'`    | `'scripts/[name].css'`       |
| `'[name].mjs?version=1'` | `'[name].css?version=1'`     |
| A function               | `'[id].css'`                 |

When using a function for JavaScript filenames, configure CSS filenames separately if you need named CSS assets:
...
This option is only used when [`devtool`](/config/devtool) uses an option that requires module names.
...
| [relative-resource-path] | Resource path relative to the source map file’s directory, or to the emitted asset’s directory for inline source maps |
...
For a single [`entry`](/config/entry) point, this can be a static name.
...
- **Type:** `string`
- **Default:** When [`output.module`](#outputmodule) is `true`, it is `'[id].[fullhash].hot-update.mjs'`, otherwise it is `'[id].[fullhash].hot-update.js'`.
...
The only placeholders allowed here are `[id]` and `[fullhash]`. For example:
...
- **Type:** `string`
- **Default:** When [`output.module`](#outputmodule) is `true`, it is `'[runtime].[fullhash].hot-update.json.mjs'`, otherwise it is `'[runtime].[fullhash].hot-update.json'`.
...
- **Type:** `boolean`
- **Default:** `false` when [`output.module`](#outputmodule) is `true`, otherwise `true`.
...
This configuration enables Rspack's dedicated rendering pipeline for ESM libraries. It emits direct `import` / `export` statements and supports library-oriented tree shaking, scope hoisting, code splitting, and [`output.library.preserveModules`](#outputlibrarypreservemodules).

Entry namespace re-exports (`export * as ns from './target.js'` and `import * as ns from './target.js'; export { ns };`) are automatically optimized by splitting the target and its synchronous dependencies into separate chunks where possible. This optimization is limited to targets whose public exports are local bindings; targets that re-export bindings from other modules keep the existing behavior. This preserves native namespace re-exports so consumers can tree-shake unused exports. With `preserveModules`, existing module chunks are reused. Later `optimization.splitChunks` processing can still move or merge these chunks; if the final chunk's exports cannot represent the target namespace, Rspack uses namespace-object rendering instead.

:::warning
Splitting shared dependencies for namespace re-exports can change side-effect execution order. Libraries using this optimization should not depend on that order.
:::

:::tip `module` vs `modern-module`

- `module` stays on the regular chunk / runtime rendering pipeline and is closer to webpack-compatible ESM semantics.
- `modern-module` is handled by a dedicated ESM library plugin that owns chunk rendering, linking, scope hoisting, and splitChunks behavior, making it a better fit for publishable tree-shakable, code-split ESM libraries.
- `modern-module` currently exists as a separate type to fill the ESM library capability gap; once it is mature enough, the long-term direction is to converge it with `module` into a single `library.type`.
- `preserveModules` is only available with `modern-module`.
- When `output.library` is configured and `output.module = true`, Rspack currently defaults `output.library.type` to `modern-module` if you don't specify one explicitly.
- Do not mix `modern-module` with other library types in the same compilation, because it also changes chunk loading, chunk rendering, and scope-hoisting behavior.
...
- Relative paths are resolved against `context`.
...
<Attribution url="https://webpack.js.org/configuration/output/" />
