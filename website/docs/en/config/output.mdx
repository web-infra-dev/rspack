...
:::warning
When debugging locally using [`@rspack/dev-server`](/config/dev-server) and [`HtmlRspackPlugin`](/plugins/rspack/html-rspack-plugin), the page entry and the chunks split from it by [`output.splitChunks`](/plugins/webpack/split-chunks-plugin) will be loaded by the HTML file with `script` tags. At this time, if the file name contains hash values, the hot update will fail due to the inability of dynamic loading of HTML.
:::
