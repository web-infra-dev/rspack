...
The `target` option describes the environment in which the output code will run. Rspack uses it to infer available platform APIs and ECMAScript syntax, then derives compatible defaults for module resolution, externals, chunk loading, and Rspack runtime code.
...
For browser applications, define the supported browsers in a shared [Browserslist](https://github.com/browserslist/browserslist) configuration:
...
If you do not use Browserslist and want browser output whose runtime uses at most ES2020 syntax, combine the platform and syntax targets:
...
For real browser support policies, Browserslist is usually more precise than a single ECMAScript version.
...
An `esX` target describes syntax capabilities, not a runtime platform. Combine it with a platform target unless you explicitly configure options such as the output chunk format and loading method.
...
The nearest configuration can come from `.browserslistrc` or the `browserslist` field in `package.json`. An explicitly selected `browserslist` target can also use the `BROWSERSLIST` environment variable. Rspack supports browser and Node.js queries; Electron queries are not supported.
