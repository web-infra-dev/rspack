...
## Default target for builtin loaders and plugins
...
- `builtin:swc-loader` uses the derived target as default `env.targets` or `jsc.target`
- `builtin:lightningcss-loader` uses the derived target as default `targets` (only for browserslist-related targets, since Lightning CSS only supports browser targets)
- `SwcJsMinimizerRspackPlugin` uses the derived target as default `ecma` version
- `LightningCssMinimizerRspackPlugin` uses the derived target as default `targets` (only for browserslist-related targets)
...
Third-party loaders and plugins can also access `compiler.target` to derive default targets based on Rspack's target configuration. The `compiler.target` object contains `platforms` (platform names with versions) and `esVersion` (the ECMAScript version number).
