...
在 Rspack 2.0 中，`ModuleFederationPlugin` 也进一步完善了共享依赖 Tree Shaking 能力。你可以通过 `shared.treeShaking` 对共享依赖做按需优化，相关配置可参考 [ModuleFederationPlugin](/plugins/module-federation-plugin#shared)。

你可以通过 Rspack 的 [ModuleFederationPlugin](/plugins/module-federation-plugin) 来使用它，无须安装任何插件。
...
> 参考：[Module Federation v1.5 示例](https://github.com/rstackjs/rstack-examples/tree/main/rspack/module-federation-v1.5)。
...
你可以通过 Rspack 的 [ModuleFederationPluginV1](/plugins/module-federation-plugin-v1) 来使用它。
