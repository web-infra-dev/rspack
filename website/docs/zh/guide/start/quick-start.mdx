...
同时我们也提供了基于 Wasm 和 WebContainer 的 StackBlitz 上的在线示例:

- [Rspack StackBlitz 示例](https://stackblitz.com/~/github.com/rspack-contrib/rspack-stackblitz-example)
...
## 使用 StackBlitz 在线预览项目

Rspack >= 1.4 实现了 Wasm 版本，支持直接在 StackBlitz (WebContainers) 中使用。在当前版本下，如果你想在 StackBlitz 上预览你的 Rspack 项目，需要在安装依赖的时候指定 CPU 架构。不同包管理器指定 CPU 架构的方式有所不同。
...
未来我们会优化使用流程，使得用户可以无缝地预览 Rspack 项目。除此之外，未来我们也会实现能够在浏览器上直接使用 Rspack。
