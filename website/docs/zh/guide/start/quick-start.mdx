...
## 使用 Stackblitz 在线部署项目

Rspack 1.4 实现了 WASM 版本，支持直接在 Stackblitz/WebContainer 使用。当前版本下，如果你想在 Stackblitz 上部署你的 Rspack 项目，你需要在安装依赖的时候指定 cpu 架构。不同包管理器指定 cpu 架构的方式有所不同。
...
未来我们会优化使用流程，使得用户可以无缝地部署 Rspack 项目。除此之外，未来我们也会实现能够在浏览器上直接使用 Rspack。
