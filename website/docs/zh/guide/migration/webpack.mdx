...
Rspack 正在积极推动 webpack 的下一个版本的功能，因此在某些配置项上与 webpack 5 存在一些默认值的不同，如下：
...
:::tip 提示
查看[同步自 webpack 的内置插件](/plugins/webpack/index)以了解 Rspack 对 webpack 所有内置插件的实现情况。
:::

例如替换 [HotModuleReplacementPlugin](/plugins/webpack/hot-module-replacement-plugin)：
...
通过 `builtin:swc-loader` 使用 Rspack 内置的 `swc-loader`，相比于外置的 `swc-loader`，它不需要在 JavaScript 和 Rust 之间进行频繁的通信，从而具有最佳的性能。
