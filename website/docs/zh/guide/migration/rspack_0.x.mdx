---
description: "在 Rspack 0.x 中，experiments.css 默认开启，这意味着以 .css 结尾的文件会默认被视为 type: 'css/auto'，不需要手动引入其他 loader 来处理 CSS 文件。"
---
...
[experiments.css](/config/deprecated-options#experimentscss) 的默认值从 `true` 调整为 `false`。
...
[splitChunks.cacheGroups.\{cacheGroup\}.reuseExistingChunk](/plugins/split-chunks-plugin#splitchunkscachegroupscachegroupreuseexistingchunk) 默认值从 `true` 调整为 `false`。
...
在 Rspack 0.x 中我们使用内置的 `rspack.SwcCssMinimizerRspackPlugin` 来压缩 CSS 代码。现在，该插件已被移除，并改用 [rspack.LightningCssMinimizerRspackPlugin](/plugins/lightning-css-minimizer-rspack-plugin) 替代它。
