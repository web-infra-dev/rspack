...
Rspack 2.0 将最低支持的 Node.js 版本从 18.x 升级到 **20.19.0** 以在运行时原生支持通过 `require()` 加载 ESM 模块。

同时 `@rspack/core` 和 `@rspack/cli` 现已发布为纯 ESM 包，其 CommonJS 构建产物已被移除。

## 依赖更新

`@rspack/dev-server` 为可选依赖，使用 `rspack serve` 或 `rspack preview` 命令时需要手动安装。

<PackageManagerTabs command="add @rspack/dev-server -D" />

## 实验性配置变更

### 默认启用 experiments.css

[experiments.css](/config/experiments#experimentscss) 的默认值从 `false` 调整为 `true`。CSS 处理能力现在默认可用，但仍需要在 `module.rules` 中配置对应的 `type` 以启用：
...
### 移动 experiments.incremental 到顶层
...
`experiments.typeReexportsPresence` 已被移除，可直接使用 [module.parser.javascript.typeReexportsPresence](/config/module#moduleparserjavascripttypereexportspresence) 控制重导出类型时的行为。
...
`experiments.outputModule` 已被移除，可直接使用 [output.module](/config/output#outputmodule) 输出 ESM 产物。

### 移除 experiments.parallelLoader

`experiments.parallelLoader` 已被移除，可通过 [module.rules.use.parallel](/config/module#modulerulesuseparallel) 启用 loader 的并行化：
...
`rspack.EsmLibraryPlugin` 插件已被移除，可以通过配置 [output.library](/config/output#outputlibrary) 来生成 ESM 格式的产物:
...
`experiments.SubResourceIntegrityPlugin` 已被移除，可直接使用导出对象上的 [SubresourceIntegrityPlugin](/plugins/rspack/subresource-integrity-plugin) 替代：
