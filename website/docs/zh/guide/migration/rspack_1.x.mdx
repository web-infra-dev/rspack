...
### 移动 builtin:swc-loader 的 rspackExperiments.import 至顶层 transformImport

`builtin:swc-loader` 的 `rspackExperiments.import`（也称为 `rspackExperiments.pluginImport`）已移动到顶层的 [transformImport](/guide/features/builtin-swc-loader#transformimport) 选项。旧的 `rspackExperiments.import` 已废弃，但仍可正常使用以保持向后兼容。
...
### JavaScript `resolve.extensions` 默认不再包含 `.wasm`
