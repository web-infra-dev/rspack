...
内置的 webpack-bundler-analyzer 将被移除，同时 `--analyze` 参数也将不再可用。

如果你需要分析产物体积，可使用分析能力更强的 [Rsdoctor](/guide/optimization/use-rsdoctor) 来替代。
...
[experiments.asyncWebAssembly](/config/experiments#experimentsasyncwebassembly) 的默认值从 `false` 调整为 `true`。异步 WebAssembly 支持默认启用，Rspack 会为常见的 `.wasm` 文件和 `application/wasm` 请求自动注入内置的 `type: 'webassembly/async'` 规则，通常不需要再手动配置 `module.rules`。
