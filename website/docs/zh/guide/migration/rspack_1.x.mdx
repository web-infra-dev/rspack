...
`builtin:swc-loader` 的 `rspackExperiments.import` 已移动到顶层的 [transformImport](/guide/features/builtin-swc-loader#transformimport) 选项。旧的 `rspackExperiments.import` 已废弃，但仍可正常使用以保持向后兼容。
