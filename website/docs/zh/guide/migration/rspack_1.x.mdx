...
`experiments.layers` 已被移除。Layer 功能现在已稳定并默认可用，但仍需要在 `module.rules` 中配置 `layer` 以启用：
...
`experiments.topLevelAwait` 已被移除。Top-level await 功能现在已稳定并默认启用，因此在 ESM 模块中使用 top-level await 会自动生效：
