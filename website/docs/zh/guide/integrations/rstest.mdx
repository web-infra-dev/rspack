---
description: '在 Rspack 项目中集成 Rstest，编写单元测试和端到端测试。'
---
...
适配器默认从 `process.cwd()` 查找 Rspack 配置。它还会关闭 Rstest 内置的 CSS 插件，让 Rspack 的 CSS 配置生效。关于 monorepo、自定义配置路径、具名配置和完整的配置映射，请参阅 [Rstest：Rspack 集成](https://rstest.rs/zh/guide/integration/rspack)。
