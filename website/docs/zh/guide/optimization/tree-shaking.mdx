...
该功能目前处于实验阶段，需要通过配置项开启，且依赖于 [optimization.sideEffects](/config/optimization#optimizationsideeffects)（sideEffects 在生产模式下默认开启）：
...
`pureFunctions` 允许你通过 Rspack 配置文件手动指定某些函数名在特定模块中是无副作用的。这在处理无法修改源码的第三方库（如 `node_modules` 中的包）时非常有用。
...
如果在对应的模块中没有找到 pureFunctions 中指定的函数名，Rspack 会发出警告。
