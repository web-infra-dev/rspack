...
`sideEffectsFree` 允许你在命中的模块中，手动将顶层导出函数标记为“调用无副作用”，供 advanced tree shaking 使用。这在处理无法修改源码的第三方库（例如 `node_modules` 中的包）时非常有用。

:::warning
该选项目前是实验能力，且只有在启用 `experiments.advancedTreeShaking` 时才会生效。
:::
...
Rspack 会校验 `sideEffectsFree` 中配置的每个名字是否确实存在于命中的模块中；如果找不到对应的导出函数名，就会发出警告。
