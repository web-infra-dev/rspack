...
该功能目前处于实验阶段，且依赖于 [optimization.sideEffects](/config/optimization#optimizationsideeffects)（sideEffects 在生产模式下默认开启）。如果你关闭了 `optimization.sideEffects`，pure function 相关提示将不会被分析。
...
配置方式请参考 [`module.parser.javascript.pureFunctions`](/config/module-parser#javascriptpurefunctions)。
