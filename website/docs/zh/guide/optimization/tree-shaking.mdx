...
配置方式请参考 [`experiments.pureFunctions`](/config/experiments#experimentspurefunctions) 和 [`module.parser.javascript.pureFunctions`](/config/module-parser-javascript#moduleparserjavascriptpurefunctions)。

:::warning
该选项目前是实验能力，且只有在启用 `experiments.pureFunctions` 时才会生效。
:::
