...
该功能目前处于实验阶段，由默认开启的 `experiments.pureFunctions` 控制，且依赖于 [optimization.sideEffects](/config/optimization#optimizationsideeffects)（sideEffects 在生产模式下默认开启）：
...
:::warning
该选项目前是实验能力，且只有在启用 `experiments.pureFunctions` 时才会生效；该实验选项默认开启。
:::
