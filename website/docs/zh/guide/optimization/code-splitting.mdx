...
如果在调低 `minSize` 之后，`shared.js` 仍然重复出现，下一步应优先检查 `splitChunks.minSizeReduction`、`splitChunks.minChunks`，以及 `maxAsyncRequests` / `maxInitialRequests` 这类请求数量限制，而不是直接使用 `enforce`。
