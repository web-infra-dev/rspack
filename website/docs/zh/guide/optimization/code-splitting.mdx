---
description: 'Rspack 支持代码分割特性，允许让你对代码进行分割，控制生成的资源体积和资源数量来获取资源加载性能的提升。'
---
...
如果到达某个异步 chunk 的每条路径都保证已加载它需要的模块，Rspack 可以省略重复副本。这发生在 code splitting 阶段，即使设置 `optimization.splitChunks: false` 也会生效。

例如，父 chunk 包含 `shared.js`，并动态导入同样依赖 `shared.js` 的子 chunk，子 chunk 可以复用父级模块。带相同 `webpackChunkName` 的 import 可能在遍历后期才向父 chunk 加入模块，Rspack 也会更新后代的可用模块信息。如果另有一条路径能在未加载 `shared.js` 时到达子 chunk，子 chunk 就必须保留副本。独立入口与 worker 保持各自的加载边界。
...
我们可以通过 [splitChunks.minSize](/plugins/split-chunks-plugin#splitchunksminsize) 配置最小拆分体积为 0，来让 `shared.js` 被单独抽离。
...
如果在调低 [`splitChunks.minSize`](/plugins/split-chunks-plugin#splitchunksminsize) 之后，`shared.js` 仍然重复出现，下一步应优先检查 [`splitChunks.minSizeReduction`](/plugins/split-chunks-plugin#splitchunksminsizereduction)、[`splitChunks.minChunks`](/plugins/split-chunks-plugin#splitchunksminchunks)，以及 [`splitChunks.maxAsyncRequests`](/plugins/split-chunks-plugin#splitchunksmaxasyncrequests) / [`splitChunks.maxInitialRequests`](/plugins/split-chunks-plugin#splitchunksmaxinitialrequests) 这类请求数量限制，而不是直接使用 [`splitChunks.cacheGroups.{cacheGroup}.enforce`](/plugins/split-chunks-plugin#splitchunkscachegroupscachegroupenforce)。

如果你的目标是让体积已经足够大的候选 chunk 在触发这些请求限制后仍然继续拆分，优先考虑 [`splitChunks.enforceSizeThreshold`](/plugins/split-chunks-plugin#splitchunksenforcesizethreshold)，而不是直接对整个 cacheGroup 使用 `enforce`。
...
同时保留默认的 cacheGroups 配置。这样通常可以同时对 initial chunk 和 async chunk 去重，同时 runtime 仍然只会按当前页面实际可达的依赖去加载对应 chunk。

### 强制拆分某些模块

我们可以通过 [`optimization.splitChunks.cacheGroups.{cacheGroup}.name`](/plugins/split-chunks-plugin#splitchunkscachegroupscachegroupname) 强制将指定模块分到一个 Chunk 中去，例如如下配置：
...
更重要的是，`name` 不只是一个文件名选项。固定的 `name` 会把原本可能分属不同候选组的模块强制合并到同一个共享 chunk 中，这可能让某个路由去请求本来不需要的模块依赖链。对大多数项目来说，除非你明确想要一个共享资产并愿意接受这个权衡，否则不要轻易加 `name`。

这个权衡只和 chunk 分组及网络加载边界有关，不会改变 JavaScript 执行顺序，也和 tree shaking 无关。CSS 需要单独看待：在 extracted CSS 场景下，splitChunks 可能影响最终 CSS 顺序。详见 [SplitChunksPlugin 文档](/plugins/split-chunks-plugin#faq)。
...
<Attribution url="https://webpack.docschina.org/guides/code-splitting/" />
