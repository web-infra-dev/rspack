...
如果你的目标是让体积已经足够大的候选 chunk 在触发这些请求限制后仍然继续拆分，优先考虑 [`splitChunks.enforceSizeThreshold`](/config/optimization#splitchunksenforcesizethreshold)，而不是直接对整个 cacheGroup 使用 `enforce`。
