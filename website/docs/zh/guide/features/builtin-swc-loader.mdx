...
- 你需要通过 [exclude](/config/module#ruleexclude) 来排除 `core-js` 包，因为 `core-js` 如果被 SWC 编译，会导致无法正常工作。
- 如果你需要引用非 ES modules 的模块，需要添加 [isModule: 'unknown'](https://swc.rs/docs/configuration/compilation#ismodule) 来允许 SWC 识别模块的类型。
