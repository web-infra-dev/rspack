...
:::info
默认会对所有 `enum` 进行内联优化，若仅对 `const enum` 进行内联请使用 `'const-only'` 并配置 `transform.tsEnumIsMutable = true`，详细示例可参考：[inline const enum 示例](https://github.com/rspack-contrib/rstack-examples/tree/main/rspack/inline-const-enum)
:::
