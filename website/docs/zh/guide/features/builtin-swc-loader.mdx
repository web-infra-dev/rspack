...
通过设置 `autoParser: true`，`builtin:swc-loader` 会根据文件后缀自动配置 `jsc.parser.syntax`：

- 对于 `.ts` 和 `.tsx` 文件（不区分大小写），会自动开启 TypeScript 转译功能
- 对于 `.jsx` 和 `.tsx` 文件（不区分大小写），会自动开启 JSX 转译功能

示例配置如下：

```js title="rspack.config.mjs" {9}
export default {
  module: {
    rules: [
      {
