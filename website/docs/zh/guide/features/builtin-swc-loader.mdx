---
description: 'SWC（Speedy Web Compiler）是基于 Rust 语言编写的高性能 JavaScript 和 TypeScript 转译和压缩工具。'
---
...
:::tip 来自 Rspack 的默认 target
如果既未配置 `env.targets` 也未配置 `jsc.target`，`builtin:swc-loader` 将自动从 Rspack 的 [`target`](/config/target) 配置中推断出默认 target。

这意味着你可以依赖 Rspack 的 `target` 配置，而无需在 loader 选项中手动指定相同的 target。
:::
...
当设置为 `"auto"` 时，`builtin:swc-loader` 会根据资源文件扩展名推断 `jsc.parser`。当一条规则需要同时处理 `.js`、`.jsx`、`.ts`、`.tsx` 等多种模块类型时，这个选项尤其有用。
...
:::warning
`rspackExperiments.import` 已迁移至顶层 [`transformImport`](#transformimport) 选项。`rspackExperiments.import` 选项已废弃，将在未来版本中移除。
:::
...
是否收集类型导出信息，提供给 [`typeReexportsPresence`](/config/module-parser-javascript#moduleparserjavascripttypereexportspresence) 使用，用于 `'tolerant'` 模式下检查子模块的类型导出。
