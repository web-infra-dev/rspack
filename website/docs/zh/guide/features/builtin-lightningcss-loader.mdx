...
`targets` 支持多种写法，它可以设置为 `browserslist` 查询字符串、`browserslist` 查询结果，或是 lightningcss 的 `targets`。
...
- 设置为 lightningcss 的 `targets`，这依赖 lightningcss 内部的 `browserslistToTargets` 方法，它会将 `browserslist` 查询结果转换为 lightningcss 的 `targets` 格式：
