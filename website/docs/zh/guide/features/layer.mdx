---
description: 'Layer 是 Rspack 提供的一个强大特性，允许你对模块进行分类管理。通过为模块分配不同的 layer，你可以针对不同类型的模块执行差异化处理，比如：'
---
...
你可以使用 [`rules[].layer`](/config/module-rules#ruleslayer) 选项为所有匹配规则的模块指定 layer：
...
使用 [`rules[].layer`](/config/module-rules#ruleslayer) 分配的 layer 将覆盖模块本身的 layer，例如某个模块被 Entry 指定了 layer，通过 Rule 分配的 layer 会覆盖 Entry 指定的 layer。
...
你可以使用 [`issuerLayer`](/config/module-rules#rulesissuerlayer) 来筛选特定 layer 的模块，并为它们应用不同的处理规则。以下示例展示了如何为不同 layer 使用不同的编译目标：
