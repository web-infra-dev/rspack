...
如果你正在寻找更多的 Rspack 插件，请查看 [插件](/plugins/) 列表。

你也可以参考[社区插件兼容性](/plugins/community-plugin-compatibility)列表，查看已通过 Rspack 兼容性测试的 webpack 插件。
...
作为插件的作者，插件的结构非常简单：只需要实现一个 `apply` 方法，这个方法接受一个 `Compiler` 实例，并会在 Rspack 插件初始化时被调用。详细的 API 可以参考 [Plugin API](/api/plugin-api/)。
