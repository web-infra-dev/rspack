---
description: '介绍 Rspack 的模块解析过程和基于 enhanced-resolve 的路径解析规则。'
---
...
Rspack 的解析器支持 `package.json` 的 [imports](https://nodejs.org/api/packages.html#subpath-imports) 字段，用于解析包内以 `#` 开头的模块标识符。
...
import main from '#/main'; // 解析为 ./src/main.js
```
