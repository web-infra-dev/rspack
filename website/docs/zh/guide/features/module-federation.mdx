...
在 Rspack 2.0 中，`ModuleFederationPlugin` 也进一步完善了共享依赖 Tree Shaking 能力。你可以通过 `shared.treeShaking` 对共享依赖做按需优化，相关配置可参考 [ModuleFederationPlugin](/plugins/webpack/module-federation-plugin#shared)。
