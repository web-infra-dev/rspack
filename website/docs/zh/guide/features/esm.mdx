...
1. [output.module](/config/output#outputmodule): 设置为 `true`，表示生成 ESM 格式的产物。开启此选项后，会对以下配置产生影响：
   ...
   3. 间接影响 [output.chunkFilename](/config/output#outputchunkfilename) 默认值：该值由 `output.filename` 推断，因此 ESM 输出下默认为 `'[name].mjs'`。自定义 `output.filename` 会影响此默认值；当其为函数时，`output.chunkFilename` 默认为 `'[id].js'`。
