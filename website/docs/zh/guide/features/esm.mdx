...
与应用场景的 `output.module: true` 不同，`modern-module` 会切换到一条专门面向库的 ESM 渲染路径。它会自动启用 `output.module`，并切换到基于 `import` 的 chunk loading，因此通常不需要再手动指定 `chunkFormat: 'module'` 或 `chunkLoading: 'import'`。如果你的库还需要在 Worker 中以 ESM 方式加载 chunk，再额外配置 `workerChunkLoading: 'import'`。`modern-module` 使用自己的链接与 scope hoist 流程，而不是常规的模块拼接插件。但 [`optimization.concatenateModules`](/config/optimization#optimizationconcatenatemodules) 仍用于控制该流程是否可以提升导出可静态分析的 CommonJS 模块：将其设为 `false` 或 `{ commonjs: false }` 可禁用 CommonJS 提升。
...
1. [output.module](/config/output#outputmodule): 设置为 `true`，表示生成 ESM 格式的产物。开启此选项后，会对以下配置产生影响：
   ...
   9. 影响 [output.environment.dynamicImport](/config/output#outputenvironment) 默认值：当 `output.module` 为 `true` 时会被启用
   10. 影响 [output.environment.dynamicImportInWorker](/config/output#outputenvironment) 默认值：当 `output.module` 为 `true` 时会被启用
   11. 影响 [output.environment.module](/config/output#outputenvironment) 默认值：当 `output.module` 为 `true` 时会被启用
...
6. [optimization.concatenateModules](/config/optimization#optimizationconcatenatemodules): 对于 `modern-module`，控制导出可静态分析的 CommonJS 模块是否可以参与其专用的 scope hoist 流程。将其设为 `false` 或 `{ commonjs: false }` 可禁用 CommonJS 提升。
