...
Loader 对模块进行预处理，将它们转换为 Rspack 支持的类型，例如使用 [sass-loader](https://github.com/webpack-contrib/sass-loader) 将 Sass 文件转换为 CSS，或者使用 [builtin:swc-loader](/guide/features/builtin-swc-loader) 将 JSX 文件转换为 JavaScript。而配置 [Rule.type](/config/module#ruletype) 则可以指定如何对 Rspack 已内置支持的模块进行后置处理。
