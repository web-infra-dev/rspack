...
Loader 是一种模块转换器，它将各种类型的模块转换为 Rspack 能够理解和打包的格式。通过 Loader，你可以扩展 Rspack 来支持处理额外的模块类型（如 JSX、Markdown、Sass、Less 等）。

## 内置支持的模块

Rspack 内置了对 JavaScript、CSS、JSON、静态资源等模块类型的支持，可以通过 [Rule.type](/config/module#ruletype) 来配置。

Loader 用于对资源做前置处理，而 `Rule.type` 则用于对资源做后置处理。
...
虽然 Rspack 内置了对 CSS 的支持，但对于 `.less` 文件来说，你需要借助外部的 [less-loader](https://github.com/webpack-contrib/less-loader) 对 `.less` 文件的内容做相应的转换：
