---
description: "在 Rspack 中使用Next.js的指南，涵盖环境配置、集成方式、调试要点、开发模式与常见问题处理。"
---
...
在项目的 `next.config.mjs` 或 `next.config.ts` 中，对现有配置进行包装：
...
在 `next.config.mjs` 中，通过以下回调函数来修改 Rspack 配置：
