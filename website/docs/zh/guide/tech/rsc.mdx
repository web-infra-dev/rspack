...
添加 `builtin:swc-loader` 处理 `.js`、`.mjs`、`.jsx`、`.ts` 和 `.tsx` 文件，并开启 `rspackExperiments.reactServerComponents`，以启用对 `"use client"`、`"use server"`、`"use server-entry"` 等 RSC 语法的解析与转换。
