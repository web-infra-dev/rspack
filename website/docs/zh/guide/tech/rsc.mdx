...
Rspack 通过 `ModuleFederationPlugin.experiments.rsc` 提供一等的 Module Federation + RSC 集成能力。

服务端构建要求：

- `target` 必须包含 `async-node`
- `ModuleFederationPlugin.experiments.asyncStartup` 必须为 `true`
- `ModuleFederationPlugin.runtimePlugins` 必须包含 `@module-federation/node/runtimePlugin`
...
Rspack 通过一对协同工作的插件来支持 React 服务器组件。你需要**调用一次** `rsc.createPlugins()` 得到 `ServerPlugin` 和 `ClientPlugin`，并分别挂到对应的 Compiler 上：
...
当 Server Compiler 在服务端代码中扫描到 `"use client"` 组件时，会把这些模块交给 Client Compiler 构建。由于 Client 侧可以配置多个入口，RSC 插件约定：**与 Server 入口同名的 Client 入口**即视为该 RSC 运行时的挂载点。

扫描到的 `"use client"` 模块会被注入到该同名入口的模块图中。若未配置同名入口，插件会报错「RSC Client Entry Mismatch」，客户端组件将无法在浏览器端正确加载。

## 配置 layer

在 Server Compiler 中，Rspack 使用 [layer](/guide/features/layer) 区分 RSC 与 SSR 环境。RSC 插件使用的层名为：
