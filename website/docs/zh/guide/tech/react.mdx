---
description: '在 Rspack 中使用React的指南，涵盖环境配置、集成方式、调试要点、开发模式与常见问题处理。'
---
...
你可以添加 [builtin:swc-loader](/guide/features/builtin-swc-loader) 以支持 `.js`、`.mjs`、`.jsx`、`.ts` 和 `.tsx` 文件：
...
- 代码注入会注入 `react-refresh` 包中的一些代码，以及一些自定义的运行时代码，都已集成在 [@rspack/plugin-react-refresh](https://github.com/rstackjs/rspack-plugin-react-refresh) 插件中，可通过该插件注入。
...
- 配合 `builtin:swc-loader` 使用方式可参考：[examples/react-refresh](https://github.com/rstackjs/rstack-examples/tree/main/rspack/react-refresh)，使用 `swc-loader` 只需将 `builtin:swc-loader` 换为 `swc-loader` 即可。
- 配合 `babel-loader` 的使用方式可参考：[examples/react-refresh-babel-loader](https://github.com/rstackjs/rstack-examples/tree/main/rspack/react-refresh-babel-loader)
...
如果你需要使用 React 服务器组件，请参考 [Rspack - React 服务器组件](/guide/tech/rsc)。
