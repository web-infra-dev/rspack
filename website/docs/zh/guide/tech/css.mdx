...
默认情况下，内置 CSS 支持使用 `link` export type，将 CSS 作为样式表产物输出。你也可以通过 `module.parser.css.exportType`，或 `css/auto`、`css/global`、`css/module` 的 parser 选项，将 CSS 通过 JavaScript 导出。
