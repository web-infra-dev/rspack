---
description: '分析 Rspack 构建性能、定位常见瓶颈，并调整工作线程配置。'
---
...
请参考 [使用 Rsdoctor](/guide/diagnostics/use-rsdoctor) 了解更多。
...
该命令会生成一个 `.rspack-profile-${timestamp}-${pid}` 文件夹。默认情况下，该文件夹下会包含 `rspack.log` 文件，它由 Rspack 基于 [tracing](https://github.com/tokio-rs/tracing) 生成，并以 JSON Lines 格式记录构建阶段的耗时信息。

`@rspack-debug/core` 是 `@rspack/core` 的诊断版本，包含额外的调试和 tracing 能力。如果使用 `@rspack-debug/core` 并设置 `RSPACK_TRACE_LAYER=perfetto`，profile 目录下会生成 `rspack.pftrace`，可以使用 [ui.perfetto.dev](https://ui.perfetto.dev/) 进行查看。
...
如果你需要使用 Babel 插件实现自定义的转换逻辑，可以在配置 babel-loader 时使用 [rules[].include](/config/module-rules#rulesinclude) 来匹配尽可能少的文件，以减少 Babel 编译的开销。
...
[terser-webpack-plugin](https://github.com/webpack/terser-webpack-plugin) 基于 Terser 来压缩 JavaScript 代码，可以将 Terser 替换为更快的 SWC minimizer，使用 Rspack 内置的 [SwcJsMinimizerRspackPlugin](/plugins/swc-js-minimizer-rspack-plugin) 即可。

### css-minimizer-webpack-plugin

[css-minimizer-webpack-plugin](https://github.com/webpack/css-minimizer-webpack-plugin) 基于 cssnano 等压缩工具来压缩 CSS 代码，可以将 cssnano 替换为更快的 Lightning CSS minimizer，使用 Rspack 内置的 [LightningCssMinimizerRspackPlugin](/plugins/lightning-css-minimizer-rspack-plugin) 即可。
...
[html-webpack-plugin](https://github.com/jantimon/html-webpack-plugin) 在生成大量 HTML 文件时，性能表现不佳，Rspack 基于 Rust 实现的 [HtmlRspackPlugin](/plugins/html-rspack-plugin) 可以提供更好的性能。
...
Rspack 在内部使用 Tokio 线程池和 Rayon 线程池来处理 CPU 密集型任务（Tokio 用于异步任务，Rayon 用于同步任务）。默认情况下，Tokio 和 Rayon 会根据可用的 CPU 核心数自动确定工作线程的数量。

如果在构建过程中消耗了过多的 CPU 资源，可以通过设置 `TOKIO_WORKER_THREADS` and `RAYON_NUM_THREADS` 环境变量来限制工作线程的数量，例如：
...
请注意，设置过低的值可能会导致构建时间延长。设置环境变量后，可以观察构建时间的变化，以找到最合适的配置。
