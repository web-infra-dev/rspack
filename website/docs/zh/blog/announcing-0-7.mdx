...
该配置为 `resolve.aliasFields = ["browser"]` 的简写，在 Rspack 支持 `resolve.aliasFields` 后不再需要该配置。

### 移除 experiments.newSplitChunks

该配置用于开启新的 splitChunks 实现，Rspack 已经默认使用新的 splitChunks 实现，不再需要该配置。

### 移除 generator.css.exportsConvention

该配置用于控制 experiments.css 的 CSS 模块导出的命名形式，仅对于模块类型为 `css/module` 的模块才会有导出，对于模块类型为 `css` 的模块并不存在导出所以不需要该配置。
