...
我们在 0.7.0 版本梳理了一些常见的社区插件的兼容状态及其替代方案，如 `@vanilla-extract/webpack-plugin`、`moment-locales-webpack-plugin` 等目前已兼容，如 `workbox-webpack-plugin`、`webpack-manifest-plugin` 等存在替代方案。
...
在 0.7.0 版本中，我们重新整理了目前暴露的 API 及其接口定义。如果你使用到这些 API，我们希望你进行相应的调整，切换到与 webpack 一致的实现方式。
...
如果你之前配置了 `module.generator.css.exportsConvention` 或在 `module.rule` 中配置了 `generator.exportsConvention`，只需要删除该配置即可。import { experiments } from 'webpack';
