---
description: 'Rspack 0.7 版本发布，支持 lazy compilation，能够显著提升大型应用的 dev startup 性能。同时引入了全新的 css-module-lexer，使 CSS 打包速度提升 4 倍。'
...
现在，你可以通过 [experiments.lazyCompilation](/config/lazy-compilation) 配置项来开启懒编译功能：
...
在 v0.7 版本中，我们对 [experiments.css](/config/deprecated-options#experimentscss) 的内部实现进行了重构。
...
0.7 版本我们对 [experiments.css](/config/deprecated-options#experimentscss) 的内部实现进行了部分重构。
...
Rsbuild v0.7 已与 Rspack v0.7 同步发布，请阅读 [Rsbuild v0.7 发布](https://v0.rsbuild.rs/zh/community/releases/v0-7) 了解更多。
