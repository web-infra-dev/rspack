...
Rsbuild v0.7 版本已经正式发布！

这是 Rsbuild v1.0 版本发布前的最后一个 minor 版本，此后 Rspack 团队将投入到 v1.0 版本的开发中，并致力于尽快推出 Rspack v1.0 alpha 版本。

在 Rsbuild v0.7 中，值得关注的变更如下：
...
Rspack v0.7 支持了 lazy compilation（懒编译），它对于提高多入口应用（MPA）或大型单页面应用（SPA）的构建性能非常有帮助。
...
注意，当前 lazy compilation 是对齐 webpack 实现的，**并且仍处于实验性阶段**。我们将持续完善 lazy compilation 在不同场景下的可用性，使之达到更稳定的状态。如果你在使用过程中遇到任何问题，欢迎通过 [GitHub Issues](https://github.com/web-infra-dev/rspack/issues) 向我们反馈。
