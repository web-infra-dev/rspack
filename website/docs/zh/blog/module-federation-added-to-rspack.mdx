...
我们已经开始将一些[模块联邦示例](https://github.com/module-federation/module-federation-examples)迁移到 Rspack 和 Rsbuild。其中一个值得注意的例子是 CRA 迁移，这是无缝的，只需几分钟即可[从 CRA 切换到 Rsbuild](https://github.com/module-federation/module-federation-examples/tree/master/cra)。这个指南对于 CRA 用户寻求易于使用的性能提升也非常有益：[Rsbuild 迁移指南](/guide/migrate-from-cra)。将 Rsbuild 作为 [Vue 项目从 vue-cli 迁移](https://rsbuild.dev/guide/migration/vue-cli)到更快、更易用和更适合模块联邦的东西也非常合适。
