---
description: 'Rspack 0.4 版本发布，移除对一些内置功能的支持。'
...
此功能主要解决三类问题：[内置](https://v0.rspack.rs/config/builtins) 代码转换功能，[目标](/config/target)，和自定义 [rules[].type](/config/module-rules#rulestype)。
...
3. 移除非 webpack 兼容的 [rules[].type](/config/module-rules#rulestype)
...
查看更多详情，请访问 [这里](/guide/integrations/react#fast-refresh)。
...
迁移示例 [migrate example](https://github.com/rstackjs/rstack-examples/pull/2) 展示了如何从 Rspack 0.3.14 迁移到 Rspack 0.4.0
...
对于 `builtins.html` 可以直接迁移到 [HtmlRspackPlugin](/plugins/html-rspack-plugin)：
...
对于 `builtins.copy` 可以直接迁移到 [CopyRspackPlugin](/plugins/copy-rspack-plugin)。

原先的 `builtins.minifyOptions` 我们提供了 [SwcJsMinimizerRspackPlugin](/plugins/swc-js-minimizer-rspack-plugin)：
