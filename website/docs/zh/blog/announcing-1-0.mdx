...
在字节跳动内部，我们已经基于 Rspack 实验性地支持了 RSC（React Server Components），并在一个大型 web 项目中得到验证。未来 Rspack 将会为 RSC 提供一等公民的支持，提供更多的核心能力来帮助实现 RSC。例如，目前 Rspack 已经支持 [layer](/guide/features/layer) 特性，能够在单次打包时构建出多种环境的产物。
