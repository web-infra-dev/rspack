...
在字节跳动内部，我们已经基于 Rspack 实验性地支持了 RSC（React Server Components），并在一个大型 web 项目中得到验证。未来 Rspack 将会为 RSC 提供一等公民的支持，提供更多的核心能力来帮助实现 RSC。例如，目前 Rspack 已经支持 [layer](https://rspack.dev/config/experiments#experimentslayers) 特性，能够在单次打包时构建出多种环境的产物。
...
1.0 的发布，意味着 Rspack 实现了 webpack 的核心功能，API 达到稳定。在未来 12～18 个月内，我们会保证 Rspack 1.x API 的稳定性，开发者可以放心地基于 Rspack API 开发上层的框架和工具。在 1.x 的迭代期间，我们仍然可能会发现一些 Rspack 不合理的设计，我们将通过 [future flags](https://rspack.dev/config/experiments#experimentsrspackfuture) 的方式，实现渐进式地升级。
...
Rspack 提供了一些实验性特性，这些特性可以通过 [experiments](https://rspack.dev/config/experiments) 配置项使用。在 minor release 中，Rspack 可能对这些实验性特性的 public API 做一些调整，并在更新日志中对这些变动进行详细的说明。因此，如果你使用了实验性特性，请留意 minor 版本的更新日志。
