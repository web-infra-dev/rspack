---
description: 'Rspack 和 Rsbuild 1.1 版本发布，显著提升了冷启动和增量构建的性能，同时改进了内置的 HTML 插件和配置选项的类型提示。'
...
在 Rspack v1.1 中，我们引入了 [experiments.incremental](/config/deprecated-options#experimentsincremental) 作为 `experiments.incrementalRebuild` 的后继者和超集，旨在为开发者带来进一步的重新构建时的性能提升和更快的 HMR。
...
> 更多详细信息请参考 [experiments.incremental](/config/deprecated-options#experimentsincremental)。
...
> 更多详细信息请参考 [HtmlRspackPlugin](/plugins/html-rspack-plugin)
