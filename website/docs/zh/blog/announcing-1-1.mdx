...
在 Rspack 0.x 的早期版本中，我们实现了[experiments.incrementalRebuild](https://v0.rspack.dev/config/experiments#experimentsincrementalrebuild)。随着这个功能逐渐稳定，我们移除了实验性配置并默认启用。
...
在 Rspack v1.1 中，您可以在 `dev` 模式下通过以下方式启用此功能：
...
目前这是一个实验性功能，还需更多的验证以保证其稳定。如果您感兴趣可以尝试一下，并将错误报告和反馈提交至[#8106](https://github.com/web-infra-dev/rspack/issues/8106)。
...
因此，我们大幅重构了 `HtmlRspackPlugin`，在对齐了更多 `HtmlWebpackPlugin` 配置项的同时，也完成了 `hooks` 对齐。现在您可以通过 `HtmlRspackPlugin.getCompilationHooks` 来获取它们，并使用其修改 HTML 产物的内容：
...
> 更多详细信息请参考 [HtmlRspackPlugin](https://rspack.dev/plugins/rspack/html-rspack-plugin)
...
> 我们仍在寻求改进 JSDoc。如果您感兴趣，请随时提交 PR。❤️

## 生态系统

### Docusaurus 构建提速

[Docusaurus v3.6](https://docusaurus.io/blog/docusaurus-v3.6) 带来了`Docusaurus Faster` 选项，允许用户将 Rspack 用作 Docusaurus 站点的打包工具。

[Docusaurus Faster](https://docusaurus.io/blog/releases/3.6#docusaurus-faster) 的目标是减少 Docusaurus 的构建耗时和内存占用。Docusaurus 团队致力于多项优化，并使用像 Rspack 和 SWC 这样基于 Rust 的高性能工具对其基础设施进行了现代化改造。

在对其社区站点的性能测试中，生产模式下的站点的构建速度可以提速 2 到 4 倍。

### Nuxt Rspack 构建器

[Nuxt v3.14](https://nuxt.com/blog/v3-14) 将 Rspack 作为 Nuxt 内置支持的构建器。

它仍然处于实验阶段，Nuxt 团队重构了内部的虚拟文件系统来支持 `unplugin` 以使得它成功运行。

您可以尝试 [nuxt-rspack-starter](https://github.com/danielroe/nuxt-rspack-starter) 作为初始项目，或者安装 [@nuxt/rspack-builder](https://www.npmjs.com/package/@nuxt/rspack-builder) 并在 Nuxt 配置文件中设置 `builder: 'rspack'`。
...
Rsbuild 现在可通过 [dev.cliShortcuts](https://rsbuild.dev/config/dev/cli-shortcuts) 启用 CLI 快捷方式。如果您已在使用 Rsbuild CLI，则该功能默认开启。

CLI 快捷方式允许您清空控制台、打开浏览器、重启服务器或自定义任何您想要的注释。
...
Rsbuild 开发服务器现在会在 `/rsbuild-dev-server` 路径下提供了一个报告页面，允许您查看当前构建期间生成的所有静态资源。
...
- [source.assetsInclude](https://rsbuild.dev/config/source/assets-include)：设置应被视为静态资源的额外文件。
