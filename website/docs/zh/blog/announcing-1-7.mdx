...
### Rspack CLI 默认开启 Lazy Compilation

继 [LazyCompilation](/zh/config/lazy-compilation) 特性在 1.5.0 发布稳定后，
Rspack CLI [dev](/zh/api/cli#rspack-dev) 命令将会按以下配置默认开启该特性；`@rspack/core` API 保持默认关闭，需手动启用。
...
```
:::note
目前默认仅对应用中的动态引用 `import('module')`，采用的懒编译的方式加载。
:::

在多入口应用和动态加载路由的 SPA 大型应用中，Lazy Compilation 通过按需编译需要的模块，能显著的降低应用的冷启动时间和首次可交互时间。
从 Rspack CLI 1.7 开始，Lazy Compilation 功能将在 Web 应用的项目中默认开启后，我们将获得**更快的 Dev server 的启动速度**和**更低的初始 CPU/内存占用**。
...
- **TypeScript enum 内联优化**：该优化已正式稳定。
  ...
  - 可以通过 [collectTypeScriptInfo.exportedEnum](/guide/features/builtin-swc-loader#collecttypescriptinfoexportedenum) 控制是否收集导出的 `enum` 信息
  - 可以通过 [optimization.inlineExports](/config/optimization#optimizationinlineexports) 来控制是否内联 `enum`
- **类型重导出检查**：该优化已正式稳定。
...
Rsdoctor 1.4 带来了全新的 Tree Map 视图，能帮助开发者**更直观地**分析 bundle 的构成，以及**资源（Assets）和模块（Modules）的占比**。同时支持搜索模块资源，点击模块资源可以放大到该模块区域。双击模块可以展开**模块依赖链**等信息
...
自 Rslib 0.19 起，该插件在 bundle 模式下会被默认启用。开发者无需任何额外配置，即可直接获得高质量、对静态分析更友好且完整支持代码分割的 ESM 产物。

#### 全新的 JavaScript API

Rslib 0.19 版本引入了全新的 [JavaScript API](https://rslib.rs/api/start)，以便于开发者在 JavaScript 或 TypeScript 代码中直接使用 Rslib 的功能。使用方式如下：
