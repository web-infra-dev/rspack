---
description: 'Rspack 1.7 版本发布，提升了 SWC Wasm 插件兼容性，支持 Import Bytes 资源导入，并将多项实验特性稳定化。'
...
为缓解这一问题，我们向 SWC 社区贡献了一系列 [兼容性改进](https://swc.rs/docs/plugin/ecmascript/compatibility)，包括：
...
Rspack 现已原生支持 [Import Bytes](https://github.com/tc39/proposal-import-bytes) 提案，可以将静态资源作为 `Uint8Array` 导入，并通过 `TextDecoder` 解码。
...
在 Rspack 1.5 中，我们完成了 [Lazy Compilation](/guide/advanced/lazy-compilation) 特性的稳定化，并在 Rsbuild 中默认对动态导入的模块启用了按需编译。
...
如果你有特殊需求，例如需要调试完整构建的产物，或分析完整的模块图，可以通过将 [lazyCompilation](/config/lazy-compilation) 设置为 `false` 来显式关闭该功能。
...
- **常量内联优化**：该优化已正式稳定，并在生产构建中默认启用。
  - 原有的 [experiments.inlineConst](/config/optimization#optimizationinlineexports) 选项已废弃
  - 如需关闭该行为，可通过 [optimization.inlineExports](/config/optimization#optimizationinlineexports) 来控制
- **TypeScript enum 内联优化**：该优化已正式稳定。
  - 原有的 [experiments.inlineEnum](/config/optimization#optimizationinlineexports) 选项已废弃
...
- **类型重导出检查**：该优化已正式稳定。
  - 原有的 [experiments.typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) 选项已废弃
...
在创建 Rsbuild 项目时，你现在可以选择自动集成 [Rstest](https://rstest.rs/) 作为测试框架，或启用 [Storybook](https://storybook.rsbuild.rs/) 用于 UI 组件的开发与调试。相关依赖和配置都会在初始化时一次性完成，减少重复的手动配置成本。
...
Rsdoctor 1.4 对原有的 [Treemap](https://rsdoctor.rs/zh/guide/usage/bundle-size) 视图进行了改进。新的交互设计可以帮助你更直观地分析 bundle 的整体构成，以及各类资源和模块的体积占比。
...
在此前的版本中，Rslib 通过实验性配置 [experiments.advancedEsm](https://rslib.rs/zh/config/lib/experiments#experimentsadvancedesm) 来启用 Rspack 的 [新版 ESM 库产物](https://v1.rspack.rs/plugins/rspack/esm-library-plugin)，用于提升 ESM 产物的整体质量。经过多个版本的验证与打磨，该能力现已完成稳定化。

自 Rslib 0.19 起，Rslib 的 bundle 模式将默认启用该特性。开发者无需进行任何额外配置，即可直接获得对静态分析更友好、并完整支持代码分割的 ESM 产物。

#### JavaScript API

Rslib 0.19 引入了 [JavaScript API](https://rslib.rs/api/start)，使开发者可以在 JavaScript 代码中以编程方式直接调用 Rslib 的构建能力。
