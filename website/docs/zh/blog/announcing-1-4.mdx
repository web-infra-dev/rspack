...
在过去几个月中，Rspack 团队致力于与 SWC 团队合作，共同优化 JavaScript 工具链的性能和可靠性。经过双方团队和社区开发者的共同努力，我们很高兴地看到，SWC 在性能方面取得了显著的提升：
...
在最新版本中，SWC 还改进了死代码消除（DCE），这项改进与 Rspack 的 tree shaking 相结合，使得 Rspack 1.4 能够生成体积更小的构建产物。

我们以 `react-router` 为例进行测试：在源代码中仅引入其部分功能，然后对比不同打包工具的构建结果，可以发现 Rspack 生成的产物尺寸更小。
