...
[a0d8edff-ee60-498e-aec8-bba77b42de81.mp4]
...
_Author [@ahabhgk](https://github.com/ahabhgk) @何庚坤_

在 Rspack v1.4.0 中，**增量构建功能已默认开启所有阶段的优化**（'advance-silent' 模式），这将为开发者带来显著的重构建速度提升和更流畅的热模块替换 (HMR) 体验。

增量构建开启后一般来说能够有 **30%-40%** 左右的重构建速度提升，具体需视项目情况而定。如果您需要保留之前的行为，可以通过配置 `experiments.incremental: 'safe'` 来恢复。但我们建议大多数项目直接使用新的默认设置，以获得最佳性能。具体文档请参考 [experiments.incremental](https://rspack.dev/config/experiments#experimentsincremental)。

### 升级 SWC

在 Rspack 1.4.0，我们将底层编译工具 SWC 升级至 [**v29.1.2**](https://crates.io/crates/swc_core/29.1.2)！此次升级带来：

- JavaScript / TypeScript 的编译速度显著提升
- 代码压缩效率提高

⚠️ **重要 Breaking change**

若您的项目中使用了 SWC Wasm 插件（如 `@swc/plugin-emotion` 等），必须将插件升级至兼容 `swc_core@29.1.2` 的版本，否则可能因版本不兼容导致构建报错。详情请查阅：[SWC Plugin Version Unmatched](https://rspack.dev/errors/swc-plugin-version)
...
若您的项目中使用了 SWC Wasm 插件（如 `@swc/plugin-emotion` 等），必须将插件升级至兼容 swc_core@29.1.2 的版本，否则可能因版本不兼容导致构建报错。

> 详情请查阅：[SWC Plugin Version Unmatched](/errors/swc-plugin-version)
