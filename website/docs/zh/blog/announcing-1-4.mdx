...
经过持续优化，Rspack 的增量构建功能已趋于稳定，在 Rspack 1.4 中，已默认开启所有阶段的增量优化。这将显著提升重构建速度，HMR 性能通常可提升 **30%-40%**，具体效果因项目而异。
...
我们正在积极开发 [Rspress 2.0](https://github.com/web-infra-dev/rspress)，并已发布多个 beta 版本。目前已完成代码层面的大部分重构，默认集成 [Shiki](https://shiki.style/) 提供更强大的代码高亮功能。

与此同时，我们正在开发全新的主题，预览如下：
...
Rsdoctor 结合人工智能能力推出了 [@rsdoctor/mcp-server](https://rsdoctor.rs/zh/guide/start/mcp)，旨在协助用户更便利地运用 Rsdoctor 的构建数据。该服务器可与 Rsdoctor 的本地构建分析数据协同使用，以问答形式助力用户迅速获取构建分析结果。
