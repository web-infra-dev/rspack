---
description: 'Rspack 1.4 版本发布，支持在浏览器中运行、默认启用增量构建，同时集成更快的 SWC、生成更小的构建产物，并引入了 `CssChunkingPlugin` 等新功能。'
...
如果你需要降级到之前的行为，可以设置 [experiments.incremental](/config/deprecated-options#experimentsincremental) 为 `'safe'` ，但我们推荐大部分项目直接使用新的默认配置，以获得最佳性能。
...
Rspack 1.4 新增了实验性的 [CssChunkingPlugin](/plugins/css-chunking-plugin) 插件，专门用于处理 CSS 代码分割。该插件能够确保样式的加载顺序与源代码中的导入顺序保持一致，避免因 CSS 加载顺序错误而导致的 UI 问题。
...
启用 `CssChunkingPlugin` 后，CSS 模块的代码分割将完全由该插件处理，`optimization.splitChunks` 配置将不再对 CSS 模块生效，你可以查看 [使用文档](/plugins/css-chunking-plugin) 了解更多。
...
Rspack 现已支持在 `MultiCompiler` 中启用 lazy compilation，这意味着当你在单次构建中使用多份 Rspack 配置时，可以为不同的 compiler 实例独立设置各自的 [lazyCompilation 选项](/config/lazy-compilation)。
...
通过引入 [rsbuild-plugin-unplugin-vue](https://github.com/rstackjs/rsbuild-plugin-unplugin-vue) 插件，你可以使用 Rslib 生成 Vue 组件库的 bundleless 产物。
...
Rsdoctor 推出了 [@rsdoctor/mcp-server](https://rsdoctor.rs/zh/guide/usage/mcp)，结合大模型来帮助你更好地分析构建数据。它能调用 Rsdoctor 的本地构建分析数据，提供智能的分析和优化建议。
...
Lazy compilation 中间件的接入方式有所变化，该中间件现在可以从 compiler 实例中自动读取 [lazyCompilation](/config/lazy-compilation) 选项，因此你不再需要手动传入 `lazyCompilation` 选项。
