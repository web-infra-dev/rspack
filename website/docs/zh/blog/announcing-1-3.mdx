...
- 新功能
  - [循环引用检测](#循环引用检测)：引入插件来检测模块间的循环依赖
  - [支持 `experiments.buildHttp`](#支持-experimentsbuildhttp)：在构建时自动将 HTTP/HTTPS 资源下载到本地，然后将它们进行打包
...
### 支持 `experiments.buildHttp`

在 Rspack 中，你可以通过 [externalsPresets.web](/config/externals#externalspresetsweb) 或 [externalsPresets.webAsync](/config/externals#externalspresetswebasync) 来导入 HTTP/HTTPS 资源，这只是将 HTTP/HTTPS 资源设置为外部资源，然后让浏览器（或其他平台）在运行时获取它们。
...
Rspack 1.3 版本引入了对 [output.environment](/config/output#outputenvironment) 选项的完整支持，这允许你指定在 Rspack 生成的运行时代码中可使用的 ECMAScript 特性，能够生成更精简、更现代的运行时代码。

默认情况下，Rspack 会解析 [target](/config/target) 配置项，并基于 `browserslist` 来判断目标环境所支持的 ECMAScript 特性，自动设置 `output.environment` 子选项的值，从而输出最优的代码。
...
2. 我们在 macOS 上默认使用了 [mimalloc](https://github.com/microsoft/mimalloc) v3。这缓解了macOS 在 rebuild 过程中的一些内存消耗问题。根据一些社区和内部项目的反馈，这将减少 rebuild 时的 RSS 使用量，具体减少的比例因项目大小而异，根据我们的测试，减少的比例从 10% 到 85% 不等。
