...
在组织项目代码时，我们通常会将常量集中管理，例如 `constants.js` 或者 TypeScript 项目中包含 `enum` 的 `types.ts` 文件。
...
> 更多可参考 [experiments.inlineConst 文档](/config/experiments#experimentsinlineconst)，该功能计划在 v1.6 中默认启用。

`inlineEnum` 会对 TypeScript 的 `enum` 进行跨模块内联优化，工作原理与 `inlineConst` 类似。
...
需要注意的是，启用 `inlineEnum` 后，Rspack 默认会内联所有 `enum`。如果只希望内联 `const enum`，可以通过配置 [`'const-only'` 和 `tsEnumIsMutable: true`](https://github.com/rspack-contrib/rstack-examples/tree/main/rspack/inline-const-enum) 来实现。

> 更多可参考 [experiments.inlineEnum](https://rspack.rs/config/experiments#experimentsinlineenum)。
