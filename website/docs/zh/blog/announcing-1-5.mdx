...
- [常量内联](#常量内联)
- [#barrel files 构建优化](##barrel-files-构建优化)
...
![repack-web-repl](https://assets.rspack.rs/rspack/assets/rspack-v1-5-repl.rspack.png)
...
未来我们将持续完善在线打包所需的能力，欢迎前往 [https://repl.rspack.rs/](https://repl.rspack.rs/) 进行体验。
...
[experiments.lazyBarrel](/config/experiments#experimentslazybarrel) 功能，能够对无副作用的 barrel files 的 `reexports` 进行懒构建，
避免因重导出引入过多模块到构建过程中，减少模块解析和构建的性能开销，该功能能够大幅提升存在大量 barrel files 项目的构建性能。

[build-tools-performance/ui-components](https://github.com/rspack-contrib/build-tools-performance/tree/main/cases/ui-components)
在开启 `experiments.lazyBarrel` 后：

- 构建时间由 1.47s 减少到 1.19s，提升约 20%
- 模块路径解析次数由 39675 次减少到 20071 次
- 模块构建次数由 9358 次减少到 5062 次

内部某大型项目在开启 `experiments.lazyBarrel` 后:

- 构建时间由 17.943s 减少到 16.004s，提升约 10%
- 模块路径解析次数由 181078 次减少到 137232 次
- 模块构建次数由 38046 次减少到 29405 次。

更多可参考 [`experiments.lazyBarrel` 文档](/config/experiments#experimentslazybarrel)， 计划在 v1.6.0 默认开启该功能。
...
在 Rslib v0.12 版本中，我们在模板中添加了 Rstest 作为配套的测试框架，库开发者现在可以使用同一套 Rstack 工具链进行开发与测试。
...
Rspack 构建时会将部分模块提升合并为 ***concatenated module***，以优化体积和性能，但此前 Rsdoctor 无法对其内部结构做进一步 AST 拆解与分析。

Rsdoctor 1.2 版本新增对 ***concatenated module*** 的深入分析，帮助开发者准确获知 Tree Shaking 和压缩后子模块的实际体积，便于深入分析产物细节。
