...
现在你可以**直接在任何现代浏览器中使用 Rspack**了。 新发布的 [@rspack/browser](/api/javascript-api/browser) 是专为纯浏览器环境设计的版本，而无需依赖 WebContainers 或是特定平台。`@rspack/browser` 为复杂 Web 项目的在线打包提供底层支持，提供了更轻量的问题复现、配置分享方案，并通过提供在线交互式 Demo 的方式帮助开发者入门和学习 Rspack。
