---
description: 'Rspack 1.5 版本发布，支持 Barrel 文件优化、常量内联优化，新增了内置文件系统监听器、虚拟模块插件和 Rust 扩展机制，并不再支持 Node 16。'
...
为了解决这一问题，Rspack 1.5 引入了实验性的 [lazyBarrel](/guide/optimization/lazy-barrel) 功能，它能够自动识别无副作用的 barrel 文件，对其中的重导出进行延迟构建优化，只在真正需要时才会解析和构建相关模块，从而显著减少不必要的模块解析和构建开销。这一优化对于包含大量 barrel 文件的项目尤其有效，能够带来显著的构建性能提升。
...
- [Benchmark 应用](https://github.com/rstackjs/build-tools-performance/tree/main/cases/ui-components)：
...
我们已经在 Rsbuild 1.5 中默认开启了 barrel 文件优化，并计划在 Rspack 1.6 中为所有项目默认启用该功能。详见 [experiments.lazyBarrel 文档](/guide/optimization/lazy-barrel)。
...
你可以使用 [官方模板](https://github.com/rstackjs/rspack-binding-template) 快速开始，更多信息请参考 [设计理念](https://rstackjs.github.io/rspack-rust-book/custom-binding/getting-started/rationale.html)。需要注意的是，使用此方案会带来额外的维护成本，建议仅在需要极致性能优化时使用。

### 常量内联优化

在组织项目代码时，我们通常会将常量集中管理，例如 `constants.js` 或者 TypeScript 项目中包含 enums 的 `types.ts` 文件。

Rspack 引入了 [experiments.inlineConst](/config/optimization#optimizationinlineexports) 和 [experiments.inlineEnum](/config/optimization#optimizationinlineexports) 两个实验性功能，用于对常量进行跨模块内联优化。这些优化可以帮助压缩工具更加进行准确的静态分析，消除无用代码分支，从而进一步减小产物的体积。
...
> 详见 [experiments.inlineConst 文档](/config/optimization#optimizationinlineexports)，该功能计划在 v1.6 中默认启用。
...
需要注意的是，启用 `inlineEnum` 后，Rspack 默认会内联所有 enums。如果你希望只内联 const enums，可以参考此 [示例](https://github.com/rstackjs/rstack-examples/tree/main/rspack/inline-const-enum)。

> 详见 [experiments.inlineEnum](/config/optimization#optimizationinlineexports)。
...
Rspack 1.5 引入了 [experiments.typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) 配置，用于改进对 TypeScript 类型导出的识别。开启此配置后，Rspack 能够正确识别跨模块分析类型重导出，从而避免误报警告。

> 详见 [experiments.typeReexportsPresence](/config/module-parser#javascripttypereexportspresence)。

### 内置虚拟模块插件

在 Rspack 1.4 中，我们引入了[自定义 InputFileSystem](/blog/announcing-1-4#自定义文件读取系统) 功能，配合 `webpack-virtual-modules` 插件可以支持虚拟模块。然而，当虚拟模块数量较大时，该方式仍然存在性能瓶颈。

为了更好地支持虚拟模块，Rspack 1.5 新增了内置的 [VirtualModulesPlugin](/plugins/virtual-modules-plugin)，它基于 Rust 实现，将虚拟模块的存储和管理迁移到 Rust 层，有效减少了模块读取和解析的开销，在处理大量虚拟模块时能够保持更好的性能表现。
...
为了让 Rspack 用户能够更便捷地使用 Rspack 的模块解析功能，我们已将 [rspack-resolver](https://github.com/rstackjs/rspack-resolver) 集成到 Rspack 的 JavaScript API 中，它提供了类似 [enhanced-resolve](https://github.com/webpack/enhanced-resolve) 的模块解析能力。

使用方法请参阅 [Resolver API 文档](/api/javascript-api/resolver#resolver-api)。

### 稳定化 lazy compilation

经过充分验证，[experiments.lazyCompilation](/config/lazy-compilation) 配置项已从实验性特性升级为稳定特性，并移动到了 Rspack 配置顶层：
...
Rspack 的 [experiments.topLevelAwait](/guide/migration/rspack_1.x#其他实验性配置变更) 选项用于控制对 top level await 的支持，一直以来都是默认开启的。经过观察，我们发现没有实际场景需要关闭 top level await 支持，因此我们决定废弃这个选项，并计划在 Rspack 2.0 中移除它，届时将无法禁用 top level await 支持。
...
- 启用 [lazyBarrel](/guide/optimization/lazy-barrel) 来优化 barrel 文件的构建速度，减少不必要的模块解析。
- 启用 [inlineEnum](/config/optimization#optimizationinlineexports) 来内联 TypeScript 枚举，这可以减少枚举被编译后的包体积。
- 启用 [typeReexportsPresence](/config/module-parser#javascripttypereexportspresence) 来正确识别 TypeScript 类型重导出，提升类型处理的准确性。
...
- **Mock API**：Rstest 现在提供了完整的 [mock API](https://rstest.rs/zh/api/runtime-api/rstest/mock-modules)，用于在测试环境中替换模块的实际实现，支持对 ES 模块进行模拟操作。
