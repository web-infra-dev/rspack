...
- 新功能
  ...
  - [@rspack/browser 发布](#rspackbrowser-发布)
  ...
  - [安装体积优化](#安装体积优化)
  - [Seal 阶段性能提升](#seal-阶段性能提升)
- 其他
  - [停止支持 Node.js 16](#停止支持-nodejs-16)
...
此前，Rspack 使用文件系统监听器 [watchpack](https://github.com/webpack/watchpack) 来监听文件变化。在实际使用中，我们发现 `watchpack` 存在性能瓶颈。例如，每次文件变更都会重新创建实例，在大型项目中会消耗大量 CPU 和内存资源（参见 [#7490](https://github.com/web-infra-dev/rspack/issues/7490)）。

为解决这一问题，我们基于 Rust 重新设计和实现了原生的文件系统监听器，新的实现具有以下优势：
...
现在你可以**直接在任何现代浏览器中使用 Rspack**了。 新发布的 [@rspack/browser](https://www.npmjs.com/package/@rspack/browser) 是专为纯浏览器环境设计的版本，而无需依赖 WebContainers 或是特定平台。`@rspack/browser` 为复杂 Web 项目的在线打包提供底层支持，提供了更轻量的问题复现、配置分享方案，并通过提供在线交互式 Demo 的方式帮助开发者入门和学习 Rspack。
...
在组织项目代码结构时，我们经常将常量统一放到单独的模块中进行管理，例如 JavaScript 项目中的 `constants.js` 或者 TypeScript 项目中统一组织 `enum` 的 `types.ts`

Rspack 提供了 [`experiments.inlineConst`](/config/experiments#experimentsinlineconst) 和 [`experiments.inlineEnum`](/config/experiments#experimentsinlineenum) 两个新功能来对这些常量进行跨模块内联优化，帮助压缩工具更加进行准确的静态分析，消除无用代码分支，从而进一步减小产物的体积。

`inlineConst` 能够对模块图中叶子节点模块中的常量进行跨模块内联优化，帮助压缩工具更进一步的进行静态分析，消除无用代码分支，从而进一步减小最终产物的体积。
...
开启 `inlineConst` 使用 Rspack 打包，示例中的 `if` 分支能够更加明确的被压缩工具优化，得到更加精简的产物：
...
`inlineEnum` 会对 TypeScript `enum` 进行跨模块的内联优化，该功能与 `inlineConst` 类似。
...
开启 `inlineEnum`后， `builtin:swc-loader` 默认会对所有 `enum` 都进行内联，可通过 [`'const-only'` 和 `tsEnumIsMutable: true`](https://github.com/rspack-contrib/rstack-examples/tree/main/rspack/inline-const-enum) 来控制仅对 `const enum` 进行内联优化。
...
之前 Rspack 在处理 TypeScript 类型重导出时会报 `ESModulesLinkingWarning` 警告，例如 `ESModulesLinkingWarning: export 'MyType' (reexported as 'MyType') was not found`。这是因为 Rspack 在解析模块时采用独立隔离的方式处理每个模块，导致类型的导出(例子中的 `MyType`) 被错误识别为一个值而非类型，由于 `./types.ts` 中找不到相应的值导出，因此最终会抛出 `ESModulesLinkingWarning` 警告。

Rspack 1.5 新增 [`experiments.typeReexportsPresence`](/config/experiments#experimentstypereexportspresence) 配置来增强对 Typescript 类型导出的识别。 开启该配置后，Rspack 能够正确识别 `./types.ts` 中存在类型 `MyType` 的导出，从而避免误报警告，提升了 TypeScript 的研发体验。
...
自 1.4 以来，我们做了一系列的优化来降低 Rspack 的安装体积，目前已经有了显著的效果。

![binary-size-by-dates](https://assets.rspack.rs/rspack/assets/rspack-v1-5-rspack-install-size.png)

其中几个效果显著的优化：

- 通过修改 [编译参数](https://github.com/web-infra-dev/rspack/pull/11077) 足足减少 5M 的安装体积
- 通过优化上游依赖 [wasmer](https://github.com/wasmerio/wasmer/pull/5621/files) 和 [SWC](https://github.com/swc-project/swc/pull/10638) 带来了近 3M 的优化。
- 通过控制 [feature 开关](https://github.com/web-infra-dev/rspack/pull/10965) 带来了近 2M 的尺寸优化；
- 通过优化 [browserlist-rs](https://github.com/browserslist/browserslist-rs/pull/32)数据结构，为 Rspack 减少了约 2M 安装体积

目前 Rspack 已经将产物尺寸纳入到日常代码质量[检查](https://github.com/web-infra-dev/rspack/blob/main/.github/actions/binary-limit/action.yml)中，未来我们会持续关注产物尺寸的变化。

### Seal 阶段性能提升

在构建性能方面，Rspack 1.5 针对 Seal 阶段（代码生成和优化的阶段）进行了大量优化，通过优化数据结构、提升并行度、增加热点代码缓存等手段，提升了大型项目的构建性能。得益于并行化优化，在多核机器上性能提升更为显著。

以字节跳动某大型应用为例，该项目包含约 40,000 个模块，整体 Seal 阶段耗时降低约 50%。
...
鉴于 Node.js 16 已于 2023 年 9 月 11 日停止维护，同时众多社区 npm 包（如 `webpack-dev-server`、`css-loader`、`sass-loader` 等）也相继停止了对 Node.js 16 的支持，为了降低维护复杂度，Rspack 1.5 将不再支持 Node.js 16。
...
- **Mock 模块支持**：Rstest 现在提供了完整的 mock API，用于在测试环境中替换模块的实际实现，支持对 ES 模块进行模拟操作。

![Mock 模块](https://assets.rspack.rs/rspack/assets/rspack-v1-5-rstest-mock-modules.png)

- **Watch 模式优化**：在 watch 模式下，Rstest 现在支持增量 re-run。当测试文件或其依赖的模块发生变化时，Rstest 会只重新运行相关的测试文件，从而提升测试执行效率。
