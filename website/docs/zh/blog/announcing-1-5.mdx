...
现在你可以直接使用 Rust 来扩展 Rspack 了！在官方提供的模板中，开发者能够轻松编写自定义的 Rust Plugin 和 Rust Loader。
...
在 Web 项目中，通常会将公共常量统一维护在 `constants.js` 文件中，并在多个模块中引用。为优化此类场景，Rspack 提供了 `experiments.inlineConst` 功能。
...
`inlineEnum` 会对 TypeScript `enum` 进行跨模块的内联优化，该功能与 `inlineConst` 类似，能够有效帮助压缩工具更进一步的进行静态分析，消除无用代码分支，从而进一步减小最终产物的体积。
...
`builtin:swc-loader` 默认会对所有 `enum` 都进行内联，可通过 [`'const-only'` 和 `tsEnumIsMutable: true`](https://github.com/rspack-contrib/rstack-examples/tree/main/rspack/inline-const-enum) 来仅对 `const enum` 进行内联优化。

更多详情可参考 [experiments.inlineEnum](https://rspack.rs/config/experiments#experimentsinlineenum)。

`typeReexportsPresence` 优化了存在历史包袱而没有使用 [`isolatedModules`](https://www.typescriptlang.org/tsconfig/#isolatedModules) 的项目的开发体验。之前 Rspack 在处理 TypeScript 类型重导出时会报告 `ESModulesLinkingWarning`，这是因为 Rspack 在解析模块时采用独立隔离的方式处理每个模块，导致 `MyType` 被错误识别为一个值而非类型，由于 `./types.ts` 中找不到相应的值导出，因此最终会抛出 `ESModulesLinkingWarning` 警告。

开启 `typeReexportsPresence` 后，Rspack 能够正确识别 `./types.ts` 中存在类型 `MyType` 的导出，从而避免误报警告。

```ts
// 之前会报 ESModulesLinkingWarning，开启 typeReexportsPresence 后不会误报警告
...
在 Rspack 1.4 中，我们支持了[自定义的 InputFileSystem](https://rspack.rs/blog/announcing-1-4#custom-input-file-system)。
