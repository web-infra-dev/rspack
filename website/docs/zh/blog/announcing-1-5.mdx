...
为了解决这一问题，Rspack 1.5 引入了实验性的 [lazyBarrel](/guide/advanced/lazy-barrel) 功能，它能够自动识别无副作用的 barrel 文件，对其中的重导出进行延迟构建优化，只在真正需要时才会解析和构建相关模块，从而显著减少不必要的模块解析和构建开销。这一优化对于包含大量 barrel 文件的项目尤其有效，能够带来显著的构建性能提升。
...
我们已经在 Rsbuild 1.5 中默认开启了 barrel 文件优化，并计划在 Rspack 1.6 中为所有项目默认启用该功能。详见 [experiments.lazyBarrel 文档](/guide/advanced/lazy-barrel)。
...
- 启用 [lazyBarrel](/guide/advanced/lazy-barrel) 来优化 barrel 文件的构建速度，减少不必要的模块解析。
