...
Rspack 1.0 默认在 production 构建时开启 `optimization.concatenateModules` 优化项，它用于启用 module concatenation，也被称为作用域提升（Scope Hoisting）。

Module concatenation 的作用是将多个模块合并到一个函数中，从而减少浏览器解析和执行 JavaScript 代码的开销。合并模块后，可以减少一些冗余的代码，比如模块间的导入和导出语句，这样可以进一步减小打包后的产物体积。

开启 module concatenation 后，Rspack 的产物体积可以减少约 4% ～ 10%（Gzip 前）。
