...
之前默认转换是内置的，它将源文件（如 TypeScript）内部转换为兼容的源文件（如 JavaScript）。为了使转换更加可定制，我们通过使用 `builtin:swc-loader` 将此功能交给用户控制，并放弃了对几个 [rule.type](/config/module.html#ruletype) 的支持。这些 `rule.type` 已被删除：
...
如果你正在使用 `builtins.noEmitAssets`, `builtins.devFriendlySplitChunks`, `builtins.html`, `builtins.copy`, `builtins.minifyOptions`，请查看[这篇文档](https://rspack.dev/zh/blog/announcing-0.4.html#%E8%BF%81%E7%A7%BB-builtin-options-%E5%88%B0-builtin-plugins)并进行迁移。

而如果你正在使用 `builtins.presetEnv`, `builtins.decorator`, `builtins.react`, `builtins.pluginImport`, `builtins.emotion`, `builtins.relay`, 请查看[这篇文档](https://rspack.dev/zh/blog/announcing-0.4.html#%E5%BA%9F%E5%BC%83%E9%BB%98%E8%AE%A4%E8%BD%AC%E6%8D%A2)并进行迁移。
...
该行为通过 [experiments.rspackFuture.disableApplyEntryLazily](/config/experiments.html#experimentsrspackfuturedisableapplyentrylazily) 进行废弃，该配置在 v0.4.5 中引入，在 v0.5.0 中默认启用，并将在 v0.6.0 中移除。
...
在将 `@rspack/core` 升级到 v0.5.0 后，如果构建失败并出现错误：`Can't resolve './src/xxx.tsx'`，或者 `Can't resolve './src/xxx.ts'`，或者 `Can't resolve './src/xxx.jsx'`，则需要在配置中添加 `resolve.extensions = ['...', '.tsx', '.ts', '.jsx']`。
...
在将 `@rspack/core` 升级到 v0.5.0 后，如果构建失败并出现错误：`Failed to resolve @swc/helpers/xxx` 或 `Failed to resolve react-refresh/xxx`，则需要在您的项目中安装 `@swc/helpers` 或 `react-refresh`。
