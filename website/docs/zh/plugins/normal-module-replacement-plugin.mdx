---
description: 'NormalModuleReplacementPlugin 可以帮你把 resourceRegExp 匹配的 import/require 等模块请求（后面简称 请求 或 request）替换为 newResource。'
---
...
在 `beforeResolve` 阶段匹配模块请求，在 `afterResolve` 阶段匹配解析后的资源。

### newResource

- **类型：** `string | ((data: ResolveData) => void)`

用于替换的请求，或用于修改解析数据的函数。相对路径形式的替换请求会相对于原资源进行解析。

<Attribution url="https://webpack.docschina.org/plugins/normal-module-replacement-plugin/" />
