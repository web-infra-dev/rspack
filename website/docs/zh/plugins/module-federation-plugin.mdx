---
description: 'ModuleFederationPlugin 用于在 Rspack 中启用模块联邦（Module Federation）能力。'
---
...
`ModuleFederationPlugin` 用于在 Rspack 中启用模块联邦（Module Federation）能力。

该插件实现了 Module Federation 1.5 规范。如果你希望系统地了解模块联邦的核心概念、使用方式与最佳实践，请参考 [Module Federation 指南](/guide/advanced/module-federation)。

## 安装依赖

该插件依赖运行时包 [@module-federation/runtime-tools](https://www.npmjs.com/package/@module-federation/runtime-tools)，用于提供模块联邦的运行时支持。请先安装该依赖：
...
数组会保留共享作用域的顺序。例如，`['custom', 'default']` 在初始化旧版容器时使用 `"custom"` 作为主作用域。增强容器会接收完整的有序作用域列表，各作用域保留独立的共享模块注册表。

数组必须至少包含一个作用域。多个作用域需要 `ModuleFederationPlugin` 使用的增强运行时，或在使用底层联邦插件时设置 `enhanced: true`。不使用增强运行时时，单元素数组会被视为字符串，包含多个元素的数组会被拒绝。这些规则也适用于 `remotes.*.shareScope` 和 `shared.*.shareScope`。
...
为暴露的模块指定[模块层](/guide/advanced/layer)。无需通过 `module.rules` 指定层，即可将同一模块请求暴露到不同的层中：
...
此选项适用于使用增强运行时的 Module Federation 1.5。直接使用 `ContainerPlugin` 时，需要设置 `enhanced: true`；Module Federation 1.0 不支持此选项。省略 `layer` 表示不指定层。空字符串（`layer: ''`）会作为显式的层值保留，与省略 `layer` 不同。
...
- request：覆盖此共享配置匹配的模块请求。省略 `import` 时，默认使用 `request`；若同时省略 `request`，则使用配置键。
- layer：指定共享提供者和后备模块的层。匹配的层优先于不指定层的后备配置。空字符串表示显式指定的层，与省略该选项不同。
- issuerLayer：仅对从此层导入的模块应用共享消费配置，与共享模块自身的 `layer` 相互独立。
...
当相同共享键、共享作用域和层对应多个具体提供者时，共享条目会包含可选的 `providers` 数组。每个提供者记录其 `version`、相对于构建上下文解析后的 `import` 和 `assets`。原有共享条目及其字段会保留，以兼容旧版读取器。只有一个提供者的条目不会输出 `providers`；读取器也应接受不包含此字段的 manifest。

使用层的构建会在这两种输出中添加可选的身份字段：

- `layer` 表示暴露条目或共享条目所属的层；未指定层时省略此字段。
- 共享条目通过 `shareScope` 表示非默认作用域或有序作用域数组。省略此字段时，表示标量作用域 `"default"`。
- 使用标量作用域且未指定层的共享条目保留原有 `id`。当原有 ID 冲突或共享键以保留前缀 `shared:` 开头时，`identityId` 用于区分条目。读取器应使用 `identityId ?? id` 作为共享条目的身份标识，不应解析其内部格式。
- 当仅凭共享名称不足以区分依赖时，暴露条目会包含 `requiredShared`。每项包含 `name`，以及可选的 `shareScope` 和 `layer`；stats 输出同时保留仅包含共享键的 `requires` 列表。禁用产物分析时，不输出推断出的依赖关系。

读取器应兼容不包含这些可选字段的旧版 manifest。
...
- disableAssetsAnalyze：禁用产物分析。共享和暴露模块的信息来自配置，资源列表为空，且不包含解析后的提供者详情。
