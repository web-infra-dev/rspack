...
当资源分析发现同一共享名称存在多个具体提供者时，两个文件都会包含可选的 `shared[].providers` 数组。每条记录包含 `version`、相对于构建上下文的 `import` 和 `assets`，原有共享字段保持不变。没有提供者、只有一个提供者或禁用资源分析时，不输出此数组。提供者专属的 fallback 入口元数据和按 layer 分组不属于此草案，参见 [provider 元数据 RFC](https://github.com/module-federation/core/issues/5082)。
