...
数组会保留共享作用域的顺序。例如，`['custom', 'default']` 在初始化旧版容器时使用 `"custom"` 作为主作用域。增强容器会接收完整的有序作用域列表，各作用域保留独立的共享模块注册表。若入口的初始 chunk 消费了有序作用域数组中的共享模块，该入口会异步启动，因为作用域会先于消费模块安装完成初始化；module 类型的库输出会等待该初始化完成。
...
此选项适用于使用增强运行时的 Module Federation 1.5。直接使用 `ContainerPlugin` 时，需要设置 `enhanced: true`；Module Federation 1.0 不支持此选项。省略 `layer` 表示不指定层。层必须是非空字符串，空字符串会被拒绝。
...
- request：覆盖此共享配置匹配的模块请求。省略 `import` 时，默认使用 `request`；若同时省略 `request`，则使用配置键。需要增强运行时（Module Federation 1.5）；直接使用 `ConsumeSharedPlugin` 或 `ProvideSharedPlugin` 时需设置 `enhanced: true`。
- layer：指定共享提供者和后备模块的层。匹配的层优先于不指定层的后备配置。层必须是非空字符串，空字符串会被拒绝。需要增强运行时。
- issuerLayer：仅对从此层导入的模块应用共享消费配置，与共享模块自身的 `layer` 相互独立。必须是非空字符串；需要增强运行时。
...
- 使用标量作用域且未指定层的共享条目保留原有 `id`，但不保证其唯一性。在同一个 manifest 中，应结合 `name`、`shareScope` 和 `layer` 区分共享条目，同时保留作用域数组的顺序。
...
- disableAssetsAnalyze：禁用产物分析。共享和暴露模块的信息来自配置，资源列表为空。
