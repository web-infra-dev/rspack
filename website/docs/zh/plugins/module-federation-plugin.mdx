...
此选项适用于使用增强运行时的 Module Federation 1.5。直接使用 `ContainerPlugin` 时，需要设置 `enhanced: true`；Module Federation 1.0 不支持此选项。省略 `layer` 表示不指定层。
...
- layer：指定共享提供者和后备模块的层。匹配的层优先于不指定层的后备配置。
