---
description: 'VirtualModulesPlugin 允许你在内存中创建、修改和删除文件，Rspack 会将这些虚拟文件视为真实存在于文件系统中的文件进行处理。'
---
...
可选的构造函数参数将每个虚拟文件路径映射到对应的文件内容。路径相对于 [compiler.context](/api/javascript-api/compiler#context) 解析。
