...
:::tip
内建默认值是 `chunks: 'async'`，但对大多数生产环境应用，建议优先尝试：
...
同时保留默认 cacheGroups。这样通常可以同时对 initial chunk 和 async chunk 去重，并且不会改变 JavaScript 的执行语义。Rspack runtime 仍然只会加载当前入口/runtime 实际可达的 chunk。
:::
...
- 共享候选模块太小，不满足 `minSize`
- 拆分后的总体积缩减不满足 `minSizeReduction`
- 候选模块不满足 `minChunks`
- `maxAsyncRequests` 或 `maxInitialRequests` 拒绝了这次拆分
...
在 `mini-css-extract-plugin`、`experiments.css` 这类 extracted CSS 场景里，splitChunks 改变 chunk group 后，最终 CSS 顺序也可能随之变化。可以参考 [Deep dive into webpack CSS order issue](https://github.com/orgs/web-infra-dev/discussions/12)。如果 CSS 顺序是主要诉求，也建议同时参考 [CssChunkingPlugin](/plugins/rspack/css-chunking-plugin)。
