...
启用 `experiments.env` 后，可以通过 `DefinePlugin` 定义 `import.meta.env`：
...
读取整个 `import.meta.env` 时，Rspack 会生成包含所有已定义字段的对象：
...
启用 `experiments.env` 时，[`EnvironmentPlugin`](/plugins/webpack/environment-plugin)、`NODE_ENV` 以及 `DefinePlugin` 中的 `import.meta.env.*` 键会被收集到 `import.meta.env`。
