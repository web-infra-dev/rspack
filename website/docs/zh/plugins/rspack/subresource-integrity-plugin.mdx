...
当使用 HTML 插件（[`HtmlRspackPlugin`](/plugins/rspack/html-rspack-plugin) 或 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin)）时，`integrity` 和 `crossorigin` 属性将被自动设置。

SubresourceIntegrityPlugin 默认会与 [`HtmlRspackPlugin`](/plugins/rspack/html-rspack-plugin) 交互：
...
对于 [`HtmlRspackPlugin`](/plugins/rspack/html-rspack-plugin)，模板的语法与标准 `.ejs` 稍有不同（参考[文档](/plugins/rspack/html-rspack-plugin#supported-ejs-syntax)），可以这样注入属性：
