...
1. 默认 `targets` 为 `"Chrome 60"`，以保证压缩后不会出现高级语法导致浏览器不兼容（压缩过程中可能会出现由于高级语法更短而将低级语法转为高级语法）。
2. 默认 `exclude` 掉所有的 features，一般会在 loader 中进行语法降级，所以 exclude 掉所有 features 避免在 minimize 过程中的语法降级。
