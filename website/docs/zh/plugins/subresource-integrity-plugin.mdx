---
description: 'rspack.SubresourceIntegrityPlugin 是一个用于在 Rspack 中启用子资源完整性的插件。'
---
...
> 在 1.2.4 至 1.6.8 版本中，可使用 `rspack.experiments.SubresourceIntegrityPlugin`。
...
支持与 [`HtmlRspackPlugin`](/plugins/html-rspack-plugin) 和 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin) 集成。它会自动为注入的标签设置 `integrity` 和 `crossorigin` 属性。
...
当使用 HTML 插件（[`HtmlRspackPlugin`](/plugins/html-rspack-plugin) 或 [`html-webpack-plugin`](https://github.com/jantimon/html-webpack-plugin)）时，`integrity` 和 `crossorigin` 属性将被自动设置。

SubresourceIntegrityPlugin 默认会与 [`HtmlRspackPlugin`](/plugins/html-rspack-plugin) 交互：
...
对于 [`HtmlRspackPlugin`](/plugins/html-rspack-plugin)，模板的语法与标准 `.ejs` 稍有不同（参考[文档](/plugins/html-rspack-plugin#支持的-ejs-语法)），可以这样注入属性：
