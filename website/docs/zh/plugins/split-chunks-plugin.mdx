...
花费额外的构建时间，寻找更多可以抽离的公共模块，以减小产物体积。如果愿意用更长的构建时间换取更小的生产产物，可以开启此选项：
...
此选项为未指定名称的缓存组发现更多抽离候选。抽离仍遵守缓存组的模块与 chunk 筛选条件、优先级、体积和请求数限制，并保持模块执行语义。开启后，chunk 边界和文件名可能发生变化。体积收益和构建耗时取决于项目，建议测量后再启用。
