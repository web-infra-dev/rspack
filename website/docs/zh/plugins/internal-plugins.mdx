---
description: '介绍用于目标预设、运行时模板、子编译器和上层插件的 Rspack 底层插件。'
---
...
本页面汇总 Rspack 主要用于实现目标预设、运行时模板、子编译器和上层插件的底层插件。

:::warning 警告
大多数应用构建应使用对应的上层配置。只有在开发自定义编译器、配置子编译器或研究 Rspack 内部机制时，才建议直接使用这些插件。
:::
