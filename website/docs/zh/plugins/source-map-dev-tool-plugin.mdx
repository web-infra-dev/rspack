---
description: '本插件实现了对 source map 生成内容进行更细粒度的控制。它也可以根据 devtool 配置选项的某些设置来自动启用。'
---
...
本插件实现了对 source map 生成内容进行更细粒度的控制。它也可以根据 [`devtool`](/config/devtool) 配置选项的某些设置来自动启用。
...
<Attribution url="https://webpack.docschina.org/plugins/source-map-dev-tool-plugin/" />
