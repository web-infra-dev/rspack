...
要在本地分析对象所有权树，先构建启用了 allocative 的 binding。该功能默认关闭：
...
安装 Inferno（`cargo install inferno --locked`），然后在 Rspack 仓库中渲染快照：
...
报告包含独立的 `memory.svg` 和 `counts.svg` 火焰图、`counts.csv`，以及覆盖范围诊断。数量表示当前模块图中的条目数，并非进程内全部对象的数量。字节权重包含已遍历对象拥有的堆分配和容器容量，但不包含无法访问的第三方内部状态、V8 内存，以及部分容器和分配器元数据。比较结果前请检查警告；总量既不是 RSS，也不是内存峰值。

恢复普通构建时，取消设置 `ALLOCATIVE`。不要设置为 `ALLOCATIVE=0`，因为构建脚本会将非空的环境变量视为启用。
