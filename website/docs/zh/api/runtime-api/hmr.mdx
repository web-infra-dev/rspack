---
description: 'Rspack 提供 webpack HMR API 和独立的 import.meta.hot 上下文。'
---
...
- `import.meta.hot` 是独立的模块级上下文，仅支持下文介绍的四种
  `accept` 形式、`data` 和 `dispose`。

请通过 [HotModuleReplacementPlugin](/plugins/webpack/hot-module-replacement-plugin) 启用这些 API。在 ESM 中，请使用 `import.meta.webpackHot` 访问 webpack API，或使用独立的 `import.meta.hot` 上下文，而不是 `module.hot`。
