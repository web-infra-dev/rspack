---
description: 'Rspack 提供 webpack HMR API 和 import.meta.hot 兼容 facade。'
---
...
- `module.hot` 和 `import.meta.webpackHot` 实现 webpack HMR API。
- `import.meta.hot` 是基于 webpack HMR runtime 的模块级兼容 facade，仅支持下文介绍的四种
  `accept` 形式、`data` 和 `dispose`。

请通过 [HotModuleReplacementPlugin](/plugins/webpack/hot-module-replacement-plugin) 启用 webpack API。在 ESM 中，请使用 `import.meta.webpackHot` 访问 webpack API，而不是 `module.hot`。

`import.meta.hot` 目前是实验性能力，默认关闭。除 HMR 外，还需要启用对应开关：
...
`import.meta.hot` 支持以下四种 `accept` 形式：
...
if (import.meta.hot) {
  ...
  // 接受自身更新，回调接收更新后的模块命名空间。
  import.meta.hot.accept((module) => {
    console.log(module?.default);
  });

  // 接受单个依赖，回调接收该依赖更新后的命名空间。
  import.meta.hot.accept('./library.js', (module) => {
    console.log(module?.default);
  });

  // 接受多个依赖，命名空间与请求列表保持对齐。
...
如果某个依赖无法执行，依赖接受回调会在对应位置收到 `undefined`。多个依赖的回调仍会收到与声明的依赖列表对齐的数组。只有在更新后的模块成功执行后，才会调用自接受回调。

使用 `data` 和 `dispose` 向模块的下一个版本传递状态，并在模块被替换前释放资源：
...
此上下文不支持其他方法。特别是，本版本不支持 `decline`、`invalidate`、`check` 或 `apply` 等未支持的生命周期或管理方法，`on`、`off` 或 `send` 等客户端传输方法，以及 `acceptExports` 或 `prune` 等导出边界方法。

### TypeScript

在仅使用 Rspack 的项目中，通过 `@rspack/core/import-meta-hot` 选择性启用全局 `ImportMeta.hot` 声明：
...
此 opt-in 入口适用于尚未提供 `ImportMeta.hot` 声明的项目。如果迁移项目已经提供了兼容的 `HotContext` 声明，请勿再加载此 opt-in 入口；Rspack 运行时支持其中的 `accept`、`data` 和 `dispose` 子集，但不实现 `on`、`off`、`send`、`prune` 或 `acceptExports`。
...
以下“模块 API”和“API 管理”章节仅适用于 `module.hot` 和 `import.meta.webpackHot`。
...
T> 当调用 `invalidate` 时，将最终调用 [`dispose`](#dispose或-adddisposehandler) 处理函数并填充 `module.hot.data`。如果未注册 [`dispose`](#dispose或-adddisposehandler) 处理程序，则将空对象提供给 `module.hot.data`.
