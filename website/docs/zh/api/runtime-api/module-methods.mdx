---
description: '当使用 Rspack 打包应用程序时，你可以选择多种模块语法风格，包括 ES modules 和 CommonJS。'
---
...
:::tip
`require.context` 的返回值与 [import.meta.webpackContext](/api/runtime-api/module-variables#importmetawebpackcontext) 相同。
:::
...
:::tip
建议使用更符合标准的 `import()` 来替代 `require.ensure()`。
:::

将给定 `dependencies` 单独打包，并异步加载这些依赖。
...
- 和 webpack 一样，callback 函数也可以接收一个名为 require 的参数（且必须名为 require），该 require 参数和 cjs 的 require 全局变量一致，但我们建议直接使用全局的 require 来加载模块而不是该 require 参数，因为 loader 可能会将其重命名导致依赖分析失败。
...
<Attribution url="https://webpack.docschina.org/api/module-methods/" />
