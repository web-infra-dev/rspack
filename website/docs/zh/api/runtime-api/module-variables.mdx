...
- **参数：**
  - `pattern`：匹配文件的 glob 模式字符串，或 glob 模式字符串数组（例如 `'./dir/*.js'`）。支持 `*`、`?`、`[abc]`、`{a,b}` 以及 `**`（递归匹配）。不以相对路径符号开头的模式可以使用 `resolve.alias`（例如 `'@/pages/*.js'`）；当文件位于项目根目录内时，返回对象的 key 使用相对于项目根目录的解析后路径。数组模式也可以包含负向模式，例如 `'!**/bar.js'`。
...
`import.meta.webpackContext` 是 [`require.context()`](/api/runtime-api/module-methods#requirecontext) 的 ESM 等价写法，它允许你动态地引入一组模块，并用于兼容 webpack 的 context module 行为。

在不需要兼容 webpack context 行为的新 Rspack 代码中，我们更推荐使用 [`import.meta.glob`](#importmetaglob)。
...
访问当前模块的 [layer](/guide/advanced/layer)。它的值是配置的 layer 名称；如果模块未分配 layer，则为 `null`。
...
<Attribution url="https://webpack.docschina.org/api/module-variables/" />
