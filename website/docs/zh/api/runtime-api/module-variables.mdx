...
:::note
由于 Rspack 需要静态分析 `import.meta` 元数据的使用情况，所以 Rspack 不支持动态访问 `import.meta` 并会抛出警告。你应该通过属性访问或解构赋值的方式来访问 `import.meta` 上的元数据。
...
类似 [`require.resolve()`](/api/runtime-api/module-methods#requireresolve) 获取模块的 ID，会将模块打包进最终的 bundle 中，并返回模块的模块 ID。

```ts
import.meta.resolve(dependency: string);
```
