...
Rspack logger 可以在 [loaders](/guide/features/loader) 和 [plugins](/guide/features/plugin) 中使用。 生成的日志将作为 [Stats](/api/objects/stats) 的一部分并且可以在 [rspack configuration](/config/) 中配置。
...
通过引入 Rspack 的 Logger API。我们希望能够统一 Plugin 和 Loader 输出日志的方式以更方便地排查问题和提升开发体验。同时为非 CLI 的集成 Rspack 提供解决方案，如 dashboard 和其他 UI 工具。
