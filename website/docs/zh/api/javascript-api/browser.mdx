...
注意，`@rspack/browser` 内部使用了 [SharedArrayBuffer](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer) 来实现多线程的共享内存，因此你需要为开发服务器或线上部署环境设置响应头。参考 [Rspack](/config/dev-server#devserverheaders) 的设置方法。
...
- [napi-rs#2867](https://github.com/napi-rs/napi-rs/issues/2867)： 目前你需要在使用了 `@rspack/browser` 的项目中，通过 [DefinePlugin](/plugins/webpack/define-plugin) 或其他等价方式设置 `process.env.NODE_DEBUG_NATIVE`。参考 [Rspack Playground 的做法](https://github.com/rspack-contrib/rspack-playground/blob/main/rsbuild.config.ts)。
