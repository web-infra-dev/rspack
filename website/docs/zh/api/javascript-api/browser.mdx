---
description: '@rspack/browser 是专为浏览器环境打造的 Rspack 版本，无需依赖 WebContainers 或特定平台。'
---
...
请把 `@rspack/browser` 放到专用的 Web Worker 中运行，因为浏览器主线程不允许调用 `Atomics.wait`。

除了浏览器侧的文件系统和 Worker 启动方式之外，其余 API 与 `@rspack/core` 的 [JavaScript API](/api/javascript-api/) 保持一致。
...
在 Rspack 中，某些场景需要动态加载和执行 JavaScript 代码，如 [Loader](/guide/features/loader) 或 [HtmlRspackPlugin 的模板函数](/plugins/html-rspack-plugin#使用模板生成函数)。由于这些代码可能来自不可信的第三方用户，直接在浏览器环境中执行会带来潜在的安全风险。为了保障安全性，`@rspack/browser` 在遇到此类场景时会默认抛出错误，阻止不安全代码的执行。
...
`@rspack/browser` 支持 Rspack 的 [ModuleFederationPlugin](/plugins/module-federation-plugin) 插件。你可以通过这个功能，将一些复杂的依赖模块提前打包为生产者项目部署到 CDN 上，然后在浏览器中在线打包的消费者项目中使用。
...
3. 所有 [ModuleFederationPlugin.shared](/plugins/module-federation-plugin#shared) 配置必须设置 `import: false`，即在线打包的消费者项目不能打包共享模块，共享模块必须由预先打包好的生产者项目提供。
4. 不支持设置 [ModuleFederationPlugin.implementation](/plugins/module-federation-plugin#implementation) 选项。
