...
NormalModule 持有可复用的 loader 元数据，包括各 loader 的 request、type 和缓存选项。每次运行 loader 都独立创建新的 pitch data 和执行标记。

在 JavaScript 边界，Rspack 传入普通的 `JsLoaderContext` 对象，其中包含 loader 元数据、只读执行信息和一个 `JsLoaderContextState` 对象。loader-context 封装通过 JavaScript getter/setter 访问和更新 state。JavaScript runner 返回这个 state 对象，Rust 合并其中的内容、source map、附加数据、依赖、parse metadata 和执行标记。不可变的 loader 元数据无需随返回结果再次转换到 Rust。并行 loader 同样分开传递元数据和执行状态。
...
JavaScript loader-context 封装：getter/setter 访问 state
  ↓ 返回 state 对象
Rust runner：合并执行状态并继续
```

例如，内置 loader 前面的 JavaScript loader 可以修改 pitch data 和依赖。这些改动通过 state 对象传回，loader 的 request 和 type 则保留为元数据。内容和 source map 沿用原有的 Buffer 传递表示；这层拆分不会改变 loader hook 的执行时机或原生上下文的所有权。
