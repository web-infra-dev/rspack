...
`Compiler.compilation` 使用仅由 compiler 持有的所有权容器，内部为 `Arc<Compilation>` 或 `UniqueArc<Compilation>`。hook 和 pass 保持原来的 `&Compilation`、`&mut Compilation` 参数。这个容器不在线程间共享；调度器在共享阶段发布 owned Arc，并在修改前回收 UniqueArc。

compiler 的 emit 调度在 `emit` 与 `afterEmit` 之间完成这个交接：后台任务持有 Arc，写入资产并调用 `assetEmitted`；任务全部结束后，调度器先恢复 UniqueArc，再传播任务错误或调用 `afterEmit`。如果 reader 仍被持有，回收返回错误。build/rebuild 通过替换内容保留分配地址。Weak 只在共享阶段能够 upgrade，读取的是当前 compilation，而非历史快照。其他 scoped 任务仍沿用借用方式。JavaScript native-backed 句柄尚未迁移为 Weak，仍遵循上面的生命周期约束。
