...
例如，内置 loader 两侧的 JavaScript loader 都可以在 pitch 阶段设置 `this.data`。进入 normal 阶段后，每个 loader 读取自己的 data，而内容从一个 loader 传递到下一个。pitch data 通过 `loaderData` 传回，依赖改动通过 `state` 传回，loader 的 request 和 type 则保留为元数据。内容和 source map 沿用原有的 Buffer 传递表示；这层拆分不会改变 loader hook 的执行时机或原生上下文的所有权。
