...
JavaScript 调用结束后，原生 runner 收回上下文，该 `JsLoaderContext` 实例的访问随即失效。这是内部桥接机制；loader 仍然使用兼容 webpack 的 `this` 上下文、字符串或 Buffer 内容、source-map 对象以及回调。
