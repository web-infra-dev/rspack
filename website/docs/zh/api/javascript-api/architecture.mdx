...
`Chunk` 句柄属于提供它的 `Compilation`。将其他 compiler 或之前构建的 Chunk 传给 `compilation.chunks.has()` 会返回 `false`；将其传给 ChunkGraph 方法会抛出错误。增量构建内部复用 Chunk 数据，不会延长之前 JavaScript 句柄的有效期。
