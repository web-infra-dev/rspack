...
执行切换到 JavaScript 时，原生 `JsLoaderContext` class 在本次调用期间直接持有 `Option<Box<LoaderContext>>`。只有 JavaScript runner 读取对应 getter 时，才转换内容和 source map。JavaScript 封装在本次调用中缓存各 getter 的结果，包括空值；下次进入 JavaScript 时，新封装重新读取原生值。JavaScript 产生的输出优先于缓存的输入。
...
```text
原生 loader：String/Buffer + 独立 source map
  ↓ move 装箱的 LoaderContext
JsLoaderContext：state 快照 + 按需 content/source-map getter
  ↓
