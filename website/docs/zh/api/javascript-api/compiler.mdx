...
1. 这个 API 一次仅支持一个编译。请在 `compiler.run` 回调中调用 `compiler.close`，并等待它完成后再次执行 `compiler.run`。并发编译将破坏输出文件。
2. Rspack 缓存失效检测依赖 `modifiedFiles` 和 `removedFiles`，在启用缓存的情况下，需要将它们通过 options 参数传递给 Rspack。
