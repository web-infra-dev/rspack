---
description: 'Compiler 是 Rspack 内部的核心对象，当你调用 Rspack 的 JavaScript API 或 CLI 时，都会创建一个 Compiler 实例。'
---
...
Compiler 是 Rspack 内部的核心对象，当你调用 Rspack 的 [JavaScript API](/api/javascript-api/) 或 [CLI](/api/cli) 时，都会创建一个 Compiler 实例。
...
一个常见的例子是在 Rspack 插件中获取 [sources](/api/javascript-api/#sources-对象) 对象：
...
获取根 Compiler 所使用的完整[构建配置](/config/)。
...
获取各个 Compiler 所使用的完整[构建配置](/config/)。
