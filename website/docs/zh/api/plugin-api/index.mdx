---
description: 'Rspack 插件 API 总览，介绍钩子分类、兼容性现状、调用时机以及面向 webpack 生态的兼容写法。'
---
...
- [ExternalModule 钩子](/api/plugin-api/external-module-hooks)：控制外部模块如何分配到 chunk
- [NormalModuleFactory 钩子](/api/plugin-api/normal-module-factory-hooks)：介入模块创建过程中的各个阶段
- [NormalModule 钩子](/api/plugin-api/normal-module-hooks)：介入单个模块的构建过程
