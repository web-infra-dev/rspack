...
`NormalModuleFactory` 模块被 [Compiler](/api/javascript-api/compiler) 用来生成模块。从每个 entry point 开始，它解析每个请求，解析内容以找到进一步的请求，并通过解析所有新文件来继续遍历文件。最后，每个依赖项都变成一个 Module 实例。

`NormalModuleFactory` 类继承自 `Tapable` 并提供以下生命周期钩子。它们可以像 compiler 钩子一样被使用：

```js
NormalModuleFactory.hooks.someHook.tap(/* ... */);
```

除了 `tap()`，你还可以使用 `tapAsync()` 和 `tapPromise()`，具体取决于钩子的类型。
