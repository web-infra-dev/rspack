---
description: 'Compiler hooks 允许 Rspack 插件在特定阶段介入构建流程。它们代表了从初始化到资源输出的各个生命周期阶段。'
---
...
:::note
这个钩子仅在调用 [compiler.run()](/api/javascript-api/#compilerrun) 时触发（对应 `rspack build` 命令），在监听模式（watch mode）下不会执行。在监听模式下，你可以使用 [watchRun](#watchrun) 钩子。
:::
...
:::note
这个钩子仅在调用 [compiler.watch()](/api/javascript-api/#compilerwatch) 时触发，在非监听模式下不会执行。在非监听模式下，你可以使用 [run](#run) 或 [beforeRun](#beforerun) 钩子。
:::
...
在 compilation 完成并 seal 之后、Rspack 判断是否输出产物（[`shouldEmit`](#shouldemit)）之前调用。

此时产物已经生成并可以通过 `compilation.assets` 访问，但尚未写入输出目录，因此插件仍然可以在这里终止构建，而不会留下已写出的产物文件。
...
在输出资源之前调用。返回 `false` 表示跳过资源输出，返回 `true` 表示输出资源；这两种返回值都会停止执行后续 tap。返回 `undefined` 或不返回值则继续执行下一个 tap。如果没有 tap 返回布尔值，Rspack 会输出资源。

- **类型：** `SyncBailHook<[Compilation], boolean | void>`
- **参数：**
  - `Compilation`: 当前 [Compilation](/api/javascript-api/compilation) 对象
