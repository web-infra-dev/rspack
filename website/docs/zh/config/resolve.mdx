...
- `undefined`: 如果启用 `resolve.pnp` 且未提供此选项，Rspack 将尝试查找最近的 `.pnp.cjs` 文件作为 PnP manifest 文件路径。
- `string`: 强制 Rspack resolver 使用指定的 PnP manifest 文件路径。
- `false`: 用于支持禁用了 globalCache 的交叉引用项目。
