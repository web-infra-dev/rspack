...
export default {
  resolve: {
    ...
    byDependency: {
      ...
      commonjs: {
        extensions: ['.cjs', '...'],
        // commonjs 的最终值：['.cjs', '.ts', '.tsx', '.js', '.json']
      },
    },
  },
};
...
- **类型：** `string[]`
- **默认值：** 取决于 dependency type。常见 JavaScript 模块请求默认是 `[".js", ".json"]`，CSS `@import` 默认是 `[".css"]`。
...
从实现上看，顶层 `resolve.extensions` 的默认值其实是 `[]`。之所以大多数场景仍然会自动尝试 `.js`、`.json` 或 `.css`，是因为 Rspack 会根据请求类型，从 [resolve.byDependency](#resolvebydependency) 应用对应的默认配置。

如果你不熟悉 [resolve.byDependency](#resolvebydependency)，可以把它理解为"不同引用方式使用不同的 `resolve` 默认值"。例如：

- `import`、`require()` 等常见 JavaScript 模块请求默认使用 `[".js", ".json"]`
- CSS 的 `@import` 默认使用 `[".css"]`

如果你还在 [resolve.byDependency](#resolvebydependency) 下配置了 `resolve.byDependency.esm.extensions` 或其他按依赖类型区分的 `extensions` 选项，那么匹配到的 `byDependency` 配置会在之后继续应用。不带 `'...'` 时会替换当前顶层 `resolve.extensions` 的值，带 `'...'` 时则会在该位置插入当前顶层 `resolve.extensions` 的值。

如果你希望在添加自定义扩展名的同时保留 Rspack 针对当前 dependency type 的默认扩展名，可以使用展开语法 `'...'`：

```js title="rspack.config.mjs"
export default {
  resolve: {
    // 对常见 JavaScript 模块请求，等价于 ['.ts', '.js', '.json']
    extensions: ['.ts', '...'],
  },
};
```
...
- `false`: 不使用全局 PnP 清单，而是分别在每个项目中解析 PnP 清单。这可以支持在 `.yarnrc.yml` 中禁用了 `enableGlobalCache` 的交叉引用项目。
