...
启用后，resolver 会在 `node_modules` 目录之后，继续搜索 [`NODE_PATH`](https://nodejs.org/api/modules.html#loading-from-the-global-folders) 环境变量中指定的目录。这遵循了 Node.js 从全局文件夹加载模块的行为。

> 注意：开启后，模块解析结果会依赖外部 `NODE_PATH` 环境变量，可能导致不同环境下的构建结果不一致，或意外解析到全局安装的模块。建议仅在确有需要时启用，并在 CI / 生产环境中确保 `NODE_PATH` 可控。
```js title="rspack.config.mjs"
export default {
  resolve: {
    nodePath: true,
