...
:::warning
开启 `parallelCodeSplitting` 的情况下，请确保同时开启 `optimization.removeAvailableModules`(1.3.0后同样是默认开启)，这样才能和原有的 splitting 算法对齐,因为旧的 splitting 算法强制开启了 removeAvailableModules 算法，并且不受到 `optimization.removeAvailableModules` 控制。
:::
