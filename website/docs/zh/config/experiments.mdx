...
该能力默认开启。若要关闭 pure function 分析，可将该选项设为 `false`。若要手动为命中的模块标记标识符 —— 既可以是第三方库中的导出函数，也可以是你自己代码中的 import 或本地声明 —— 推荐通过 `module.rules[i].parser` 来配置 [`module.parser.javascript.pureFunctions`](/config/module-parser#javascriptpurefunctions)，这样规则只会作用于目标模块。
