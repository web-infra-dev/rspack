...
是否增量地进行重构建，加快重构建和 HMR 的速度。提供两种配置方式：
...
1. 预设配置：包括 `boolean | 'none' | 'safe' | 'advance' | 'advance-silent'`
   ...
   - `true | 'advance-silent'`：开启所有阶段的增量，最大优化 rebuild 和 HMR 的性能，在未来这些阶段稳定后，我们会将此选项作为 Rspack 的默认行为
   - `'advance'`：同上，但对于增量不友好的情况会抛出警告提示用户，该选项作为 `true | 'advance-silent'` 的补充帮助用户排查潜在可能影响增量构建性能的配置
2. 详细的对象配置：`Incremental`，细粒度控制各个阶段的增量是否开启
...
增量仅会提升重构建的速度，对于初次构建不会有任何影响，但对于持久化缓存可用时的初次构建也会被视为重构建，以利用增量加快构建速度：
