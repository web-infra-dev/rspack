...
当 `entry` 被设置为相对路径时，Rspack 会使用 [context 配置项](/config/context) 设置的值作为基础路径，默认为 Node.js 进程的当前工作目录，即 `process.cwd()`。
...
当某个入口的值是数组时，Rspack 仍然只会为该入口生成一个 chunk。数组中的模块会按声明顺序加载，并合并到同一个依赖图中，因此 `pre.js` 会早于 `post.js` 执行。

这种模式常用于在主应用入口之前注入额外代码，例如 polyfill 或仅在开发环境使用的工具脚本，而不必在业务源码中手动导入它们。
...
<Attribution url="https://webpack.docschina.org/configuration/entry-context/#entry" />
