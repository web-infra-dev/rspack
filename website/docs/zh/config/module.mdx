...
控制 CSS 模块在 JavaScript 中的导出方式。

- `'link'`: 将 CSS 作为 CSS 资源产物输出，并保留常规 CSS 导出行为。
- `'text'`: 将生成后的 CSS 文本作为默认导出。
- `'css-style-sheet'`: 将 `CSSStyleSheet` 实例作为默认导出。
- `'style'`: 在运行时通过 `<style>` 元素把生成后的 CSS 注入到文档中，并保留常规 CSS 导出行为。

当 CSS modules 使用 `'text'` 或 `'css-style-sheet'` 时，局部类名的命名导出仍然可用。
...
参考 [`module.parser["css/auto"].exportType`](#moduleparsercssautoexporttype)。
