...
export default {
  module: {
    rules: [
      {
        test: /mod\.js$/, // 仅为 index.js 模块中的导入语句启用 strictThisContextOnImports
        parser: {
          javascript: {
...
手动将命中模块中的顶层导出函数标记为“调用无副作用”，供 advanced tree shaking 使用。
...
:::warning
该选项只有在启用 `experiments.advancedTreeShaking` 时才会生效；如果在命中的模块中找不到配置的导出函数名，Rspack 会发出警告。
:::
