...
自定义生成的 CSS modules 的局部类名格式，除了在[文件级别](/config/output.html#file-context)和[模块级别](/config/output.html#module-context)的替换之外，还包括 `[uniqueName]` 和 `[local]`。
...
- `'jsx'`: 支持 jsx 的 JavaScript 模块（🚧 已废弃，详见 [experiments.rspackFuture.disableTransformByDefault](/config/experiments.html#experimentsrspackfuturedisabletransformbydefault)）。
