...
对使用不存在的导出进行警告或报错，默认会遵循 `module.parser.javascript.exportsPresence` 的配置。
...
对存在冲突的重导出进行警告或报错，默认会遵循 `module.parser.javascript.exportsPresence` 的配置。
