...
通过控制**模块解析阶段**的依赖追踪机制来优化构建性能。启用后，Rspack 将假定匹配的模块文件保持稳定（路径不变且不被删除），从而跳过文件监听以提升构建速度。

`module.unsafeCache` 存在以下默认行为：
...
此配置特别适用于稳定的第三方依赖。在实际开发中，通常不会修改 `node_modules` 中的模块路径，因此可以安全地启用此优化来减少文件系统监听开销。
...
:::tip 与 Webpack 的差异
即使当 `module.unsafeCache: false` 时，Rspack 仍会缓存 `Dependency`（依赖）与 `Module`（模块）之间的关系，这是因为 Rspack 在模块图重构中使用了增量算法。

当设置为 `false` 时，Rspack 会追踪模块解析所需的全部文件，确保在相关文件变化时能够重新执行模块解析，从而获得正确的模块文件路径。
:::
