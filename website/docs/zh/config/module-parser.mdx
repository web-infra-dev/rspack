...
`import.meta.resolve("./asset.txt")` 返回资源产物的 URL 字符串，类似于 `new URL("./asset.txt", import.meta.url).href`。它使用 `url` 依赖类别，因此默认按资源规则处理目标文件。将 `module.parser.javascript.url` 设置为 `"relative"` 时，返回相对 URL。

如果匹配的模块规则将目标类型设为 JavaScript，Rspack 会将其输出为独立的 JavaScript 入口，并返回入口 URL。解析 URL 不会执行该入口，例如可以将这个 URL 交给稍后加载脚本的 API。

仅处理恰好有一个可静态求值的字符串参数的调用，其他调用会保留到运行时。
...
const assetUrl = import.meta.resolve('./asset.txt');
const scriptUrl = import.meta.resolve('./script.js');
```
