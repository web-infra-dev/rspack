...
`string[]` 写法等价于将相同的值配置到 `worker.alias`。
不配置 `worker.url` 时，Rspack 会保留已有的 Worker URL 输出。
当 `worker.url` 设置为 `'new-url-relative'` 且启用 `output.module` 时，
Rspack 会为 `new Worker(new URL(..., import.meta.url))` 输出静态的 `new URL()`
表达式。对于非 ESM 输出，Rspack 会保留已有的运行时 Worker URL 输出：
...
- **类型：** `'link' | 'text' | 'css-style-sheet' | 'style'`
- **默认值：** `'link'`

配置 CSS 内容如何导出到 JavaScript。

默认情况下，`exportType` 为 `'link'`，会将 CSS 作为样式表产物输出，并通过内置 CSS loading runtime 加载。

- `'link'`：将 CSS 作为样式表产物输出，并保留 CSS Modules 导出。
- `'text'`：将生成后的 CSS 文本作为默认导出。
- `'css-style-sheet'`：将 `CSSStyleSheet` 实例作为默认导出。
- `'style'`：在运行时通过 `<style>` 标签将生成后的 CSS 注入到文档中。

当使用 `exportType: 'text'` 或 `exportType: 'css-style-sheet'` 时，生成后的 CSS 会成为 JavaScript 模块的一部分，并且该模块不会额外输出独立 CSS 资源。
...
### ["css/auto"].runtimePublicPath

- **类型：** `boolean`
- **默认值：** `false`

当 `exportType` 为 `'style'` 时，使用运行时公共路径生成输出资源的 URL。此选项不影响其他导出类型。

默认情况下，注入的 CSS 使用构建时解析的资源 URL。当 `output.publicPath: 'auto'` 时，这些 URL 仍相对于文档解析。设置 `runtimePublicPath: true` 可改用运行时公共路径，包括赋给 `import.meta.rspackPublicPath` 的值。

如果动态设置 `import.meta.rspackPublicPath`，请在导入样式之前执行的独立模块中设置它。显式配置的[资源生成器公共路径](/config/module-generator#assetpublicpath)具有更高优先级。内联 data URL 和外部 URL 保持不变。
...
和 [`module.parser["css/auto"].exportType`](#cssautoexporttype) 一样。
...
### css.runtimePublicPath

同 [`module.parser["css/auto"].runtimePublicPath`](#cssautoruntimepublicpath).
...
<Attribution url="https://webpack.docschina.org/configuration/module/" />
