...
- Rspack 特有字段：`env`、`glob`、`resolve`、`rspackBaseUri`、`rspackHash`、`rspackInitSharing`、`rspackNonce`、`rspackPublicPath`、`rspackRsc`、`rspackShareScopes`、`rspackUniqueId` 和 `rspackVersion`。
...
`env` 仅在启用 [`experiments.env`](/config/experiments#experimentsenv) 后生效。将其设为 `false` 可以让该模块中的 `import.meta.env` 保留到运行时再求值，而不是在编译时替换。
