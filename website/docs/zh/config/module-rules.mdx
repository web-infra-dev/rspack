---
description: '它是一个规则数组。每条规则都会在模块被解析和创建时，与模块的信息进行匹配，例如文件路径、文件类型或查询参数。'
---
...
更多关于 layer 的信息，请参考[模块分层指南](/guide/advanced/layer)。
...
### 通过 URL 引用 JavaScript 和 CSS 模块

你可以使用 `new URL('./module.js', import.meta.url)` 或 `new URL('./style.css', import.meta.url)` 获取 JavaScript 或 CSS 模块打包后的 URL。需要在规则中配置 `dependency: 'url'` 和对应的模块 `type`：
...
创建 URL 本身不会执行脚本或应用样式，你可以在需要时通过 `<script src>`、`<link rel="stylesheet" href>` 等方式使用这些 URL 加载产物。

默认情况下，`new URL()` 引用的文件会被视为资源，即使扩展名是 `.js` 或 `.css`。仅包含 `test` 的规则不会改变这一行为：需要像上面的示例一样显式配置 `dependency: 'url'` 和 `type`，才能将它们作为模块打包。
...
对于特定的解析器选项及其对应的模块类型，你可以参考 [`module.parser`](/config/module-parser#parser)。
...
对于特定的生成器选项及其对应的模块类型，你可以参考 [`module.generator`](/config/module-generator#generator)。
...
- `'json'`：JSON data 模块，参考 [JSON](/guide/languages/json)。
- `'css' | 'css/auto' | 'css/global' | 'css/module'`：CSS 模块，参考 [内置 CSS 支持](/guide/languages/css#内置-css-支持)。
...
- **类型：** `number`
- **默认值：** `Math.max(os.cpus().length - 1, 1)`

当 `parallel` 配置为对象时，限制共享 loader worker pool 使用的最大 worker 线程数。请使用正整数。

默认情况下，Rspack 使用 `os.cpus()` 返回的逻辑 CPU 数量减一，最少为一个 worker。

loader worker pool 在当前进程内共享；如果多个 loader 都配置 `parallel`，建议使用一致的 `maxWorkers` 值。
