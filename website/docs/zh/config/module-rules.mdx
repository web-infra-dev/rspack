...
- `'css' | 'css/auto' | 'css/global' | 'css/module'`：CSS 模块，参考 [内置 CSS 支持](/guide/tech/css#内置-css-支持)。
...
为单个 loader 启用缓存。该选项仅在启用 [`experiments.newCache`](/config/experiments) 时生效。
...
- 输入 source map 不属于缓存标识的一部分。如果 loader 的结果依赖该 source map，请勿启用此选项。
