...
- loader 选项必须是对象。选项可以包含函数，函数在主线程执行。
- compilation 和 module API 通过代理访问主线程实现，loader hook 也在主线程执行。`additionalData` 经过并行 loader 和 builtin loader 时，会保留主线程上的对象身份。
...
如果省略 `maxWorkers`，Rspack 默认使用 CPU 数量减一，且至少启动一个 worker。注册并行 loader 规则时会异步初始化 worker，编译会在构建模块前等待初始化完成。loader worker pool 在当前进程内共享；如果多个 loader 都配置 `parallel`，建议使用一致的 `maxWorkers` 值。
