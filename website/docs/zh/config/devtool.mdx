...
如需更细粒度地控制 source map 的生成方式，将 `devtool` 设为 `false`，并改用 [SourceMapDevToolPlugin](/plugins/source-map-dev-tool-plugin)；对于基于 `eval` 的 source map，改用 [EvalSourceMapDevToolPlugin](/plugins/eval-source-map-dev-tool-plugin)。
