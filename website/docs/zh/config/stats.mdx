---
description: '开启 stats.all 会导致大量的 Rust 与 JavaScript 的数据传输，这将显著提高 Stats 生成耗时，请谨慎使用。'
---
...
- **`@rspack/cli` 中的默认输出选项：** `{ "all":false, "errors":true, "warnings":true, "moduleTrace":true, "timings":true }`

当 `stats.preset` 和 `stats.all` 均未指定时，会合并这些默认值。由于 `stats.all` 默认为 `false`，因此 `stats.logging` 默认也处于禁用状态。
...
隐藏匹配的资源，匹配方式支持字符串、正则，或接收资源名称和资源对象并返回布尔值的函数，同时也支持以上类型的数组。
...
隐藏匹配的模块，匹配方式支持字符串、正则，或接收模块名称、模块对象和模块类型并返回布尔值的函数，同时也支持以上类型的数组。
