...
按照预设值进行输出，选项见[预设](/config/stats#预设stats-presets)
...
<PropertyType.CN type="string" defaultValueList={[{ defaultValue: '"id"' }]} />
...
<PropertyType.CN type="number" defaultValueList={[{ defaultValue: '10' }]} />
...
<PropertyType.CN type="number" defaultValueList={[{ defaultValue: '5' }]} />
...
<PropertyType.CN type="number" defaultValueList={[{ defaultValue: '1000' }]} />
...
基于模块的属性分组（是否包含错误、是否包含警告、是否包含资源、是否必须、是否孤儿模块、是否被依赖模块等）。

#### stats.groupModulesByCacheStatus

<PropertyType.CN type="boolean" />

基于模块的缓存状态分组（包括已缓存、已构建且可缓存）。
...
是否展示产物的[输出路径](/config/output#outputpublicpath)。
