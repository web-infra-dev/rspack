...
:::warning
`compat-hashed` 生成的模块 id 区分大小写且可能包含大写字母。在大小写不敏感的文件系统上，不要仅使用 `[id]` 区分 [Asset module 文件名](/config/output#outputassetmodulefilename)。请优先使用 `[contenthash]`，或将其与 `[id]` 一起使用。
:::
