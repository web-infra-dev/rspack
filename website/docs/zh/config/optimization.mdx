...
| `compact`       | 使用具有更密集标识符空间的稳定字母数字 id。等同于使用默认选项的 `CompactModuleIdsPlugin` 插件。               |
...
`compact` 选项在保持长期稳定性的同时，使用字母开头、后续为字母数字的方式编码 id。更密集的 id 空间可以减少包含大量模块的产物中重复出现的模块 id 字节。该选项需要显式启用；生产模式仍默认使用 `deterministic`。
