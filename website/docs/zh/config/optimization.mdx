...
传入配置对象同样会开启模块拼接。`commonjs` 选项用于控制是否拼接导出可静态分析的 CommonJS 模块，默认值为 `true`。将 `commonjs` 设为 `false` 后，仅拼接 ECMAScript 模块。

当 [`output.library.type`](/config/output#outputlibrarytype) 为 `'modern-module'` 时，Rspack 会使用专门的链接和 scope hoist 流程。在该流程中，`optimization.concatenateModules` 仅控制 CommonJS 提升：设为 `false` 或 `{ commonjs: false }` 会阻止 CommonJS 模块参与 scope hoist。
