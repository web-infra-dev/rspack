...
本页会补充 [`splitChunks.enforceSizeThreshold`](#splitchunksenforcesizethreshold) 等常用配置项说明；如果你需要更多背景信息或更完整的插件视角参考，可以继续查看 [SplitChunksPlugin](/plugins/webpack/split-chunks-plugin) 页面。
...
设置一个体积阈值，让 Rspack 在拆分候选 chunk 足够大时继续强制拆分。该选项在生产模式下默认值为 `50000`，在其他模式下默认值为 `30000`。

当候选 chunk 达到这个阈值后，Rspack 会忽略 `splitChunks.maxAsyncRequests` 和 `splitChunks.maxInitialRequests` 这类请求数量限制。

相比 `splitChunks.cacheGroups.{cacheGroup}.enforce`，这个选项更克制一些：只有体积已经足够大的候选才会被强制拆分，较小的候选仍然遵循默认启发式规则。

当使用 `number` 形式配置时，会对 [`splitChunks.defaultSizeTypes`](/plugins/webpack/split-chunks-plugin#splitchunksdefaultsizetypes) 中定义的所有 size type 使用同一个阈值；使用对象形式时，则可以按 size type 分别设置不同阈值。
