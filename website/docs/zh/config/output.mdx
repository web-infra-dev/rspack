...
chunk 产物的格式，默认值会根据 [`target`](/zh/config/target.html) 和 [`output.module`](#outputmodule) 的配置而决定，一般来说，target 为 web 或 webworker 时会使用 `'array-push'` 格式；为 ESM 时会使用 `'module'` 格式；为 node.js 时会使用 `'commonjs'` 格式
...
加载 chunk 的方式，默认值会根据 [`target`](/zh/config/target.html) 和 [`chunkFormat`](#outputchunkformat) 的配置而决定，一般来说，target 为 web 时会使用 `'jsonp'` 来加载 chunk；为 ESM 时会使用 `'import'` 来加载 chunk；为 webworker 时会使用 `'import-scripts'` 来加载 chunk；为 node.js 时会使用 `'require'` 来加载 chunk；为 async node.js 时会使用 `'async-node'`（`fs.readFile` + `vm.runInThisContext`）来加载 chunk
...
详见 [`output.devtoolModuleFilenameTemplate`](/zh/config/output.html#outputdevtoolmodulefilenametemplate).

## output.devtoolModuleFilenameTemplate

- **Type:** `string = 'webpack://[namespace]/[resource-path]?[loaders]'` | `function (info) => string`

此选项仅在 [`devtool`](/zh/config/devtool.html) 使用了需要模块名称的选项时使用。
...
如果多个模块产生相同的名称，使用 [`output.devtoolFallbackModuleFilenameTemplate`](/zh/config/output.html#outputdevtoolfallbackmodulefilenametemplate) 来代替这些模块。

## output.devtoolNamespace

此选项确定 [`output.devtoolModuleFilenameTemplate`](/zh/config/output.html#outputdevtoolmodulefilenametemplate) 使用的模块名称空间。未指定时的默认值为：[`output.uniqueName`](/zh/config/output.html#outputuniquename)。在加载多个通过 Rspack 构建的 library 时，用于防止 source map 中源文件路径冲突。
...
以 ESM 模块的形式输出 JavaScript 文件。由于此功能还处于实验阶段，默认关闭，想要使用的话必须将 [`experiments.outputModule`](zh/config/experiments.html#experimentsoutputmodule) 设置为 true
...
默认值会受不同 [`target`](/zh/config/target.html) 的影响：
...
只在 [chunkLoading](/config/output.html#outputchunkloading) 为 `"jsonp"` 或 `"import-scripts"` 时会被使用。该全局变量会用来请求并加载热更新文件。
