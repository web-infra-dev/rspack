...
默认情况下，仅在检测到代码中使用了 `__rspack_version__` 和 `__rspack_unique_id__` 时才会注入。可将 `force` 设定为 `true` 始终注入：
...
如果你显式开启过强制注入，将 `force` 设定为 `false` 可以恢复按需注入：
