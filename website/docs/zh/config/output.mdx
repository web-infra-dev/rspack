...
入口的命名空间重新导出（`export * as ns from './target.js'` 和 `import * as ns from './target.js'; export { ns };`）会自动优化：在可行时，将目标模块及其同步依赖拆分为独立 chunk，保留原生的命名空间重新导出，让库的使用方可以 tree-shake 未使用的导出。使用 `preserveModules` 时，会复用已有的模块 chunk。后续的 `optimization.splitChunks` 仍可以移动或合并这些 chunk；如果最终 chunk 的导出无法表示目标模块的命名空间，Rspack 会使用命名空间对象渲染方式。
