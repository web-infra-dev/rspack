...
该环境支持使用异步 `import()` 函数来导入 EcmaScript 模块。
...
该环境支持在创建 worker 时使用异步 `import()`，目前仅适用于 web。
...
该环境支持 ECMAScript 模块语法来导入 ECMAScript 模块（`import ... from '...'`）。
...
:::warning
已弃用，请改用 [output.strictModuleErrorHandling](#outputstrictmoduleerrorhandling)。
:::

告诉 Rspack 如果在 `require` 时抛出异常，则从模块实例缓存 (`require.cache`) 中删除该模块。

出于性能原因，它默认为 `false`。

当设置为 `false` 时，模块不会从缓存中删除，这导致异常只会在第一个`require`调用时抛出（这与 node.js 不兼容）。
...
当 `strictModuleExceptionHandling` 设置为 `false` 时，只有第一次 `require` 会抛出异常：

```js
// with strictModuleExceptionHandling = false
require('module'); // <- 抛出异常
require('module'); // <- 不抛出异常
```

相反，当 `strictModuleExceptionHandling` 设置为 `true` 时，所有的 `require` 行为都会抛出异常：

```js
// with strictModuleExceptionHandling = true
require('module'); // <- 抛出异常
require('module'); // <- 也会抛出异常
```
...
在生成哈希时使用的编码方式。支持 Node.JS [`hash.digest`](https://nodejs.org/api/buffer.html#buffers-and-character-encodings) 中的所有编码。
使用 `'base64'` 作为文件名可能会有问题，因为其中包含 `/` 字符。同样 `'latin1'` 也可能包含任何字符。
