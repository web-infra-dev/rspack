...
:::warning
在本地使用 [`@rspack/dev-server`](/config/dev-server) 和 [`HtmlRspackPlugin`](/plugins/rspack/html-rspack-plugin) 调试时，页面入口以及其被 [`output.splitChunks`](/plugins/webpack/split-chunks-plugin) 切分出来的 chunk 会被 HTML 文件以 `script` 标签的方式加载。此时若文件名内包含 Hash 值，则会由于 HTML 无法被动态加载而使得热更新失效。
:::
