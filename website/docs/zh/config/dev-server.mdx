...
- **类型：** `'http' | 'https' | 'http2' | string | object`
- **默认值：** `'http'`
...
:::tip 提示
如果你使用 `server: 'https'` 或 `server: 'http2'`，但没有提供 `server.options.key` 和 `server.options.cert`，则需要在项目中安装 [`selfsigned`](https://www.npmjs.com/package/selfsigned)，以便 dev server 自动生成本地证书。
:::
...
- **类型：** `string | string[] | object | (string | object)[]`

该选项允许你配置需要监听的目录和文件列表，基于 [chokidar](https://github.com/paulmillr/chokidar) 实现。
...
- 更多监听选项请参考 [`chokidar`](https://github.com/paulmillr/chokidar) 文档。
- 如果 [`devServer.liveReload`](#devserverlivereload) 被设置为 `false`，这些被监听文件的变化将不会触发浏览器刷新。

:::

## devServer.webSocketServer

- **类型：** `boolean | 'ws' | string | object`
...
<Attribution url="https://webpack.docschina.org/configuration/dev-server/" />
