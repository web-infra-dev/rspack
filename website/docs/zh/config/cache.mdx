...
Rspack 支持与 webpack 对齐的顶层 `snapshot` 配置，用于持久化缓存。为了兼容已有 Rspack 配置，路径相关选项也可以配置在 `cache.snapshot` 下。如果两处都配置了同一个路径选项，顶层 `snapshot.*` 的值优先于 `cache.snapshot.*`。

`snapshot.contextModule` 必须配置在顶层 `snapshot` 选项中。
...
#### snapshot.managedPaths / cache.snapshot.managedPaths
...
- **默认值：** `{ hash: true, timestamp: true }`

控制构建 context module 时使用的快照策略，例如由带上下文的动态 `import` 或 `require` 创建的模块。

- `hash: true` 会比较递归内容哈希。
- `timestamp: true` 会比较递归时间戳。
- 两者同时启用时，Rspack 会先检查时间戳，并且只在时间戳发生变化后比较哈希，因此内容未变化时仍可以复用缓存。

`hash` 和 `timestamp` 至少应启用一个。两者都关闭会使 context module 快照始终失效。
...
4. 保留与 webpack 对齐的顶层 `snapshot`

Rspack 支持用于持久化缓存的 webpack 顶层 `snapshot` 配置，包括 `snapshot.contextModule`。
