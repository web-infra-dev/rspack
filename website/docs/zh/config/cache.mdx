...
控制 `compilation.fileDependencies` 记录路径的快照策略。`snapshot.dependencies` 配置在顶层 `snapshot` 选项中。

- `hash: true` 会比较文件内容哈希。
- `timestamp: true` 会比较文件时间戳。
- 两者同时启用时，Rspack 会先检查时间戳，并且只在时间戳发生变化后比较哈希，因此内容未变化时仍可以复用缓存。

策略对象未设置 `hash: true` 时，`timestamp` 默认开启。设置 `hash: true` 但不设置 `timestamp: true` 时，Rspack 会使用仅 hash 模式。

### snapshot.contextDependencies

- **类型：** `{ hash?: boolean; timestamp?: boolean }`

- **默认值：** `{ timestamp: true }`

控制 `compilation.contextDependencies` 记录路径的快照策略。`snapshot.contextDependencies` 配置在顶层 `snapshot` 选项中。
...
将路径相关的 snapshot 选项迁移到 `cache.snapshot` 中。依赖策略配置保留在顶层 `snapshot` 选项中，对应 `snapshot.dependencies` 和 `snapshot.contextDependencies`。
...
function transform(webpackConfig, rspackConfig) {
  ...
  rspackConfig.snapshot = {
    dependencies: webpackConfig.snapshot?.module,
    contextDependencies: webpackConfig.snapshot?.contextModule,
  };
