...
Rspack 会将 filesystem cache 存放在按 compiler 划分的目录中。即使多个 compiler 共享同一个 `cache.storage.directory`，清理选项也只会影响当前 compiler 的缓存。

Rspack 会自动清理当前 compiler 下超过 `cache.storage.maxAge` 未访问的缓存文件夹。
:::

#### cache.storage.maxAge

- **类型：** `number`（`0` 到 `4294967295` 之间的整数，单位为秒）

- **默认值：** `7 * 24 * 60 * 60`（7 天）

缓存版本的最大保留时间。超过该时间未被访问的版本会在缓存清理时被删除。设置为 `0` 可以禁用基于时间的清理。
...
- **类型：** `number`（`0` 到 `4294967295` 之间的整数）

- **默认值：** 不限制 generation 数量；仍会执行基于时间的清理

每个编译器最多保留的缓存 generation 数量。超过限制时，Rspack 会删除该编译器最久未访问的 generation。使用同一存储目录的其他编译器不会计入此限制。设置为 `0` 时会在清理时删除之前的 generation。
...
每个编译器会单独计数，因此共享 `storage.directory` 的编译器不会占用彼此的限额。

filesystem cache 会按 compiler 划分，因此该限制只会统计当前 compiler 的 generation。

:::warning
同一编译器的并发构建共享此限制，因此可能会删除彼此较旧的缓存版本。
:::

### 查看持久化缓存日志

持久化缓存相关日志会通过 `rspack.persistentCache` logger 输出。可以开启 `stats.logging`，在构建 stats 中查看这些日志：
...
如果只想打印 `rspack.persistentCache` 相关日志，可以将 `stats.logging` 设置为 `false` 来关闭其他 logger 的输出，并通过 `stats.loggingDebug` 包含这个 logger：
