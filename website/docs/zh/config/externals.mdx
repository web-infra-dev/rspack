...
:::warning
带有 `{ root, commonjs, commonjs2, amd, ... }` 的对象只允许用于 [`library.type: 'umd'`](/config/output#outputlibrarytype) 和 [`externalsType: 'umd'`](#externalstype)。其他库的 target 不允许这样做。
:::
...
如果需要根据模块请求或其他模块信息决定是否将依赖外部化，可以使用函数。函数可以直接返回结果、返回 Promise，或调用回调。

例如，下面的配置将 `react` 及其子路径保留为 ES 模块导入：
...
使用产物的环境需要能够解析这些导入，例如通过另一个打包工具或浏览器的 import map。

同步返回结果时，函数应只声明一个参数，例如 `({ request }) => ...`。Rspack 通过函数的参数个数区分同步返回和回调用法。需要异步处理时，可以使用 `async` 函数或返回 Promise。

#### 请求信息 \{#request-data}

第一个参数包含依赖的相关信息：

- `request`：源码中的模块请求，例如 `react` 或 `./utils.js`，此时尚未解析为文件路径。
- `context`：解析请求时使用的起始目录。
- `dependencyType`：依赖类别，例如 `esm` 或 `commonjs`。
- `contextInfo.issuer`：发起请求的模块路径。
- `contextInfo.issuerLayer`：发起请求的模块所属的层（如果已设置）。
- `getResolve`：使用当前解析选项创建解析函数，详见[解析请求](#resolving-requests)。

#### 返回结果 \{#results}

直接返回的值、Promise 的结果和回调的结果具有相同含义：

- **字符串、字符串数组或对象**：按对应的值格式将请求外部化，详见[字符串](#字符串)、[字符串数组](#字符串数组)和[对象](#对象)。
- **`true`**：使用原始请求名称和 [`externalsType`](#externalstype) 将请求外部化。
- **`false`**：停止检查当前 `externals` 配置的剩余规则，不将请求外部化。其他 `ExternalsPlugin` 实例仍可能将其外部化。
- **`undefined`**：继续检查下一条规则。使用回调时，调用 `callback()`。

如果没有规则将请求外部化，Rspack 会继续正常的模块处理流程。若要为单个结果覆盖 `externalsType`，可以添加 `'window _'` 这样的类型前缀，或通过回调的第三个参数指定类型。

#### 回调用法 \{#callback-usage}

第二个参数是回调函数。失败时调用 `callback(error)`，提供结果时调用 `callback(null, result, externalsType?)`，继续检查其他规则则调用 `callback()`。使用这种形式时，每个分支都应调用回调，而不是直接返回结果。
...
#### 解析请求 \{#resolving-requests}

如果需要根据解析后的路径或包元数据决定是否将依赖外部化，可以使用 `getResolve()`。例如，在接收 `{ context, request, getResolve }` 的异步外部依赖函数中：
...
可以向 `getResolve(options)` 传入[解析选项](/config/resolve)，定制解析函数。返回的函数也支持回调：`resolve(context, request, (error, resolvedPath, resolveData) => { ... })`。

解析成功时会返回路径。如果请求被忽略，例如其别名被设为 `false`，Promise 的结果为 `undefined`，回调的第二个参数为 `false`。解析失败时，Promise 会被拒绝，或通过回调的第一个参数传递错误。

使用回调时，可选的第三个参数包含解析详情。其中的 `descriptionFileData` 字段在有值时是解析后的 `package.json` 对象，可以直接读取 `name`、`version`、`type` 等字段，无需调用 `JSON.parse()`。
...
<Attribution url="https://webpack.docschina.org/configuration/externals/" />
