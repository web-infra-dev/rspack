...
exports.checkChunkModules = function checkChunkModules(
	...
	for (const chunkId of Reflect.ownKeys(chunkModulesMap)) {
		...
		const expectedModules = new Set(...chunkModulesMap[chunkId]);
		const chunkModules = new Set(...chunk.modules.map(m => m.identifier));

		if (strict) {
			expectedModules.size == chunkModules.size;
		}
