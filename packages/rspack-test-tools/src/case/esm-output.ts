...
import { build, check, checkSnapshot, compiler, configMultiCompiler, findMultiCompilerBundle, run } from "./common";
	...
	steps: ({ name }) => [
		{
			...
			run: async (env: ITestEnv, context: ITestContext) => {
				await run(env, context, name, (context: ITestContext) =>
					findMultiCompilerBundle(context, name, (_index, _context, options) => {
