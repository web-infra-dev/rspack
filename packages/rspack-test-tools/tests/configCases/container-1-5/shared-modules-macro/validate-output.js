...
module.exports = function validateOutput(outputPath) {
	...
	const pureAnnotations =
		mainJs.match(/\/\*\s*#__PURE__\s*\*\/\s*__webpack_require__/g) || [];
	const treeshakingMacros =
		mainJs.match(
			/\/\*\s*@common:if\s*\[condition="treeShake\.[^"]+"\]\s*\*\//g
		) || [];

	// Shared modules should have PURE annotations
	if (pureAnnotations.length === 0) {
		throw new Error(
			"Expected PURE annotations in shared modules, but found none"
		);
	}
	...
	if (treeshakingMacros.length === 0) {
		throw new Error(
			"Expected tree-shaking macros in shared modules, but found none"
		);
	}
