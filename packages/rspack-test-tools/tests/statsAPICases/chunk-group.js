...
module.exports = {
	...
	async check(stats) {
		...
		// entrypoints
		expect(string).toContain(`Entrypoint main 13.6 KiB (662 bytes) = main.js 13.6 KiB (main.js.map 662 bytes)`);
		expect(string).toContain(`prefetch: chunk.js 841 bytes {919} (name: chunk) (chunk.js.map 514 bytes)`);

		// chunk groups
		expect(string).toContain(`Chunk Group chunk 841 bytes (514 bytes) = chunk.js 841 bytes (chunk.js.map 514 bytes)`);
