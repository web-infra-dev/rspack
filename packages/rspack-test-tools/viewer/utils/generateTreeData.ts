...
import { ECompareResultType } from '../../../packages/rspack-test-tools/src/type';

export const generateTreeData = (root: string, rawData: Array<{
  name: string;
  source: string;
  dist: string;
      ...
      if (i === splitPath.length - 1) {
        ...
        if (parentDirectory) {
          if (fileMap.has(fileMeta.key!)) {
            parentDirectory.children![fileMap.get(fileMeta.key!)!].data!.wasted = true;
