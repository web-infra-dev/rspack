...
export const DiffReportPage: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fakeSummaryItem: TDiffItem = {
        name: 'summary',
        source: stats.data.filter(i => [ECompareResultType.OnlySource, ECompareResultType.Same, ECompareResultType.Different].includes(i.type)).map(i => i.name).sort().join('\n'),
        dist: stats.data.filter(i => [ECompareResultType.OnlyDist, ECompareResultType.Same, ECompareResultType.Different].includes(i.type)).map(i => i.name).sort().join('\n'),
        type: stats.data.every(i => [ECompareResultType.Different, ECompareResultType.Same].includes(i.type)) ? ECompareResultType.Same : ECompareResultType.Different,
      };
  ...
  const onTreeChange = useCallback((name: string) => {
    ...
    <Layout hasSider>
      ...
      >
        {stats.data.length > 0 && current && (
          <TreeView onChange={onTreeChange} treeData={treeData} current={current.name} />
        )}
      </Sider>
      <Layout style={{ marginLeft: 400, height: '100vh' }}>
        <Content>{current && <DiffEditor format={current.name !== 'summary'} source={current.source || ''} dist={current.dist || ''} />}</Content>
      </Layout>
    </Layout>
  );
};