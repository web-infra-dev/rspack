...
import {
	type HandleIncomingRequest,
	type HandleIncomingRequestSync,
	RequestSyncType,
	RequestType
} from "./service";
...
export async function runLoaders(
	...
	const getWorkerLoaderContext = () => {
		const workerLoaderContext = {
			...
			remainingRequest: loaderContext.remainingRequest,
			currentRequest: loaderContext.currentRequest,
			previousRequest: loaderContext.previousRequest,
	...
	const getWorkerLoaderHandlers = function (): {
		...
		return {
			handleIncomingRequestSync(requestType, ...args) {
				switch (requestType) {
					case RequestSyncType.GetData: {
						return loaderContext.data;
					}
					case RequestSyncType.SetData: {
						loaderContext.data = args[0];
						break;
					}
				}
			},
	...
	const useParallel = () => {
		return (
			...
			getCurrentLoader(loaderContext)?.parallel
		);
	};

	const startSpan = function () {
		const pitch = loaderState === JsLoaderState.Pitching;
		const worker = useParallel();
