...
class Compiler {
	...
	get #instance() {
		const processResource = (
			...
			NormalModule.getCompilationHooks(this.compilation)
	...
	#updateDisabledHooks() {
		...
		const hookMap = {
			...
			beforeResolve:
				this.compilation.__internal__normalModuleFactory?.hooks.beforeResolve,
			afterResolve:
				this.compilation.__internal__normalModuleFactory?.hooks.afterResolve,
	...
	async #beforeResolve(resolveData: binding.BeforeResolveData) {
		...
		let ret =
			await this.compilation.__internal__normalModuleFactory?.hooks.beforeResolve.promise(
	...
	async #afterResolve(resolveData: binding.AfterResolveData) {
		let res =
			await this.compilation.__internal__normalModuleFactory?.hooks.afterResolve.promise(
				resolveData
			);
	...
	async #contextModuleBeforeResolve(resourceData: binding.BeforeResolveData) {
		let res =
			await this.compilation.__internal__contextModuleFactory?.hooks.beforeResolve.promise(
				resourceData
			);
	...
	async #normalModuleFactoryResolveForScheme(
		...
		let stop =
			await this.compilation.__internal__normalModuleFactory?.hooks.resolveForScheme
				.for(input.scheme)
	...
	#compilation(native: binding.JsCompilation) {
		...
		this.hooks.compilation.call(this.compilation, {
			normalModuleFactory: this.compilation.__internal__normalModuleFactory!
		});
	...
	#newCompilation(native: binding.JsCompilation) {
		...
		this.compilation.__internal__normalModuleFactory = normalModuleFactory;
		this.hooks.normalModuleFactory.call(normalModuleFactory);
		this.compilation.__internal__contextModuleFactory = contextModuleFactory;
