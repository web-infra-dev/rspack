...
export class TestContext implements ITestContext {
	...
	setOptions<T extends ECompilerType>(
		fn: (options: TCompilerOptions<T>) => TCompilerOptions<T>,
		name = DEFAULT_COMPILER_NAME
	) {
		const compiler = this.getCompilerManage<T>(name);
		compiler.setOptions(this, fn);
	}
	setCompiler<T extends ECompilerType>(
		fn: (
		...
		const compiler = this.getCompilerManage<T>(name);
		compiler.setCompiler(this, fn);
	}
	setStats<T extends ECompilerType>(
		fn: (
			compiler: TCompiler<T> | null,
		...
		const compiler = this.getCompilerManage<T>(name);
		compiler.setStats(this, fn);
	}
	setResult<T extends ECompilerType>(
		fn: <R>(compiler: TCompiler<T> | null, result: R) => R,
		name = DEFAULT_COMPILER_NAME
	) {
		const compiler = this.getCompilerManage<T>(name);
		compiler.setResult(this, fn);
	}
