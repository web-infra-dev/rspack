...
describe(
	...
	() => {
				...
				const plugin: RspackPlugin = {
					...
					async onResolve(context) {
						if (context.importee === "foo") {
							return { uri: path.join(fixture, "foo.js"), external: false };
						}
					},
				};
		...
		it(
			"should generate correct source map",
			async () => {
				...
				const meta1 = consumer.originalPositionFor({ line: 3, column: 4 });
				expect(meta1.line).to.eq(1);
				expect(meta1.column).to.eq(0);
				expect(meta1.source.includes("index.js")).to.be.true;

				const meta2 = consumer.originalPositionFor({ line: 4, column: 4 });
